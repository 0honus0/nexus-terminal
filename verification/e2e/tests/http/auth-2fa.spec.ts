...
test('2FA can be enabled, required at login, verified, and disabled', async ({ request }) => {
  ...
  let secret = '';
  await test.step('start and activate TOTP 2FA', async () => {
  ...
  await test.step('password login is blocked behind the second factor', async () => {
  ...
  await test.step('a real current TOTP completes login', async () => {
  ...
  await test.step('2FA can be disabled only with the current password', async () => {
