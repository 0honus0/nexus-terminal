...
- **技术债务复查（2026-04-11）**：完成一轮基于仓库现状的技术债务重审
  ...
  - **日志治理（最新）**：当前仅保留最新批次记录。第二十七批（并行降级执行）在 `packages/frontend/src/composables/file-manager/useFileManagerDragAndDrop.ts`、`packages/backend/src/ssh-suspend/ssh-suspend.service.ts`、`packages/frontend/src/components/LayoutRenderer.vue` 完成 23 处信息级输出降级（`console.log` -> `console.info`），存量由 174 降至 151
