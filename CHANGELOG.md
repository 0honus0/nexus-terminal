...
- **技术债务复查（2026-04-11）**：完成一轮基于仓库现状的技术债务重审
  ...
  - **日志治理（最新）**：当前仅保留最新批次记录。第三十二批（并行降级执行）在 `packages/backend/src/sftp/sftp-archive.manager.ts`、`packages/backend/src/notifications/notification.dispatcher.service.ts`、`packages/backend/src/connections/connections.controller.ts`、`packages/backend/src/ai-ops/nl2cmd.service.ts` 完成 16 处信息级输出降级（`console.log` -> `console.info`），存量由 95 降至 79
