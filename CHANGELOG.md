...
- **技术债务复查（2026-04-11）**：完成一轮基于仓库现状的技术债务重审
  ...
  - **日志治理（最新）**：当前仅保留最新批次记录。第二十八批（并行降级执行）在 `packages/frontend/src/composables/useAddConnectionForm.ts`、`packages/frontend/src/components/FileEditorTabs.vue`、`packages/backend/src/terminal-themes/terminal-theme.repository.ts` 完成 14 处信息级输出降级（`console.log` -> `console.info`），存量由 151 降至 137
