...
test('RDP and VNC form branches persist correctly and filtered batch selection deletes exactly the intended connections', async ({ page, context }) => {
  ...
  try {
    ...
    await step('select-all obeys the active filter and invert selection operates on the restored full list', async () => {
    ...
    await step('deselect/select-all followed by batch delete removes both records from UI and persistence', async () => {
      ...
      const success = page.getByRole('dialog').filter({ hasText: 'Selected connections have been successfully deleted.' });
