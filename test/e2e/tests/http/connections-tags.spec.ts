...
test('background SSH resource refresh does not change recent connection time', async ({ request }) => {
  ...
  try {
    ...
    const readConnection = async () => {
      ...
      return (await response.json()) as { connection?: { lastConnectedAt: number | null }; lastConnectedAt?: number | null };
    };
    ...
    try {
      await expect
        .poll(async () => lastConnected(await readConnection()), { timeout: 10_000 })
        .not.toBeNull();
    } finally {
