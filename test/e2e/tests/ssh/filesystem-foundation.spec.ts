...
const query = (values: Record<string, string | number>): string => new URLSearchParams(
...
test('HTTP download APIs consume the shared remote filesystem boundary', async ({ request }) => {
  ...
  try {
    ...
    const inlineResponse = await request.get(`/api/v1/sftp/download?${query({
      connectionId,
      sessionId: session.sessionId,
      remotePath: '/seed.txt',
      disposition: 'inline',
    ...
    const directoryResponse = await request.get(`/api/v1/sftp/download-directory?${query({
      connectionId,
      sessionId: session.sessionId,
