...
const visibleFileManagerRow = (page: Page, filename: string) =>
  visibleFileManagerList(page).locator(`tr[data-filename="${filename}"]`);
...
test('a non-fatal WebSocket error does not leave file manager context actions greyed out', async ({
  page,
  context,
}) => {
        ...
        () =>
          receivedFrames.some((frame) => {
            try {
              ...
              return (
                message?.type === 'error' && String(message?.payload ?? '').includes('e2e:unsupported-nonfatal-message')
              );
            } catch {
              return false;
            }
  ...
  await step('right-click actions remain enabled and Refresh still performs real SFTP work', async () => {
    ...
    const fixtureResponse = await fetch(`${E2E_SSH.controlUrl}/fixture?name=${encodeURIComponent(fixtureName)}`, {
      method: 'POST',
    });
    ...
    for (const label of ['Copy', 'Rename', 'Delete', 'Refresh']) {
      await expect(contextMenu(page).getByText(label, { exact: true }).first().locator('..')).toHaveAttribute(
        'aria-disabled',
        'false',
      );
    }
...
test('switching SSH sessions without a page reload keeps file manager context actions bound to the active SFTP session', async ({
  page,
  context,
}) => {
  await loginAsInitialAdmin(context.request);
  ...
  await step('open a second SSH session while the existing file manager component stays mounted', async () => {
    ...
    await expect
      .poll(
        () =>
          page.evaluate(async (oldSessionId) => {
            const { useSessionStore } = await import('/src/stores/session.store.ts');
            const store = useSessionStore();
      ...
      .toEqual({ changed: true, connected: true, sftpReady: true });
