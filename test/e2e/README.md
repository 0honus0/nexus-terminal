...
`test:docs` is reserved for user-facing feature presentation. It exports full-interface screenshots to `doc/imgs/e2e/`. When the runner image changes and the grouped Playwright E2E jobs succeed, the E2E workflow regenerates those screenshots and commits the environment plus screenshot refresh together. A manual E2E workflow dispatch also regenerates the screenshots after grouped E2E succeeds, even when the runner image is unchanged. Docker deployment smoke is an independent terminal validation branch and does not gate screenshot refresh or timing rebalance.
