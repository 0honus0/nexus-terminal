...
Documentation screenshots are generated by the `screenshots` job inside `.github/workflows/e2e.yml`. The docs project reuses the E2E helpers and writes full-viewport screenshots to `doc/imgs/e2e/`. Screenshot refresh runs only after the E2E runner environment changed and the complete E2E/build pipeline passed.
