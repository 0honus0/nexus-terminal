...
Documentation screenshots are generated by the `screenshots` job inside `.github/workflows/e2e.yml`. The docs project reuses the E2E helpers and writes full-viewport screenshots to `doc/imgs/e2e/`. Screenshot refresh runs only after the E2E runner environment changed and the grouped Playwright E2E path passed.
...
`test:docs` is reserved for user-facing feature presentation. It exports full-interface screenshots to `doc/imgs/e2e/`. When the runner image changed and the grouped Playwright E2E jobs succeed, the E2E workflow regenerates those screenshots and commits the environment plus screenshot refresh together. Docker deployment smoke is an independent terminal validation branch and does not gate screenshot refresh or timing rebalance.
