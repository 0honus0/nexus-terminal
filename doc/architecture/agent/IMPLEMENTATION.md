...
| P3-A | MCP/ACP/Browser-CDP live adapters；ACP 由 Runner workspace-profile process stream 承载，Browser 由 Backend `BrowserRuntimeAdapter` 持语义/Puppeteer、Runner 可选提供 CDP tunnel | P2-C；mcp.spec.ts + Docker deployment smoke 的 ACP/Browser production-path 验收                      |
...
### 0.2 Phase 3 当前 live execution 契约

- **ACP：Active / live。** `nexus.operations` 声明并默认管理 `integration.acp.execute`；`acp_execute` 是 mutation tool，inspection 冻结 integration version、Workspace generation 与 ACP profile revision，随后使用现有 Capability / Policy / Approval / Lease / StateCommit。Backend `AcpAdapter` 不 spawn backend；Runner `AcpProcessRuntime` 在冻结 generation 的 bubblewrap sandbox 中持有 child process/stdio，并通过 authenticated binary WebSocket 暴露 bounded stream。ACP direct fs/terminal request deny；协议内 `session/request_permission` 当前固定 `reject_once`，不把外层批准传播成 nested permission。
- **Browser/CDP/Puppeteer：Active / live。** 最终 owner 是 Backend `BrowserRuntimeAdapter`：Puppeteer、BrowserContext、Page、CDPSession、snapshot/nodeRef 与 create/navigate/snapshot/click/type/close 都在 Backend。endpoint `via=backend` 时直接连接；`via=runner` 时 Runner `BrowserTunnelRuntime` 只代理 bounded text CDP frame，不含 Puppeteer/Browser semantic API。这样 Runner unavailable 不阻断 standalone direct Browser target。
- Browser endpoint `scope` 只表示 reachability：`docker-network | external-network`；后者包括宿主、其它 Docker network、LAN/VPN/VPC 与公网。`http/https/ws/wss`、`allowPlaintext`、`verifyTls` 独立配置。Agent tool 只能引用 `targetId` 或 Workspace 冻结 target，不接受 endpoint URL。Standalone session 绑定 Agent Settings revision；Workspace session 绑定 `workspaceId + generation + targetRevision`。
- Browser Page 开启 request interception，HTTP(S) navigation/redirect/subresource 全部按 `allowedUrlPatterns` 检查；popup close、download deny。Tool Catalog 不提供 `evaluate`、raw CDP、任意 selector/JS fallback；navigate/click/type 后旧 `snapshotId + nodeRef` 失效。
- **Workspace local Terminal：Active / live。** `WorkspaceRuntimeInteractiveSessionPort` 与 `WorkspaceRuntimeTerminalService` 独立于 one-shot job。Browser 使用 `/ws/agent-terminal` 连接 Backend；Backend `RunnerWorkspaceTerminalAdapter` 用临时 Ed25519 key 在 authenticated Runner WS 上建立 SSH；Runner `WorkspaceTerminalRuntime` 在 generation sandbox 内启动 ephemeral Dropbear，Dropbear 持有 PTY/shell。attach/replay/resize/signal/close 全部绑定 `workspaceId + generation`；detach grace 30s、replay 1 MiB。它不复用高层远程 SSH `WorkspaceTerminalService`。
- **Runner optional**：Agent Host/Run/Provider/MCP/Plugin Host 与 `via=backend` Browser 不依赖 Runner；Workspace job、ACP、Runner Plugin、Workspace local Terminal 才要求 Runner availability。
...
| runtime/scheduling/scheduler.ts                           | Root Run 的进程内 App round-robin dispatcher、按用户 Runtime 配额、park/wake/quiesce；durable child work 属于 collaboration scheduler                                                         |
...
| exchange/workspace-artifact.*                             | Workspace Runtime ↔ durable Artifact 的显式跨资源桥；桥接层可依赖两侧 public port，两侧不互相反向依赖                                                                                         |
...
| capabilities/tool-target.types.ts                         | Tool target fingerprint/workspace identity 等稳定 target 类型；不把 target 当 capability dispatcher                                                                                           |
...
| workspace-runtime/workspace-runtime.types.ts              | Workspace/Profile/Generation、Catalog/Storage/Command/JobResult                                                                                                                               |
| workspace-runtime/workspace-runtime.service.ts            | availability/catalog/storage、createWorkspace/action/tool-version switch；按 scope 验证 Workspace 所有权；create/start/restart 重新检查 `workspace.runtime.manage`，stop/delete 保留 teardown |
| workspace-runtime/workspace-runtime-management.service.ts | setup/Tool Pack uninstall/runtime cleanup/settings reset 的 preview-confirm 管理                                                                                                              |
| workspace-runtime/workspace-runtime-controller.port.ts    | Backend→Runner command/catalog/storage/workspace Host 接口；不提供任意 Docker 方法                                                                                                            |
| workspace-runtime/workspace-runtime-gateway.port.ts       | Agent Workspace job invoke/query 的受限执行接口；one-shot job 不暴露 Runner process/PTY handle                                                                                                |
...
Concrete adapters：packages/backend/src/infrastructure/agent/ 下 repositories/sqlite-{app,provider,conversation,run,artifact,approval,lease}.repository.ts + workspace-runtime/sqlite-workspace.repository.ts，repositories/sqlite-state-commit.ts，providers/openai-compatible.adapter.ts，providers/provider-secret.adapter.ts，providers/outbound-policy.adapter.ts，artifacts/local-artifact-store.ts，workspace-runtime/runner-http.adapter.ts，integrations/{mcp,acp}.adapter.ts。不要为每个方法创建一层wrapper。
...
Backend allowed子图与架构§2一致；host不能导出raw registry/store给插件，ai/runtime/capabilities/workspace-runtime 禁止 import apps，只有bootstrap能import所有contribution。Frontend host只依赖app-contribution.types，不import apps组件；builtin-apps.ts作为静态组合例外只用lazy public.ts。runtime不依赖apps，apps/<a>不能依赖apps/<b>。技术SDK仅Infrastructure。
...
Backend Runtime 当前物理目录固定拆为 `runtime/definitions`、`runs`、`execution`、`planning`、`scheduling`、`approvals`、`recovery`、`events`、`collaboration`；跨资源 Workspace↔Artifact bridge 位于独立 `agent/exchange`。这是实现模块边界，不增加领域实体。禁止把后续功能重新堆回 `runtime/*.ts` 一级；跨资源协调（例如 Workspace↔Artifact）放独立 `agent/exchange`，Workspace Runtime 与 Artifact 各自保持单向职责。
...
| Workspace stopped/ready→starting→running      | start 且 Profile/ToolRef/配额有效                            | command journal记录；generation 与 Workspace status 一致更新                                        |
| Workspace running→stopping→stopped            | stop及确认 generation process 退出                           | 稳定 Workspace 文件保留，Artifact不删                                                               |
| Workspace stopped/failed→deleting→deleted     | 无未决 job 且进入 teardown                                   | generation sandbox/process 清理确认后 tombstone；稳定 Workspace 是否清理由 runtime cleanup 单独决定 |

transitionRun(current,event,expectedVersion)、transitionTool 与 Workspace status CAS 只接受此表白名单；HTTP不允许直接PATCH status。version冲突409 STATE_CONFLICT，调用方重新snapshot，禁止blind retry mutation。Run cancelled/interrupted/failed仍保存用户输入和证据。

### 2.2 Scheduler、输入、retry、checkpoint算法

Root scheduler 为每 Thread 一个活跃 Run、单管理员默认最多 2 个 executing AgentRuntime、每 Run 工具串行；durable `created` Run 最多 20，超过 429 RUN_QUEUE_FULL；进程内 dispatcher 按 App 轮转并对当前进程内 queued Run FIFO。Root dispatcher 本身不持久化：`createRun` 事务先写 Run/Input/Runtime/Event，commit 后才 enqueue；若 Backend 在 commit 后、enqueue 前退出，下一次启动会由 `interruptNonTerminalRuns()` 把该 Run 明确转为 `interrupted`，不会静默卡在 created/running，也不会自动续跑旧 Run。root 与 Subagent 使用同一 `maxConcurrentRuntimes`，不设累计 Subagent 数或独立 child 并发额度；waiting/join/approval/budget 状态释放执行slot。`maxActiveExecutionSeconds`按Run wall-clock去重计时：executing_runtime_count从0→1时记录active_execution_started_at，1→0时把经过秒数累计进active_execution_seconds；并发从1→N或N→1不重复计费，状态更新随permit投影持久化，重启可据此收敛。模型调用并发默认`auto`，解析为当前有效`maxConcurrentRuntimes`（默认2）；用户可在Agent Settings里主动下调；有效模型并发不能高于Runtime并发，Provider自身429/限流按运行时背压处理。三期Subagent只按深度、依赖、Run预算和Runtime slot排队，预留父预算，不单独获得新总预算；join期间root释放model/tool/Runtime permit。
...
| maxActiveWorkspaces                                            | 4 / 8                                      | 全局 active Workspace 数量，与 AgentRuntime 数量独立；Hard Limit可在Settings二次确认调整                                                                                                                                                      |
| maxActiveWorkspaces（workspaceRuntime requested / Hard Limit） | 4 / 8                                      | 用户请求值受同名 Hard Limit 约束；每个 AgentRuntime 当前最多一个非 deleted/failed root Workspace                                                                                                                                              |
...
| workspaceIdleTtlSeconds                                        | 900 / 3600                                 | 有未决 job 不 idle；Workspace 本身不设绝对寿命，具体job/command各自有deadline                                                                                                                                                                 |
| modelRetryCount/estimateMargin                                 | 2/0.15，固定                               | 最多3attempt；缺usage按预留上界结算                                                                                                                                                                                                           |

### 3.1 Agent Settings 分区与可调边界

Agent Settings使用同一个`agent_settings.value_json`保存版本化、namespaced配置，不为每个前端小模块单独建表。推荐schema=`{schemaVersion,feature:{enabled},model,performance,budget,hardLimits,subagents,storage,workspaceRuntime,safety}`；`feature.enabled`默认true，控制Agent Host是否接受/调度执行；Provider credential仍保存在`ai_providers`并由SecretCipher保护，Settings只保存引用、默认选择和非密钥偏好。服务端`validateSettings()`负责默认值、旧schema迁移、交叉约束；Hard Limits自身就是实例策略上限，不再与另一套隐藏资源上限取min。前端另外展示`runtimeCapabilities`，用于提示当前模型/Provider/宿主/Controller是否实际上承载得住该设置。

| 设置模块          | 用户可调项                                                                                                                                                                                                          | 规则                                                                                                                                                                                                                                                                                                                                |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 功能开关          | `feature.enabled`                                                                                                                                                                                                   | 默认开启。关闭先禁止新Run/新step/新 Workspace Runtime claim并进入Host quiesce，模型stream可abort，未started工作取消；已started mutation必须确认结果或quarantine。关闭完成后Launcher隐藏；Settings及安全cleanup/审计仍可访问，历史Thread/Run/Artifact/配置数据保留但Hub不再提供交互，重新开启后可重新浏览；再次开启不自动resume旧Run |
...
CREATE TABLE agent_workspaces (
 ...
 agent_runtime_id TEXT NOT NULL, retained INTEGER NOT NULL DEFAULT 0 CHECK(retained IN (0,1)),
 ...
 retained_manifest_ref TEXT REFERENCES ai_artifacts(id), version INTEGER NOT NULL DEFAULT 1 CHECK(version>0),
...
CREATE TABLE agent_workspace_runtime_commands (
 id TEXT PRIMARY KEY, workspace_id TEXT REFERENCES agent_workspaces(id) ON DELETE CASCADE,
 user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, app_id TEXT NOT NULL,
 action TEXT NOT NULL, operation_hash TEXT NOT NULL, generation INTEGER NOT NULL CHECK(generation>=1),
...
type ToolInspection = {
  ...
  target: TargetFingerprint | { workspaceId: string; generation: number };
  resourceKeys: string[];
...
Backup默认沿用既有DB快照能力，manifest记录agentSchemaRevision、artifactPayloadIncluded=false、pluginsExcluded=true、workspaceRuntimeExcluded=true；带加密credential的备份遵守既有备份保护，不能另行导出明文。二期includeAgentArtifacts=true先quiesce相关写、冻结ready对象清单和引用计数，再复制到备份临时区/hash校验并释放冻结，备份生成本身有全局磁盘reserve；生成完才发布下载。
...
所有下列路由继承现有session/IP/2FA边界。Interface从session推导userId，路径校验appId并交service授权，不信任body.userId。成功={data,requestId}，错误={error:{code,message,details?},requestId}，仅Agent新API采用此格式；X-Request-Id合法UUID可沿用，否则生成。POST/PATCH/PUT/DELETE按§4幂等规则，commandId不进模型提示。分页limit默认50/max100，opaque before包含稳定(time,id)；非法cursor400。 `feature.enabled=false` 只关闭Agent任务执行，不关闭管理员配置/维护：Settings、Provider配置/test、App启停、Host summary、历史Thread/Run/Artifact读取、文件cleanup、Workspace Runtime Catalog/Tool Pack setup/install/uninstall/cache/runtime cleanup与reconcile仍可访问。创建Thread/Run、向Run追加会触发执行的输入、预算恢复、Agent Workspace 的 provision/start、Tool/Model/Subagent等新执行命令统一返回409 `AGENT_DISABLED`。正在执行的旧任务只允许走quiesce/cancel/reconcile终态路径，不能因总开关关闭绕过安全收敛。

Agent mutation额外要求X-Nexus-CSRF=session绑定的256-bit token；GET /api/v1/agent/security/csrf（no-store）只对已登录同源请求签发。Token 使用服务端 SESSION_SECRET 对 `nexus-agent-csrf-v1\0 || sessionID` 做 HMAC-SHA256 派生，不把可变CSRF值写入session store；因此同一登录session内稳定、session regenerate后自动失效，也不会被并发session touch的旧快照覆盖。Origin存在必须精确匹配部署AGENT_PUBLIC_ORIGIN；生产无该配置拒绝启用mutation，dev允许明确loopback origin。Sec-Fetch-Site为cross-site拒绝；Origin缺失的非浏览器调用仍须session+CSRF且不能带cross-site标记。只信配置的反向代理来源，不用任意X-Forwarded-Host构造允许origin。认证失败401，scope不匹配404，capability/hard deny403；错误details不得含密文/内部路径。
...
| POST /agent/workspace-runtime/setup/preview、/confirm                                     | 初始化/补装所选Recipe与多个Pack版本；confirm→202 WorkspaceRuntimeCommandView；P2                                                                                                                                                                     |
| POST /agent/workspace-runtime/tool-packs/:familyId/:versionId/install                     | →202 PackInstallCommand；安装/预拉取精确 Tool Pack，不创建 Workspace                                                                                                                                                                                 |
| POST /agent/workspace-runtime/tool-packs/:familyId/:versionId/uninstall/preview、/confirm | preview 引用/默认版本/可释放空间；confirm 卸载精确 Pack，不影响同 family 其他版本；active 引用返回409 WORKSPACE_TOOLCHAIN_IN_USE                                                                                                                     |
| POST /agent/workspace-runtime/runtime-cleanup/preview、/confirm                           | preview active jobs/运行资源/预计释放空间；confirm 只清 runtime/container/network/job等运行残余，保留Pack/cache/Settings/Artifact；未知副作用返回PARTIAL_RESTORE                                                                                     |
| POST /agent/workspace-runtime/cache-cleanup                                               | →202 WorkspaceRuntimeCommandView；只清安全可重建cache，不卸载Pack、不清active runtime；P2                                                                                                                                                            |
| POST /agent/workspace-runtime/settings/reset/preview、/confirm                            | 恢复发行默认Recipe/版本/资源/网络配置；必须二次确认；P2                                                                                                                                                                                              |
| GET /agent/workspace-runtime/commands/:commandId                                          | →queued/running/succeeded/failed/partial/unknown + stage/progress/result；所有安装/卸载/setup/cleanup长命令可断线后按commandId恢复；P2                                                                                                               |
...
| POST /apps/:appId/runs/:runId/workspaces                                                  | `{workspace:{recipeId,versions?,runnerPluginIds?,limits?,network?},retained,catalogRevision?}`→202 `AgentWorkspaceView`；Backend 从 Run 解析 root `agentRuntimeId`；需 `workspace.runtime.manage`；同 runtime 仅一个非 deleted/failed root Workspace |
| GET /apps/:appId/runs/:runId/workspaces                                                   | →`AgentWorkspaceView[]`；列表按 `runId` scope，root runtime ownership 由 Backend/repository 约束                                                                                                                                                     |
| GET /apps/:appId/workspaces/:workspaceId                                                  | →`AgentWorkspaceView`                                                                                                                                                                                                                                |
| POST /apps/:appId/workspaces/:workspaceId/actions                                         | `{action:'start'                                                                                                                                                                                                                                     | 'stop'                                                                                | 'restart'                                                                                                                                                                                                                                                                                        | 'delete' | 'setNetwork' | 'resize',expectedVersion,parameters?}`→202 `WorkspaceRuntimeCommandView` |
| POST /apps/:appId/workspaces/:workspaceId/tool-versions                                   | `{versions:{familyId:versionId},expectedVersion,catalogRevision?}`→`WorkspaceToolchainSwitchView`；CAS reserve 后替换 generation，稳定 Workspace 与其他 Workspace 不变                                                                               |
...
HostEvent={sequence,schemaVersion:1,type:'summary.changed'|'feature.changed'|'app.changed'|'authorization.changed',occurredAt,payload:bounded invalidation metadata}。payload 只携带触发刷新所需的最小域信息（例如 Run summary ref、App state delta、feature revision、authorization revision），不复制完整 HostSnapshot，也不含 message/tool/Artifact 正文或完整 Settings；Browser 收到 durable HostEvent 后重新读取 canonical Host summary。
...
| host/AgentHubWindow.vue、host/builtin-apps.ts、apps/operations/public.ts                                  | Hub 只通过 `builtin-apps.ts` 取得内置 App public lazy view；Operations 私有 `AgentAppSurface.vue` 不再被 Host 直接 import。安装式 App 统一进入 `PluginAppFrame`；静态 builtin seam 不做运行时组件名推断                               |
...
| runtime/run-facade.ts                                                                                     | 以明确 `appId` 绑定 Thread/Run/Approval/Subagent/Checkpoint API；内部持有单调 run-store 与 active-Run subscription lifecycle（`start/selectRun/dispose`），供 Operations 调用，不暴露可变全局 store                                   |
...
| apps/operations/AgentAppSurface.vue                                                                       | 内置 `nexus.operations` conversation/Run surface；使用 `createAgentRunFacade(appId)`，组合 AgentConversation、TaskRail、Subagent/Approval 详情                                                                                        |
...
Frontend Agent 不维护第二套认证或跨 feature 私有 store：`AgentSurfaceHost` 只从 auth 的 public facade 读取当前会话，并通过 Agent HTTP API + `/ws/agent` 与 Backend 通信。该 Host 在 `App.vue` 中与 RouterView 同级，并把 Launcher/Hub `Teleport` 到 `body`，所以 RouterView 导航不能创建第二个 Agent 实例或把当前浮窗当页面卸载；Agent 不注册 `/agent` route。首次认证装载只显示 Launcher，Host summary 的 feature `false→true` transition 才自动打开 Hub。窗口/每 App 轻量 presentation state 分别集中在 `window-manager.ts` 与 `surface-session.ts`；read-only diagnostics 仍通过 Backend capability，不让前端 Agent 绕过 Host 直接驱动 Workspace 内部对象。

### 8.2 窗口与会话规则

Hub window state 由 `window-manager.ts` 集中维护：`status/bounds/maximized/activeAppId/recentAppIds/hubView/launcherPosition`；每 App 的 `threadId/draft/scrollAnchor/selectedTaskId/hubView` 轻量会话态由 `surface-session.ts` 单独维护并带 navigation generation。Agent 没有独立 page route，也不在 Hub 内复制 Settings/Workspace Runtime Manager。Launcher `openHub` 恢复最近 App；`AgentSurfaceHost` 在 Host summary 更新时优先保持当前 enabled App，否则选 Operations/首个 enabled App。AppSwitcher 单选切换时只 pause 旧 detail generation，不调用 cancel/quiesce，不改变 Run/Workspace/Subagent。Hub 对 `nexus.operations` 明确挂 `AgentAppSurface`，其他已授权安装式 App 明确挂 `PluginAppFrame`；不通过任意组件名/资源 key 动态映射。

Launcher不使用延迟点击计时器：primary pointerup在未超过6px拖动阈值时立即openHub；拖动结束立即持久化 launcherPosition；Enter/Space同样立即恢复最后App并ignore repeat。Hub 用 `role=dialog, aria-modal=false` 表达非模态应用窗口；header drag、右下 resize、minimize/maximize/restore/close 都只改 presentation state，move/resize 在 pointerup 后一次性持久化与记录日志，pointermove 不逐帧打日志。App选择只在Hub内`AgentAppSwitcher`完成，移动端也使用同一页内选择器。关闭/minimize/switchApp均不得调用cancelRun、环境stop/delete或Subagent cancel；后台App继续由scheduler执行，Host summary保持订阅。取消Run按钮是唯一显式取消入口之一，并独立确认显示将影响的子任务和未知副作用警告。
...
Conversation 与执行状态分层：当前 host-owned `AgentAppSurface` 组合 `AgentConversation` 与右侧 `TaskRail`，TaskRail 显示当前 Run 的预算/typed PlanItem/审批，并列出当前 App 其他活跃 Run 的紧凑摘要；PlanItem/Run/verification 状态与依赖/证据标签统一走 Agent i18n。其他 App 的 Run 不混入当前 TaskRail，但 Backend 仍继续执行并由 Host summary 更新 Launcher。`TaskDetailDrawer` 按需加载 snapshot/checkpoint/Subagent/message 详情，不替换 Thread，并挂载 `WorkspaceRuntimePanel` 作为 root Run 的受控产品入口：列表使用 `?runtime=root` 由 Backend 绑定 root runtime；创建请求只发送 Recipe、显式 `runnerPluginIds` 与 retention，浏览器不提供 runtime id。同一 runtime 最多一个状态非 `deleted/failed` 的活跃 Workspace，repository 在幂等重放之后执行唯一性检查。Workspace create/start/restart 与 Workspace ACL 写入重新检查 `workspace.runtime.manage`，stop/delete 保留 teardown 路径；Workspace↔Artifact bridge 继续分别检查 `workspace.runtime.execute + artifacts.write/read`。Frontend 不持有 Runner 内部对象，Workspace 详情直接展示冻结的 plugin/version/sdkVersion/protocolVersion/packageHash/entry。Run 处于 running/awaiting_approval/awaiting_budget 时输入仍可追加；执行事实始终以服务端 Run/Ledger/Event 为权威，Frontend 不自行推进 Run/Approval 状态。

诊断日志复用现有 Frontend/Backend pino 风格；Runner 由于保持最小依赖，输出同字段风格的 JSON line（`level/time/service/msg + safe context`）。Frontend 只记录 Host/window/WebSocket 生命周期与 bounds/state/generation，Backend 只记录 Run/Workspace command/reconcile/cleanup 元数据，Runner 只记录 command/job/workspace ID、generation、状态和计数；任何 message delta/final 文本、用户输入、Tool arguments、Runner payload/argv、credential/token/cookie 都不得进入这些日志。错误只记录序列化 Error 或 bounded error code，不 dump request body。
...
当前已有 panel-local `run-store.ts`，只按 `version + eventCursor + inputRevision` 接受单调 Run projection；`run-facade.ts` 是唯一 Run HTTP 读入口并拥有 active-Run `/ws/agent` subscription 的 `start/selectRun/dispose` 生命周期。`AgentAppSurface` 只解释事件并维护 view state，不再持有 WebSocket AsyncIterable 或 AbortController；切换 Thread、Run 终态与组件卸载都会由 facade abort 旧 subscription，事件 handler 在异步 refresh 前后继续检查 facade 提供的 AbortSignal。Host summary subscription 仍由跨 App 的 `AgentSurfaceHost` 管理，避免每个 App facade 重复订阅 Host。`message.delta` 只进入临时 `streamingText`，`message.final` 后清空并刷新 durable ledger，不能把半段 delta 当 final。审批卡片提交只调用 Backend resolve，Frontend 不自行把 requested 状态改为 approved；冲突/重连重新以服务端 snapshot 为准。
...
P2 增加可选的 `nexus-agent-runner` 主服务，但 canonical Linux 部署改为专用 **host service**：**不挂 `/var/run/docker.sock`，不运行 dockerd，不使用 dockerode，不为 Workspace generation、临时 job 或 Plugin 创建子容器**。Runner 内部由 Controller + Sandbox Manager + Pack installer + Toolchain Store + job protocol + cleanup/reconcile + quota + space reporter 完成 Workspace Runtime 管理。

统一 Runtime 只统一本地 Workspace execution contract，不统一会话 owner：`workspace_execute_argv` 经 `WorkspaceRuntimeGatewayPort` 提交 bounded one-shot job；Runner Plugin 经 generation-scoped sandbox + Workspace Broker；local Workspace Terminal 经独立 `WorkspaceRuntimeInteractiveSessionPort`。Terminal 的 live transport 是 Browser `/ws/agent-terminal` → Backend session registry → authenticated Runner WebSocket → sandbox 内 ephemeral Dropbear/SSH PTY；不把 one-shot job 拉成长任务。现有 `modules/workspace/WorkspaceTerminalService` 继续只管理用户远程 SSH session，SSH/SFTP/RDP/远程 Docker 不进入 Runner；local Terminal 仅复用底层 `SshShellSessionAdapter` 的 channel byte/backpressure primitive，不共享远程 connection identity。
...
└─ Workspace N
   ...
   ├─ Workspace generation / pinned tool profile
   ├─ core/task sandbox
   ├─ explicit Runner Plugin sandbox(es)
   └─ per-plugin logical workspaces
```

Frontend 的 `WorkspaceRuntimeSettings.vue` 是长期 Workspace Runtime Manager。页面状态为 `unavailable | uninitialized | ready | degraded`；初始化、Pack 管理、资源/网络策略、storage、运行 Workspace、cleanup/reset 都在同一入口。Runner 未启用或 sandbox primitive 不可用时 Core/SSH/RDP/Backend Docker capability 继续可用，Workspace Runtime availability 明确原因。
...
`packages/agent-runner/` 的 Controller 关键文件固定为：

| 文件                                                  | 职责                                                                                       |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `controller/sandbox-engine.ts`                        | Workspace generation/job 生命周期 facade，不提供 Docker 方法                               |
| `controller/sandbox-manager.ts`                       | Linux sandbox filesystem/process/network namespace 组装；稳定 Workspace 与 generation 分离 |
| `controller/plugin-runner-runtime.ts`                 | Workspace generation 内显式 Runner Plugin target 生命周期                                  |
...
| `controller/workspace-runtime-catalog.ts`             | Workspace profile、Tool Pack、`runtimeDigest` 读取/校验                                    |
| `controller/pack-installer.ts` / `toolchain-store.ts` | Tool Pack 下载、校验、原子只读安装与同 family 多版本 inventory                             |
| `controller/journal.ts` / `reconciler.ts`             | command/job/workspace durable 对账与重启恢复                                               |
| `controller/cleanup-planner.ts` / `space-reporter.ts` | runtime/cache cleanup 与分类计量                                                           |
| `controller/quota-manager.ts`                         | Workspace admission/resource policy                                                        |
| `worker/plugin-runner-sandbox.worker.ts`              | Runner Plugin target 进程内入口，只暴露 RunnerPluginSdk                                    |

`packages/agent-runner` 构建 host Runner Controller；Linux host 必须提供受支持的 sandbox primitive；当前发行由 host prepare、Docker build 与 `check:sandbox-prerequisites` 统一固定并校验上游最新稳定 `bubblewrap 0.12.0` + release SHA-256，Runtime 源码本身不重复保存版本门槛。`scripts/docker/agent-runner/Dockerfile` 不再是 canonical deployment path，不得借它恢复高权限 Runner-in-Docker 方案。Catalog 源暂位于 `scripts/docker/agent-runner/catalog/`；`runtimeDigest` 表示 Runner sandbox ABI/runtime 的版本事实。`base-tools` 使用仓库发行 archive；Node/Python/Go 的 x64 发行 Pack 使用 Catalog 固定的 exact version + `mise 2026.9.5` materialize；mise 本身运行在仅暴露 cache/staging 的 bubblewrap materializer sandbox。该 materializer 不绑定宿主整个 `/etc`：Runner 启动安装前复制 `/etc/resolv.conf` 为受控 snapshot，在 sandbox 内创建新的 `/etc`，只读挂载 TLS/NSS/hosts 必需输入并把 snapshot 绑定为普通 `/etc/resolv.conf`，因此宿主 resolver 即使是 `/run/...` symlink 也不需要暴露宿主 `/run`。最终 tool version 再由无网络 verifier sandbox 检查，并以 Nexus canonical tree digest 作为最终 content identity，不使用 mutable latest。host prerequisite 只安装经过 release SHA-256 校验的 pinned mise；普通 API 不能覆盖 installer binary 或 source。
...
/var/lib/nexus-agent-runner/runtime/     stable Workspace + generation/job lifecycle data
...
Workspace 数据与 generation 运行树分离：
...
`.control` 不进入任何 sandbox。当前发行 Catalog 的 x64 实包包括 Node 24.21.0/22.23.2、Python 3.14.7/3.13.15、Go 1.27.1/1.26.8；未在当前 arch 声明 digest 的版本直接显示 unavailable。Tool Pack 只读；源码/普通项目文件保持在稳定 Workspace，平台管理的依赖与构建状态按当前冻结 `runtimeDigest + packRefs` 的 SHA-256 `toolchainFingerprint` 分区到 `core/toolchains/<fingerprint>/deps|build`。Sandbox 把当前 profile 映射回 `/workspace/deps` 与 `/workspace/build`，并把 npm/pip/Go cache 指向该 profile；不同版本组合不会共享这些 ABI/cache，切回相同组合则复用原 profile。Runner 同时注入 `NEXUS_TOOLCHAIN_FINGERPRINT`，供上层依赖管理器给项目内缓存做相同分区；用户主动在源码树创建的 `node_modules`、`.venv` 等目录仍属于普通项目数据，不宣称由 Runner 自动隔离。Tool Store key 为 `familyId/versionId/contentDigest`（digest 按 arch 解析）；同 family 多版本可同时 installed/enabled/inUse。不同 Workspace 冻结不同版本组合，切版本只替换目标 Workspace 的 generation，不修改 `/usr/bin` 或其他 Workspace。

Workspace Profile/Generation 在 Backend DB 冻结 `runtime_digest + recipe_id + recipe_revision + catalog_revision + pack_refs_json + runner_plugins_json`。当前 `recipe_id=workspace-dev`；`kind=code` 暂作为兼容存储字段。`runner_plugins_json` 保存精确 `{pluginId,version,sdkVersion,protocolVersion,packageHash,entry}`，新建 Runner target 使用 `protocolVersion=2`；start/restart 使用该 generation 创建时事实，不根据当前安装状态重新猜测。工具版本或 Runner Plugin 版本变化都必须创建新 generation；旧 generation 不自动升级。

### 9.2 API、显式 Runner target 与 Workspace Grant

Frontend/Backend Workspace Runtime API（前缀 `/api/v1`）包括：

| 路由                                                                                          | 作用                                                                           |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `GET /agent/workspace-runtime/availability`                                                   | Runner + sandbox capability / unavailable reason                               |
| `GET /agent/workspace-runtime/catalog`                                                        | Recipe、Tool Pack、`runtimeDigest`、installed/enabled/inUse                    |
| `GET /agent/workspace-runtime/storage`                                                        | state/packs/cache/runtime/quarantine/sandbox-overhead 分类用量                 |
| `POST /agent/workspace-runtime/setup/preview` / `confirm`                                     | 初始化 Recipe/Tool versions 与 Settings CAS                                    |
| `POST /agent/workspace-runtime/tool-packs/:familyId/:versionId/install`                       | journal-backed 精确 Tool Pack materialize/install                              |
| `POST /agent/workspace-runtime/tool-packs/:familyId/:versionId/uninstall/preview` / `confirm` | 精确版本卸载及引用影响；active 引用返回 `WORKSPACE_TOOLCHAIN_IN_USE`           |
| `POST /agent/workspace-runtime/runtime-cleanup/preview` / `confirm`                           | 清确认归属的 generation/runtime 残余，不删 Tool Pack/Artifact                  |
| `POST /agent/workspace-runtime/cache-cleanup`                                                 | 清可重建 cache                                                                 |
| `POST /agent/workspace-runtime/settings/reset/preview` / `confirm`                            | 恢复 Workspace Runtime 发行默认设置                                            |
| `GET /agent/workspace-runtime/commands/:commandId`                                            | 查询持久 command 的 pending/running/succeeded/failed/unknown 状态              |
| `GET /apps/:appId/runs/:runId/workspaces?runtime=root`                                        | 列出 Run Workspace；可只取 root AgentRuntime                                   |
| `POST /apps/:appId/runs/:runId/workspaces`                                                    | 创建 root Workspace；Backend 解析 `agentRuntimeId`，浏览器不能伪造             |
| `GET /apps/:appId/workspaces/:workspaceId`                                                    | 读取稳定 Workspace/Profile/Generation snapshot                                 |
| `POST /apps/:appId/workspaces/:workspaceId/actions`                                           | start/stop/restart/delete/setNetwork/resize；返回 journal command              |
| `POST /apps/:appId/workspaces/:workspaceId/tool-versions`                                     | CAS 切换 Tool versions，保留稳定 Workspace、递增 generation                    |
| `GET /agent/apps/:appId/workspaces/:workspaceId/plugins/:targetPluginId/grants`               | 读取目标 Plugin logical workspace ACL                                          |
| `PUT /agent/apps/:appId/workspaces/:workspaceId/plugins/:targetPluginId/grants`               | 替换 ACL；需 `workspace.runtime.manage`                                        |
| `POST /agent/apps/:appId/workspaces/:workspaceId/plugins/:targetPluginId/artifacts/export`    | workspace→Artifact streaming；需 `workspace.runtime.execute + artifacts.write` |
| `POST /agent/apps/:appId/workspaces/:workspaceId/plugins/:targetPluginId/artifacts/import`    | Artifact→workspace streaming；需 `workspace.runtime.execute + artifacts.read`  |

Workspace 创建不自动注入所有 enabled Runner Plugin，而使用明确接口：

```ts
type AgentWorkspaceCreateSpec = {
  recipeId: string;
...
Backend 对 `runnerPluginIds` 逐个解析：必须是当前用户已安装、activeVersion 匹配、enabled 且 manifest 明确声明 `targets.runner` 的 App；任一目标不可用即返回 `PLUGIN_RUNNER_TARGET_UNAVAILABLE`，不静默忽略。解析结果冻结并持久化到对应 Workspace Profile。产品创建路由不接受 `agentRuntimeId`，而由 `runs.rootRuntimeId(scope,runId)` 解析 root runtime；GET 列表可用 `?runtime=root` 取得同一 root scope。Repository 先处理 idempotency replay，再拒绝同 runtime 已存在状态非 `deleted/failed` 的 Workspace，避免重试破坏幂等语义。Workspace create/start/restart/setNetwork/resize 与 Workspace ACL 写入都会重新检查 `workspace.runtime.manage`；stop/delete 作为安全收敛动作不依赖新增执行授权。
...
Backend/Runner 两端都验证 generation、Tool Pack、runtimeDigest 和 `runnerPlugins` identity。普通 HTTP/Frontend/模型不能直接提供 package source path、sandbox binary 参数或真实 filesystem path。
...
Backend 修改 ACL 前先确认 `userId + appId + workspaceId + generation` ownership，且 target/principal 都属于该 Workspace Profile 冻结 `runnerPlugins`；Runner 再次校验。caller 身份不从插件 payload 读取，由 Runner Plugin process instance 绑定。

### 9.3 Sandbox、Plugin Runtime 与资源边界

Workspace generation core job 当前通过 `SandboxManager` 生成 bubblewrap 命令：新 PID/IPC/UTS/network namespace、最小只读 system runtime、独立 `/tmp`、只读精确 Tool Pack，并把当前 Workspace 的稳定 `core/workspace` bind 为 `/workspace`。Runner token/env 不进入 child。cwd 必须解析在 `/workspace` 内，禁止 `..`/symlink/跨 Workspace 逃逸。
...
`WorkspaceBroker` 从 process instance 绑定 `callerPluginId`。caller=target 时默认访问自己的 workspace；caller!=target 时只接受目标 workspace 的显式 grant。`mkdir` 使用目标 `write` 权限；`rename` 同时要求源路径 `delete` 与目标路径 `write`，避免用一个泛化“manage”权限绕过 ACL。读取/写入/rename 直接作用于同一底层文件，不做 copy/paste。逻辑 `/` 是目标 workspace root，不是 Runner/Linux 根；`..`、symlink 越界、跨 Workspace target 均拒绝。

Backend 与 Runner 另有仅供 Host 使用的目标 workspace 二进制 streaming 接口，先由 Backend `WorkspaceRuntimeService` 验证 `scope + workspaceId + generation + targetPluginId`，Runner 再验证 generation/target；它不暴露给 Runner Plugin。GET 在 Runner 内先打开并 `fstat` 目标文件，以 read handle 固定本次读取对象并流式返回 `application/octet-stream + Content-Length`；Backend handle 消费完成/失败/timeout 都 cancel/close。PUT 必须声明 `Content-Length`，Runner 流式写同目录 `wx` 临时文件，精确校验字节数、`fsync` 后原子 rename，失败删除临时文件。Runner transport hard cap 为 256 MiB，不能通过 chunked/无长度请求绕过。`agent/exchange/workspace-artifact.service.ts` 直接把 workspace stream 接入 Artifact `write()`；反向导入按 Artifact 已有 8 MiB read-range 上限逐段读取并连续推给 Runner，不再 `arrayBuffer()/Buffer.concat()`。导出仍检查 `workspace.runtime.execute + artifacts.write`，导入检查 `workspace.runtime.execute + artifacts.read`，且 Artifact 自身单文件/总量/磁盘 quota 继续独立生效。这一步产生/写入 durable Artifact 或 workspace 文件，是显式生命周期转换；跨 Plugin workspace 授权仍使用上述 ACL，同一底层文件不复制。

Backend Plugin 与 Runner Plugin 是不同 runtime：Backend target 由 `LocalPluginBackendRuntimeAdapter` 在 Backend 容器内启动 bwrap process，Runner target 由 `PluginRunnerRuntime` 在具体 Workspace generation 内启动；Backend target 不能被路由到 Runner 代跑。当前 BackendPluginSdk 只明确开放 AppStorage；新增能力必须新增 typed SDK/Host IPC，不提供通用 method mapper。

网络默认 `none`。只有 Runner 真正实现可验证 egress broker 后才允许 `capabilities.egressAllowlist=true`；当前为 false，任何 allowlist Workspace 请求必须 `WORKSPACE_NETWORK_ENFORCEMENT_UNAVAILABLE`，不能降级成 unrestricted 网络。
...
type RunnerStorageView = {
  ...
  byWorkspace: { workspaceId: string; runtimeBytes: number; status: string }[];
...
`provision`：验证 command 与 Catalog 中的精确 Tool Pack `{familyId,versionId,contentDigest}` → ensure Tool Pack → 创建/复用稳定 Workspace tree → 创建新的 generation `.control`/sandbox metadata → 冻结该 generation 的 `packRefs + runnerPlugins` → journal ready。此流程不创建 Docker resource，也不复制 Workspace 项目文件。
...
Nexus 备份默认不复制 Runner Packs/runtime/cache；保存 Settings、精确 PackRef、Workspace Profile 的 `runtimeDigest/runnerPlugins` snapshot 与 Run/Checkpoint 事实。恢复后 Runner 重新核对 Pack/target source，可重建才继续；缺失明确 unavailable，不伪装旧 sandbox/process 仍存在。
...
`McpAdapter.connect/listTools/invoke/close` 只管理用户登记 server；未知 annotation 按 mutation，远端声明不能替代 Nexus 本地授权。ACP live path 为 `acp_execute -> AcpAdapter -> AcpTransportPort -> RunnerHttpAdapter ACP WS -> AcpProcessRuntime -> SandboxEngine`：外层 tool 是 mutation 并走本地 inspection/Policy/Approval/Lease/StateCommit；Runner 再校验 generation/profile；ACP direct fs/terminal RPC deny，nested permission 当前固定 `reject_once`。Browser live path 为 `browser_* tools -> BrowserRuntimeAdapter`；Backend 持 Puppeteer/semantic session，`via=runner` 只把 CDP text frames 经 `BrowserTunnelRuntime` 转发到配置 endpoint。Page request interception、download deny、popup close 与 snapshot invalidation 均在 Backend runtime 执行；不存在 selector/evaluate/raw-CDP tool surface。

安装式 App 由 `host/plugin-install.service.ts` 统一提供 artifact stage、remote catalog/stage、verify/install/upgrade/uninstall/deleteData。`package-verifier.port.ts` / `tar-package-verifier.adapter.ts` 校验签名、文件 hash、路径和限额，并从 manifest 的显式 `targets.frontend/backend/runner` 与可选 `agents[]` 解析入口/AgentDefinition；禁止通用 `resources` map 推断模块。AgentDefinition 由 install hook 按 `appId + appVersion` 注册，upgrade/uninstall 同步切换/移除；没有 Frontend target 但声明 `agents[]` 的 App 使用 host-owned generic Agent UI。

Remote repository 配置位于 Agent Settings `plugins.repositories[]`。catalog 获取经过 OutboundPolicy、DNS/IP pinning、redirect deny 与 bounded response；`remote/stage` 只接受已配置 repository 的 `repositoryUrl + appId + version`，服务端重新读取 catalog 决定 package URL，并先核对 catalog size/SHA-256，再进入同一 trusted publisher + Ed25519 signature + `files.json` hash verifier。catalog 中的 public key 只是发现信息，只有用户显式 trust 后才成为安装信任。`agent_plugin_stages` migration 53 将 stage source 泛化为 artifact/remote。首个官方 preset 位于独立仓库 `0honus0/nexus-agent-plugins` 的 `plugins/nexus.developer/`，manifest 声明 `developer.default`；插件仓 `scripts/build-package.mjs` 使用仓库外 Ed25519 私钥生成可复现包，`scripts/build-release.mjs` 同时生成 release `catalog.json`。主仓仅在 `packages/e2e/fixtures/agent/` 保留自包含协议 fixture，普通 Nexus E2E 不依赖外部插件仓库。
...
Runner target：不会作为 Backend target 的 worker，也不创建 Plugin Docker。Workspace 创建请求（包括 Agent 的 `workspace_create` tool）通过 `runnerPluginIds` 明确选择；Backend 将每个新目标解析并冻结为 `{pluginId,version,sdkVersion,protocolVersion=2,packageHash,entry}`，持久化 `runner_plugins_json`。`PluginRunnerRuntime` 仅在该 Workspace start/reconcile 时启动这些 sandbox，并把冻结的 version/sdkVersion/protocolVersion 注入 activation context；worker 的 `runtime.ready` 再回报版本，Controller 不匹配即拒绝。protocol v2 的 Host↔sandbox stdio 使用 framed JSON control + raw binary payload，不允许 workspace bytes 走 Base64。当前 `RunnerPluginSdkV1` 只有 `workspace.read/write/list/stat/mkdir/rename/remove`，其中单次 read/write binary payload 仍上限 16 MiB；插件真实 workspace 不 bind 进 process，callerPluginId 由 Host 绑定。

Workspace 跨插件授权由目标 ACL 决定：`targetPluginId + principalPluginId + path + permissions`。默认只能访问自己；跨 Plugin 必须显式 grant，授权后直接访问同一底层文件，不复制。Backend 与 Runner 双重检查 target/principal 都属于同一 Workspace generation 的冻结 targets；撤销 grant 立即失效。

AppIntent 继续负责**跨 App**的小 JSON/ArtifactRef 交接；它与同一 Workspace generation 内 workspace grant 分离。AppIntent Artifact read 使用 dedicated receiver path，每次重验 receiver 状态、当前 manifest/grant、receipt TTL/revoke/schema 和 artifact grant，generic Artifact owner route 不获得跨 App 读取能力。
...
新增或修改正式能力时必须同时回答：需求是否允许；长期 owner/安全边界是否改变；当前源码是否已接线；哪条 E2E/静态验证形成证据。只新增设计文字不能把需求状态改成已实现；只出现一个 adapter/class 也不能宣称能力已进入 live composition。当前 Phase 3 的 MCP、ACP 与 Browser 都已经由 composition root 注入 Tool Catalog；后续审查应继续验证 capability manifest、runtime owner、transport authority、failure cleanup 与 E2E 是否同时存在，不能只凭类型或 Adapter 名称判断状态。
...
| DECISION-03 | AgentRuntime 是参与者；Workspace 是稳定项目边界；Profile 冻结工具/Plugin/资源/网络；Generation 是该 Workspace 的运行实例与会话生命周期边界                                                                                                                                                                                                                                                                                                                                 | 架构§3                                    |
...
| DECISION-32 | 三 Plugin target 冻结/暴露明确 sdkVersion + protocolVersion；Frontend/Backend protocol v1 与 Runner protocol v2 独立演进，Backend/Runner sandbox 以 runtime.ready 回报并由 Host 校验；Runner v2 workspace IPC 使用 framed raw binary、不使用 Base64；Workspace↔Artifact 由 agent/exchange 以 Host-only streaming bridge 显式转换，Runner transport hard cap 256 MiB 并重新检查 Workspace Runtime / Artifact capabilities，不把 Artifact/transfer handle 直通 Runner Plugin | 架构§4/9；实施§9/10.7                     |
...
| `doc/architecture/agent/ARCHITECTURE.md`                                                                                    | Thread/Run/AgentRuntime/Workspace/Profile/Generation/Plugin/Capability/Artifact 等稳定概念和边界 |
...
| `packages/backend/src/modules/agent/exchange/*`                                                                             | Workspace↔Artifact 等跨资源协调，不污染 Workspace Runtime / Artifact 自身职责                    |
