...
7. 本地/远程验证状态：
   - `packages/agent-runner` build：**通过**；Backend build：**通过**；Frontend architecture/i18n/`vue-tsc`/Vite/bundle budget：**通过（318 source files；当前 2277 i18n keys / 3 locales / 84 fragments；bundle 以最新 CI 为准）**；
   ...
   - 当前宿主 Chromium 仍缺 `libglib-2.0.so.0`，且本机没有可作为 production evidence 的 Docker/bubblewrap 环境，因此 Browser UI 全量与 production Docker smoke 不在本机伪造通过；历史 UI 产品验收证据包括产品代码 SHA `3fdc273621c9` / GitHub Actions run `34678153826` 及后续截图链。当前 `dev` 又在该基线上加入 Agent Window resize/container-responsive Run workspace、Run 配置条与本轮 destructive lifecycle hardening；权威结论以最新 pushed SHA 的 CI/Docker smoke 为准，而不是继续把旧 SHA 描述成“当前”产品事实。
...
9. Phase 3 / preset 接线状态：**MCP / ACP / Browser-CDP / Subagent / Memory / Plugin / Workspace local Terminal 均已进入当前 composition**。
   ...
   - Destructive lifecycle hardening：Run delete 的 StateCommit 在 FK cascade 前拒绝任何 non-deleted/retained Workspace；runtime cleanup 的 preview/confirm 固化 Workspace ids，Runner 只对 confirmed ids 做执行时安全重检并返回 `deleted/skipped/quarantined`，Backend 只依据 `deleted[]` 同步投影；Runner journal 使用 fsync+rename，损坏保留 `journal.json.corrupt.*` 并 fail closed，startup/cleanup 显式 compact terminal history。
