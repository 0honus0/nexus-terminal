...
> **当前 Phase 3 接线状态（dev）**：MCP、ACP、Browser/CDP、Subagent、Memory 与安装式 Plugin 均已进入 production composition。`nexus.operations` manifest 已声明 `integration.acp.execute` 与 `browser.operate`，对应 Tool contribution 已进入统一 Tool Catalog。ACP 的外层 `acp_execute` 作为 mutation 经过 Nexus 本地 Capability / Policy / Approval / Lease / StateCommit 执行链；ACP backend 自己在协议内提出的二次 permission request 当前不继承外层批准，而是固定 `reject_once` fail closed，直到独立 nested-approval broker 落地。Browser 的语义执行权与 Puppeteer session 由 Backend `BrowserRuntimeAdapter` 持有；Runner 可选，仅对 `via=runner` endpoint 提供 authenticated CDP message tunnel，不拥有 Browser/Puppeteer 语义。

### 1.1 当前 live execution 契约

- **ACP**：`IntegrationKind='acp'` 使用 `transport='workspace-profile'`。Backend 持有 Integration/Run/ToolInspection/Approval/StateCommit authority；Runner `AcpProcessRuntime` 在冻结的 `workspaceId + generation + profileRevision` sandbox 中创建 ACP child process，并通过 authenticated binary WebSocket 提供 bounded stdio stream。ACP process 默认无网络，直接 filesystem/terminal ACP request 由 Backend Adapter 拒绝；协议内敏感 permission 当前一律 `reject_once`，因此外部 Agent 不能借一次 `acp_execute` 审批升级出新的机器副作用。
- **Browser/CDP/Puppeteer**：Backend `BrowserRuntimeAdapter` 是唯一 Browser semantic owner，持有 Puppeteer、BrowserContext、Page、CDPSession、snapshot/nodeRef 与 restricted operation lifecycle。endpoint 可声明 `via=backend | runner`：`backend` 由 Backend 直接连接；`runner` 只借 Runner `BrowserTunnelRuntime` 转发 bounded text CDP message。Runner 不实现 snapshot/click/type/evaluate，也不是 Browser session owner，因此 standalone direct Browser target 在 Runner 未配置时仍可使用。
- **Browser endpoint/reachability**：endpoint `scope` 表达可达域而非安全等级：`docker-network` 表示执行连接的一侧所在/已加入 Docker network 内的 service/container DNS/IP，`external-network` 表示 Docker network 之外但该侧可达的宿主、其它 Docker network、LAN/VPN/VPC 或公网地址。`http/https/ws/wss` 与 scope 独立；当前 transport policy 显式记录 `allowPlaintext` 与 `verifyTls`。Agent 只引用管理员配置的 `targetId` 或 Workspace 中冻结的 target snapshot，不能把裸 endpoint URL 填入 tool arguments。
- **Browser 页面安全边界**：CDP control-plane endpoint policy 与 Chrome 页面 data-plane policy 分离。页面开启 request interception，HTTP(S) navigation、redirect 与 subresource 均按 `allowedUrlPatterns` 检查；popup 关闭、download deny。Tool Catalog 只暴露 create/navigate/snapshot/click/type/close，禁止 selector fallback、`evaluate`、raw CDP command 和任意 JavaScript。navigate/click/type 后旧 `snapshotId + nodeRef` 立即失效；Workspace session 还绑定 `workspaceId + generation + targetRevision`，Standalone session 绑定 Agent Settings revision。
- **Workspace local Terminal**：独立 `WorkspaceRuntimeInteractiveSessionPort` 已 live。Frontend `/ws/agent-terminal` 连接 Backend `WorkspaceRuntimeTerminalService`；Backend 通过 `RunnerWorkspaceTerminalAdapter` 在 authenticated Runner WebSocket 上建立临时 SSH transport，Runner `WorkspaceTerminalRuntime` 在同一 generation bubblewrap sandbox 内启动 ephemeral Dropbear，由 Dropbear 持有 PTY/shell 语义。session 绑定 `workspaceId + generation`，支持 attach/input/output/resize/signal/close；Backend detach grace 为 30 秒、离线输出 replay 上限 1 MiB。它不复用 Agent 模型身份、one-shot `workspace_execute_argv` 或远程 SSH 的高层 `modules/workspace/WorkspaceTerminalService`。
- **Runner optional**：Runner 不再是 Agent Host 或 Browser 的启动前提。Provider/Run/MCP/Plugin Host 和 `via=backend` Browser target 可在 Runner unavailable 时继续工作；Workspace one-shot job、ACP workspace-profile、Runner Plugin 与 local Workspace Terminal 才要求 Runner。
...
前端集中于 features/agent/{host,api,ai,files,runtime,settings,apps/operations}；公共会话/事件投影归 runtime，文件库归 files，Agent 宿主设置归 settings，Operations UI 通过 facade 使用。独立 host Runner 程序放 packages/agent-runner；Docker 构建/运维辅助文件仅放 scripts/docker/agent-runner。内置 App 不另建 npm workspace package。Frontend architecture guard 必须像 Backend 一样约束 Agent 子域依赖，而不能只把整个 `features/agent` 当成一个无内部边界的 feature。
...
Plugin manifest 还可声明 versioned `agents[]`。安装/升级/卸载由 Host hook 将这些 AgentDefinition 按 `appId + appVersion + agentDefinitionId` 注册/切换/移除；因此插件 Agent 不是 Backend 硬编码特例。声明 `agents[]` 且没有自定义 `targets.frontend` 的 App 使用 Nexus host-owned generic Conversation/TaskRail/Workspace surface；声明 Frontend target 的 App 继续走隔离 iframe。首个官方 preset 是独立仓库 `0honus0/nexus-agent-plugins` 中 `plugins/nexus.developer/` 的 `nexus.developer` / `developer.default`，只包含 manifest + Skill，使用通用 Agent surface；主仓仅保留 E2E 协议 fixture。

远程仓库是**发现源，不是信任源**。用户先在 Agent Settings 配置 repository catalog URL；Backend 通过 OutboundPolicy、DNS/IP pinning、redirect deny 和响应大小限制读取 catalog。`remote/stage` 只接受已配置的 `repositoryUrl + appId + version`，具体 package URL 必须重新从服务端 catalog 解析，随后核对 catalog 声明的 size/SHA-256，再进入与 Artifact package 完全相同的 Ed25519 publisher trust、signature、`files.json` hash 和 immutable install 链。stage 持久化 `source_kind + source_json`，migration 53 将旧 Artifact stage 无损迁为 `{kind:'artifact',appId,id}`；remote stage 记录 `{kind:'remote',repositoryUrl,appId,version}`。官方插件仓的 `scripts/build-package.mjs` 可复现生成清单、签名和 tar，`scripts/build-release.mjs` 生成 release catalog；私钥不进入仓库。
...
Runtime consumer 的统一范围固定为：Agent one-shot job、Runner Plugin、CI/task 与已实现的**本地 Workspace Terminal**共享 `workspaceId + generation`、Workspace/Profile、Tool Store 和 sandbox construction contract，但 owner/transport 各自独立。one-shot job 使用 bounded job RPC；Runner Plugin 使用 framed local IPC；local Terminal 使用 `WorkspaceRuntimeInteractiveSessionPort` + authenticated Runner WebSocket + sandbox 内 ephemeral Dropbear/SSH PTY。现有远程 SSH Terminal/SFTP/远程 Docker 仍属于 Nexus `modules/workspace` 与 remote-machine capability，它们的 live SSH session、cwd、SFTP socket 不进入 Runner；RDP/VNC 继续属于 Guacamole runtime。local Terminal 只复用低层 SSH channel byte/backpressure adapter，不复用远程 SSH 的高层 `WorkspaceTerminalService` 或连接身份。
...
当前状态：**MCP、ACP、Browser/CDP 均已接入 live composition**，但三者的 authority 不相同。MCP 是远端 tool integration；ACP 是 Workspace 内隔离 process protocol；Browser 是 Backend-owned semantic runtime，必要时借 Runner 做 endpoint tunnel。它们共同经过 Nexus capability/tool execution 边界，不能因为下游协议自己宣称 read-only 就绕过本地判断。
...
ACP `acp_execute` 只接受已登记 ACP integration 与当前 Run/runtime 拥有的 running Workspace，且 integration 的 `profileId` 必须存在于该 generation 冻结的 `acpProfiles`。Tool inspection 冻结 integration version、workspace generation、profile revision 并生成 operation hash；Runner 再次校验 workspace/generation/profile 后才在 sandbox 内 spawn。ACP direct fs/terminal RPC 全部 deny；nested `session/request_permission` 当前固定 `reject_once`，避免把外层 tool approval 当作外部 Agent 后续任意副作用的通行证。协议版本不支持直接失败，不静默 fallback。

Browser tool 可以选择 Agent Settings 中的 standalone target，或选择 running Workspace 中冻结的 Browser target。Backend `BrowserRuntimeAdapter` 持 Puppeteer/BrowserContext/Page/CDPSession；`via=runner` 仅改变 CDP control-plane transport，Runner `BrowserTunnelRuntime` 不获得 Browser semantic authority。Page request interception 对 HTTP(S) 页面请求应用 `allowedUrlPatterns`，popup 与 download fail closed。snapshot 返回短期 `snapshotId + nodeRef`，navigate/click/type 后旧 ref 失效；不提供 selector/evaluate/raw-CDP/任意 JS。页面文本仍是 untrusted input，不能升级为 system/user authority。
...
Hub内部左侧为可收起的会话/App导航，并提供用户级“文件”入口；中间始终是当前 Conversation + 固定 Composer，右侧为可折叠 TaskRail。文件库打开时复用 Hub 主内容区展示统一 Artifact 管理视图，关闭/返回后恢复原 Thread 的 scroll/draft，不创建第二套会话状态。TaskRail 展示当前会话由 Agent 生成的 typed PlanItem/进行中任务及同 App 的后台活跃 Run 摘要，不复制完整日志；第一层直接展示 Plan 完成百分比、当前 focus、blocked 数与 awaiting approval/awaiting budget/reconciliation 注意状态，后台 Run 优先使用 Host 已加载 Thread 的标题而不是裸 UUID。PlanItem 的 status/depends/evidence 与 Run/verification 状态统一走 Agent i18n。点击任务在右侧 TaskDetailDrawer 原位展开步骤、状态、资源、审批、Artifact、Subagent、Workspace Runtime 与日志，关闭详情恢复原 Conversation 滚动锚点和 draft，不切换到第二套 Composer。 新建会话标题直接提交现有 `createThread(title)` contract；前端只保留创建表单草稿，不维护第二份 Thread 标题事实。会话列表允许对已加载 Thread 做纯 view-level 搜索，并按 selected→active→updatedAt 稳定排序；活动状态来自现有 Run 投影，最近时间来自 Thread.updatedAt，不引入独立 Conversation manager store。模型选择器只决定**下一次** `createRun.model`，选项来自当前启用 Provider/Model；Run 创建后以 `Run.definition.model` 为冻结事实，非终态 Run 期间 selector 锁定，不能用前端状态制造“运行中已换模型”的错觉。当前 Thread 的 Run 历史直接来自 `listRuns(threadId)` 投影；选择历史 Run 只 lazy-load 同一个 TaskDetailDrawer，不替换当前 Conversation 或复制 Run store。终态 Run 的删除使用现有带 `expectedVersion + Idempotency-Key` 的 Run DELETE contract，并在成功后重新读取 Thread Run/ledger 投影。宽布局中的 Approval Timeline 与窄布局/TaskRail 隐藏时的 header pending-approval shortcut 最终都进入同一 Run Details `ToolTimeline`，审批 resolve 仍只走既有 versioned Approval API。Root Run 的 Workspace 产品入口读取 `/apps/:appId/runs/:runId/workspaces?runtime=root`，创建请求只提交 Workspace spec（Recipe、版本选择、显式 Runner Plugin ids、资源/网络）与 retention，由 Backend 解析 root `agentRuntimeId`，浏览器不能伪造 runtime identity。同一 runtime 最多一个状态非 `deleted/failed` 的活跃 Workspace，repository 在幂等重放后执行唯一性约束。Workspace create/start/restart 与目标 Workspace ACL 写入必须重新通过 `workspace.runtime.manage` Capability；stop/delete 作为安全收敛路径仍允许 teardown。Workspace↔Artifact 转换继续走 Backend exchange/capability 路径，并分别检查 `workspace.runtime.execute + artifacts.write/read`，不把 Artifact/Runner 内部对象直接交给前端或 Plugin。Operations 的 Timeline/Approval/Artifact/Token 都作为消息内卡片或 TaskDetail 的按需视图存在；消息 Composer 的“附件/文件”按钮打开轻量选择器，从文件库选现有 Artifact 或上传新文件，“引用知识”则是另一条知识检索入口，二者名称和职责不混用。App切换保留内存draft/scroll，暂停旧详细 Agent event subscription而不cancel；新App先授权registry→本地contribution map→lazy import→snapshot+catchup。navigationGeneration丢弃过期异步加载。授权失败绝不因本地chunk存在继续渲染。

Hub标题/工具区只提供**单选 App 切换**及当前会话/任务所需控制，不支持一个 Run 同时组合多个 App，也不提供 Settings 或 Runner 配置跳转。Agent 的宿主级配置统一并入现有 Nexus `/settings` 页面，新增 `Agent` tab；该 tab 采用 Overview / Models & Budget / Execution & Agents / Runtime Environments / Files & Storage / Apps & Extensions / Safety & System 的分组导航与只读概览卡，但底层仍由原 settings/App/provider/runtime API 与各 section component 持有状态，不创建第二套 settings document。模型区可通过现有 `settings.model.defaultProviderId/defaultModelId` 选择新 Run 默认模型，并展示每个模型的 context/output/tools 能力；App capability grant 默认折叠按需展开，避免授权矩阵吞没总览。其中继续管理 Agent 总开关、App 启停、模型与Provider、执行与性能、预算与上下文、Hard Limits、Subagent、Artifact与存储、Workspace Runtime/Runner、安全与网络、系统保护。 Agent总开关默认开启；关闭时服务端先禁止新 Run/step/Workspace Runtime claim，再按现有cancel/quiesce/reconcile收敛执行，未知远端副作用仍quarantine。关闭完成后隐藏Launcher；Settings及安全清理/审计入口仍可用，历史会话、文件、配置和审计数据都保留但不通过已关闭的Hub继续交互，重新开启后可重新浏览；重新开启不自动续跑旧Run。Workspace Runtime Manager 在该 tab 内长期存在：一期显示真实 unavailable，二期启用完整 Catalog/Pack/cleanup 管理，不另造第二套配置状态。系统保护只读展示event batch、commit queue、Agent WebSocket订阅/缓冲上限、transient delta策略和当前数据库压力/availability；普通用户不能关闭背压或改成逐chunk持久化。执行与性能页允许调低Runtime/模型并发等软上限，模型并发默认Auto跟随Runtime并发。
