...
- 当前新增执行包：`/root/luna_m00_overlay`（Luna max）审计 M00 dialog/footer/input、Escape/backdrop/focus restore；`/root/luna_m09_keyboard`（Luna max）审计 M09 command bar 多 session、Enter/Escape 与失败反馈。两者均产品优先、禁止新增测试/旧架构迁移，完成后由主代理独立验收。
