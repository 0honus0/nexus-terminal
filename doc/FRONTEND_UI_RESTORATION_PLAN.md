...
- 分支收口采用快进而非制造无意义 merge commit：本地 `test/agent-runtime-foundation` 的最新提交已包含本地 `test/ui-restoration-groups` 的全部历史，随后将计划提交推送到远程 `test/ui-restoration-groups`；远程最终只保留 `main` 与 `test/ui-restoration-groups`，不恢复已删除的独立 agent 分支。后续验收以远程 `test/ui-restoration-groups` 为唯一来源。
