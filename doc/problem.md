...
| P1                                    | 设置区控件风格分裂：**主操作按钮已收敛**（33 处 → Gen2 `UiButton appearance="solid" tone="primary"`，实测统一 `32px / fs12 / r8`，禁用态退为中性填充）；**仍开放**：次操作/危险/图标档的裸按钮、5 档圆角，以及添加/移除模型用原生 checkbox 与 11px 纯文字按钮                                                                                                                        | `features/agent/settings/**`（见 §6.2、§6.3）                                                                 |
...
| P1                                    | 设置区按钮规格失控（实测）：~~`添加 Provider` 144×38 fs16 / `关闭 Agent` 109×40 fs16~~ → **主操作档已收敛为 32×fs12×r8**；**仍开放**：`立即更新` 77×28 fs11 r6、`模型与测试(6)` 117×26、`更新模型` 91×26、`添加配置档` 86×34 r6、删除服务商 28×28 等次操作/危险档                                                                                                                    | 见 §7.11                                                                                                      |
