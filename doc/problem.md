...
- 🚧 **Agent / Workspace 仍是最大剩余面**：frontend `agent-api.ts` / `agent-events.ts`、backend Agent HTTP route request DTO、Workspace 其余 filesystem/transfer/upload/status/docker/terminal JSON operation/event，以及 Agent terminal / event WS contract 仍有手写重复。raw binary chunk 本身不作为 DTO，但其结构化 metadata / opcode / envelope 仍应收敛到 protocol。
