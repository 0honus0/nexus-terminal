...
- ✅ **Agent Threads / Ledger 已完成 canonical DTO 接线（2026-09-22）**：thread list/get/create/rename/delete/delete-all 与 ledger page 共 7 条 route 已迁入 `protocol/agent-threads`；list/ledger 分页 query 分别保留真实的 100/200 上限，create title 的 placeholder 语义与 rename 的 strict title/version 语义分离。frontend thread/ledger 手写 interface 已替换为 alias，ledger payload 由 `unknown` 收紧为共享 `AgentJsonValueDto`。
- 🚧 **Agent 仍是 transport DTO 的主要剩余面**：Settings / Grants / Runs / Approvals / Artifacts / Plugins / Collaboration / Workspace Runtime 等 HTTP surface，以及 Agent event WS 与 terminal WS contract 仍有手写重复；下一步继续按完整 domain surface 收敛，不以局部 import 替代端到端迁移。
