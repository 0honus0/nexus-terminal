...
- 🚧 **Agent 已成为 transport DTO 的主要剩余面**：frontend `agent-api.ts` / `agent-events.ts`、backend Agent HTTP route request/response mapping、Agent event WS 与 terminal WS contract 仍有手写重复；下一步继续按完整 operation/event surface 收敛，不以局部 import 替代端到端迁移。
