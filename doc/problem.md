...
| P1                                    | 设置区控件风格分裂：**按钮档位已收敛**（主/次/危险/图标四档全部走 Gen2 `UiButton`，实测统一 `32px / fs12 / r8`）；**仍开放**：分组导航胶囊与顶部 Tab 仍是 36px、`QuantityInput` 单位切换命中区 18×20，以及添加/移除模型用原生 checkbox 与 11px 纯文字按钮（§6.3/§6.7）                                                                                                               | `features/agent/settings/**`（见 §6.2、§6.3）                                                                 |
