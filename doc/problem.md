...
| **P2 · ✅ 2026-09-23 部分关闭**       | 审批卡：`risk` 枚举已本地化（新增 `agent.approvals.risk.*`）；原记录的「`审批状态：approved` 拼英文枚举」已由 §1.4/§7.15-a 关闭（`status` 映射）；仍开放：`剩余 300s` 单位硬编码、"批准"按钮用 `bg-warning`                                                                                                                                                                          | `runtime/ApprovalCard.vue:45/53/73/127`（见 §7.14-b）                                                         |
