...
- 🚧 **Agent 仍是 transport DTO 的主要剩余面**：Plugins / Workspace Runtime 等 HTTP surface，以及 Agent event WS 与 terminal WS contract 仍有手写重复；下一步继续按完整 domain surface 收敛，不以局部 import 替代端到端迁移。
