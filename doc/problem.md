...
| **P2 · ✅ 已修复 2026-09-24**             | **Runner catalog transport 已使用管理面专用 envelope**：`catalog()` 不再复用通用 1MB response cap，而是使用 8MB route-specific limit；其它 Runner HTTP route 默认仍保持 1MB。4,096-pack 合法 projection 回归约 2.30MB，真实 `decodeCatalog()` 完整通过，因此合法管理面 catalog 不会再在 decoder 前被 transport 意外拒绝。                                                                                                                                                                                                                                                                                        | Backend `runner-http.adapter.ts`、catalog response contract regression（见 §7.133）                                                                                                                                                                                                                                              |
| **P1 · ✅ 已修复 2026-09-24**             | **Backend Plugin line handler 已有显式 Promise owner**：stdout 每条 bounded frame 都以 `handleLine(...).catch(protocolFailure)` 消费；lifecycle/storage/intent decoder 或 Host response write 的异步异常都会统一 fail pending request 并 `SIGKILL` 当前 Plugin child，不再形成 Backend unhandled rejection。错误消息先截到 1KB，避免插件控制的异常文本无限扩散。                                                                                                                                                                                                                                                 | `local-plugin-backend-runtime.adapter.ts`、Plugin line error owner regression（见 §7.134）                                                                                                                                                                                                                                       |
| **P2 · ✅ 已修复 2026-09-24**             | **Backend Plugin Host→child 已统一走 bounded serialized writer**：lifecycle request、storage result、intent result 都经 `BoundedProtocolWriter`；aggregate queue 上限 32MB / 128 frames，`Writable.write()` backpressure 会等待 drain，close/error 会 reject 并由 Plugin process owner 收口。Plugin-originated storage/intent Host RPC 同时限制为最多 32 个 active operation，且先检查并发上限再启动业务 Promise。                                                                                                                                                                                               | `local-plugin-backend-runtime.adapter.ts`、Plugin writer backpressure regression（见 §7.135）                                                                                                                                                                                                                                    |
| **P1 · ✅ 已修复 2026-09-24**             | **默认模型 optimistic identity 已保持 Provider+Model 复合身份并可回滚**：当前实现使用 `optimisticDefaultModelKey = providerId\0modelId`，解析时按完整 option key 匹配，watcher 同时观察 `defaultProviderId + defaultModelId`；本轮补充独立 `settingsBusy` mutation 信号，settings write 从 busy→idle 时 optimistic key 强制收敛到最新 props，因此成功写入跟随新 Provider，失败写入回滚旧 Provider/model，fallback 过滤始终使用真实复合 key。                                                                                                                                                                     | `ModelProviderSettings.vue` / `AgentSettingsPanel.vue`、default-model identity regression（见 §7.136）                                                                                                                                                                                                                           |
| **P1 · ✅ 已修复 2026-09-24**             | **Launcher §7.38 的 deliberate-drag / reset 闭环已恢复**：当前组件重新使用 `HOLD_MS=320`，长按完成前超过 10px 移动只取消点击/hold、不改写位置；长按后才进入拖动。拖动完成后非默认位置显示 7 秒可见、可聚焦的“重置位置”按钮，右键仍为补充入口；`pointercancel` 会把临时位置恢复到手势起点且不发 layoutChange，避免后续持久化取消手势。                                                                                                                                                                                                                                                                            | `host/AgentLauncher.vue`、Agent i18n、launcher long-press regression（见 §7.137）                                                                                                                                                                                                                                                |
| **P2 · ✅ 已修复 2026-09-25**             | **Provider discovery/import 已与 100-model 持久化上限统一**：共享 `AGENT_PROVIDER_MODEL_LIMIT=100` 由 protocol 导出，Backend `validateProviderInput()` 与 Frontend 均引用同一值。新建 Provider 的“导入全部”最多取前 100 个并提示超出的数量；已有 Provider 的“添加全部/所选”继续按 `100-currentModels` 裁剪。101/1000 discovery、90+11、恰好100 的契约回归均通过。                                                                                                                                                                                                                                                | shared provider protocol + Backend `provider.service.ts` + `ModelProviderSettings.vue`、provider-model-limit regression（见 §7.138）                                                                                                                                                                                             |
| **P2 · ✅ 已修复 2026-09-25**             | **Runner coding projection 的相关性选择与截断语义已修正**：Repo Map 先对全部 scope 内已索引文件做轻量 path/import/source relevance，再排序选 top-K，之后才消费全局 symbol budget；不会再因前几个路径文件耗尽 symbol budget 而漏掉后排高相关文件。symbol collection 使用 `limit+1` 探测，Repo Map 的 file/symbol/output/scan 任一预算触发都会置 `truncated`。Code Intel 的 symbols/diagnostics/definition/references 也分别显式计算 count truncation，不再用裁剪后的长度误判完整。                                                                                                                                | Runner `workspace-code-intelligence.ts`、coding truncation regression（见 §7.139）                                                                                                                                                                                                                                               |
| **P2 · ✅ 已修复 2026-09-25**             | **MCP transport 已在 SDK decode 前实施 wire-byte 上限**：普通 JSON/错误 response 经 Nexus-owned bounded `ReadableStream`，总 wire cap 为 10MB output + 256KB JSON-RPC envelope，声明超限 Content-Length 也会在读取前拒绝；`text/event-stream` 不套总量 cap，而按 SSE 空行边界限制单 event 为 10MB + 64KB envelope，超限时 stream 直接 error，SDK 尚未完整组装 `event.data`。`jsonValue()` 继续保留为第二层业务对象限制。                                                                                                                                                                                         | `safe-mcp-fetch.ts`、MCP wire response limit regression（见 §7.140）                                                                                                                                                                                                                                                             |
| **P1 · ✅ 已修复 2026-09-25**             | **Subagent Profile 的 stale Provider-version model ref 现在可见且必须显式 rebind**：UI 会识别不再匹配当前 option 的旧 `configurationVersion` 并显示旧 ref；若同 provider/model 有当前版本，可一键原子替换 default/allowed 中该 family 的旧版本；若当前模型已不存在，可在先选择新默认后移除 stale ref。普通设默认/勾选当前模型也会按 `providerId+modelId` family 去重旧版本。Save 对任何残留 stale ref 本地 fail-closed，并给出 rebind 提示，Backend 的冻结版本校验保持不变。                                                                                                                                     | `settings/SubagentSettings.vue` + 三语 i18n、Subagent model rebind UI regression（见 §7.141）                                                                                                                                                                                                                                    |
| **P2 · ✅ 已修复 2026-09-25**             | **Settings `?tab=` 与 KeepAlive active section 已双向同步**：`SettingsPage` 用 `watch(route.query.tab, ..., { immediate:true })` 处理首次加载和同页 query 变化，并在 `onActivated` 再 reconcile KeepAlive 恢复实例；合法 tab 写入 active，缺失/非法 query 回到 workspace。页面内 `selectTab()` 同步 `router.replace({ query:{...route.query,tab} })`，保留其它 query 且避免同值循环。因此 deep-link、back/forward、重复导航和本地切页都不会再让 URL 与实际 section 长期分叉。                                                                                                                                    | `app/pages/settings/SettingsPage.vue`、Settings tab route-sync regression（见 §7.142）                                                                                                                                                                                                                                           |
| **P2 · ✅ 已修复 2026-09-25**             | **Agent Settings 四组恢复“首次访问才 mount、之后保活”语义**：Models/Tools/Runtime/Safety 全部重新使用 `v-if="visitedGroups.has(group)" + v-show="activeGroup===group"`；默认 visited 仅含 Models，`selectGroup()` 首次进入时加入 visited。未访问 Tools/Runtime/Safety 不再创建 Plugin/MCP/ACP/Workspace/Safety 等带 mount I/O 的子树，进入一次后切走仍保留组件状态且不会重复初始化。                                                                                                                                                                                                                             | `settings/AgentSettingsPanel.vue`、Agent settings lazy-mount regression（见 §7.143）                                                                                                                                                                                                                                             |
| **P2 · ✅ 已修复 2026-09-25**             | **Agent Settings tab 的 `aria-controls` 已恢复真实 target**：父级 `SettingsPage.vue` 现在显式给 `AgentSettingsPanel` 传入 `id="settings-panel-agent"`；组件单根节点继承该 attribute，因此 mobile/desktop 两套 `aria-controls="settings-panel-agent"` 都能解析到唯一 DOM target，不再暴露悬空 accessibility relationship。                                                                                                                                                                                                                                                                                        | `SettingsPage.vue`、Settings Agent aria-controls regression（见 §7.144）                                                                                                                                                                                                                                                         |
| **P2 · ✅ 已修复 2026-09-25**             | **Settings ARIA tabs 已补完整键盘/焦点模型**：mobile 横向 tablist 使用 Left/Right，desktop 纵向 tablist 声明 `aria-orientation="vertical"` 并使用 Up/Down；两者均支持 Home/End，selection follows focus。仅 active tab `tabindex=0`，其它为 -1；键盘切换复用 `selectTab()`，因此 active、URL 与 focus 同步。8 个 Settings panel 均保留真实 `aria-controls` target，并补 `role="tabpanel"` 与可访问名称。                                                                                                                                                                                                         | `app/pages/settings/SettingsPage.vue`、Settings tabs accessibility regression（见 §7.145）                                                                                                                                                                                                                                       |
| **P2 · ✅ 已修复 2026-09-25**             | **Agent i18n dead-key 门禁不再能被未消费 dummy literal 绕过**：删除 `_legacyPluginGroupKey` 与三语 `agent.settings.groups.plugins`，并把 reachability 限定到 Frontend/Backend/Runner 运行源码，tests/scripts 不再能给字典续命。扫描前会剔除只声明一次且从未消费的简单 const/let `agent.*` 绑定，同时保留运行时对象映射和动态 prefix 的合法间接引用；完整 gate 进一步发现并删除了唯一另一个无运行 caller 的 `agent.tasks.runId`。Agent Settings 的 navigation aria-label 也改为真实 `$t(...)`。                                                                                                                   | `AgentSettingsPanel.vue`、`scripts/check-agent-i18n.mjs` + reachability helper、三语 locale、reachability regression（见 §7.146）                                                                                                                                                                                                |
| **P2 · ✅ 已修复 2026-09-25**             | **MCP refresh 时间恢复 Unix-seconds 语义**：共享 `dateFromUnixSeconds()` 统一执行 seconds→milliseconds 转换并拒绝 null/undefined/NaN；`McpIntegrationSettings` 不再直接 `new Date(timestamp)`，而是经该 helper 后再做 locale time formatting。Integration protocol DTO 同时明确 refresh attempt/success/retry 字段使用 Unix epoch seconds，防止前后端单位再次漂移。固定 `1704067200` 的回归验证得到 `2024-01-01T00:00:00.000Z`。                                                                                                                                                                                 | `locale-format.ts`、`McpIntegrationSettings.vue`、integration protocol、Unix-seconds regression（见 §7.147）                                                                                                                                                                                                                     |
| **P2 · ✅ 已修复 2026-09-25**             | **Browser / ACP Profile / Model fallback 已改为真实 async commit boundary**：父级 `patchSection()` 返回明确 `Promise<boolean>`；Browser/ACP 通过 callback `await` Backend commit，成功后才落本地状态/关闭 modal，失败保持 authoritative props 与草稿，不再提前 success。Fallback 新增/删除/排序/自动清理也统一 await `saveFallbackModels()`；用户操作的 success 只由父级 commit 层提示一次，模型移除后的自动 fallback 清理使用 `success=null` 避免重复 toast。                                                                                                                                                   | `BrowserRuntimeSettings.vue`、`AcpRuntimeSettings.vue`、`ModelProviderSettings.vue`、`AgentSettingsPanel.vue` + async commit regression（见 §7.148）                                                                                                                                                                             |
| **P1 · ✅ 已修复 2026-09-25**             | **Browser / ACP Profile / MCP 已恢复非破坏性编辑路径**：Browser Target 可直接编辑 id + URL patterns，Endpoint 可编辑 url/scope/via/priority/plaintext/TLS；ACP Profile 复用现有 modal 编辑 id/argv/cwd，被 ACP Integration 引用时锁住 id 以避免悬空 profileId；MCP Integration 可直接编辑 displayName/endpoint/trust/enabled，复用 typed `updateIntegration()`，空 credential 保留旧凭据。三类 edit 都以当前 authoritative 对象初始化，失败保留 modal 草稿，成功后通过既有 reload/reconcile 更新。                                                                                                               | `BrowserRuntimeSettings.vue`、`AcpRuntimeSettings.vue`、`McpIntegrationSettings.vue` + edit-path regression（见 §7.149）                                                                                                                                                                                                         |
| **P2 · ✅ 已修复 2026-09-25**             | **Plugin catalog 分组已改为 source identity，而不是 path owner**：新增纯 helper 以 `official/remote + canonical repository URL` 作为 group key，owner 仅用于展示且包含 hostname；不同 host 的同名 owner、同 URL 的 official/remote 都不会再合组。group header 的 official 标记与 `sourceUrl` 来自该唯一 source identity，折叠状态也按 `group.key` 跟踪；package item 继续携带真实 catalog/official，因此视觉 provenance 与实际 install/trust 输入一致。                                                                                                                                                          | `PluginManagementSettings.vue`、`plugin-catalog-grouping.ts`、provenance grouping regression（见 §7.150）                                                                                                                                                                                                                        |
| **P2 · ✅ 已修复 2026-09-25**             | **ACP command-string argv 解析改为可验证的无损 lexer**：JSON string array 继续作为 canonical 快路径；普通输入仅做 shell-style argv quoting/escaping，不执行 shell/变量/glob 展开。状态机支持 escaped space、单双引号、quoted/unquoted 相邻片段合并、escaped quote 与空参数；malformed JSON、未闭合 quote、尾部 escape、NUL 都返回 invalid，不再静默持久化错误 argv。三语提示同步明确“仅 argv 解析，无 shell expansion”。                                                                                                                                                                                         | `AcpRuntimeSettings.vue`、`acp-command-argv.ts`、三语 i18n、argv parser regression（见 §7.151）                                                                                                                                                                                                                                  |
| **P2 · ✅ 已修复 2026-09-25**             | **移动端 Settings 总览恢复通用 overview 语义**：`settings-mobile-catalog` header 的副标题改用现成 `settings.mobile.settingsOverview`，不再把 Workspace/System/Appearance/Security/Data/About 等全部目录说明成 Agent 设置；Agent 卡片本身仍通过 `item.descriptionKey` 使用 `settings.descriptions.agent`，其它卡片继续使用各自 description key。                                                                                                                                                                                                                                                                  | `app/pages/settings/SettingsPage.vue`、mobile overview copy regression（见 §7.152）                                                                                                                                                                                                                                              |
| **P2 · ✅ 已修复 2026-09-25**             | **Provider drawer footer 已按完整 default pair 隔离**：新增纯 resolver 以 `providerId + modelId` 判定当前卡片状态；只有 `provider.id === defaultProviderId` 且存在 `defaultModelId` 时才显示默认模型。非默认 Provider 仅展示自身 configured model count，不再回退首模型冒充默认。若 global default pair 指向当前 Provider 但 model 已不存在，则保留该 model id 并显示明确 stale/missing 警告。                                                                                                                                                                                                                   | `ModelProviderSettings.vue`、`provider-default-model-status.ts`、三语 missing 文案、provider default-model regression（见 §7.153）                                                                                                                                                                                               |
...
### 7.133 Runner catalog 与 Backend decoder 都允许 4,096 packs，但 HTTP adapter 默认只收 1MB，合法 catalog 会在 decode 前被 transport 拒绝（P2 · ✅ 已修复 2026-09-24）
...
**修复（2026-09-24）**：`RunnerHttpAdapter.catalog()` 现在使用独立 `MAX_CATALOG_RESPONSE_BYTES=8MB`，不再落入通用 1MB bounded-read。Runner `/v1/catalog` 本身返回的是精简管理 projection（recipe + pack family/version/displayName/contentDigest/disk/status/installed/enabled/inUse），因此该 route 可以在保留全局默认 1MB 防护的同时，为合法 4,096-pack catalog 提供足够 envelope。

新增 `tests/backend/workspace-catalog-response-contract.regression.ts`：构造 4,096 packs、每项约 320B displayName 的合法 wire，序列化结果约 2.30MB，明确大于旧 1MB、低于 8MB；真实 `decodeCatalog()` 返回 4,096 条，并静态锁定 catalog route 使用专用 cap。focused regression 与 Backend TypeScript 均通过。

---

### 7.134 Backend Plugin 的 async line handler 无 Promise owner，合法 JSON 但非法 protocol frame 可把插件错误升级成 Backend unhandled rejection（P1 · ✅ 已修复 2026-09-24）
...
**修复（2026-09-24）**：`BackendPluginProcess` 的 stdout ingestion 不再 fire-and-forget 丢弃 `handleLine()` Promise。每个由 `BoundedProtocolLineBuffer` 产出的 frame 都使用 `void this.handleLine(...).catch(error => this.protocolFailure(error))`；`protocolFailure()` 将异常压缩为最多 1KB message，调用 `failAll()` 让 pending lifecycle request/ready promise 收口，并仅 `SIGKILL` 当前 Plugin child。于是 `decodeLifecycleResult()`、`decodeStorageRequest()`、`decodeIntentRequest()`，以及 storage/intent response write 过程中抛出的异常都不会逃出 Plugin process owner。

新增 `tests/backend/plugin-backend-line-error-owner.regression.ts`，锁定唯一 stdout line ingestion 必须显式 `.catch(protocolFailure)`，三类 decoder 仍在该 owned path 内，并防止旧 `void handleLine(...)` 形式回归。focused regression 与 Backend TypeScript 均通过。

---

### 7.135 Backend Plugin Host→child response path 忽略 stdin backpressure，插件停读 stdin 时合法小请求可让主 Backend Writable queue 无界增长（P2 · ✅ 已修复 2026-09-24）
...
**修复（2026-09-24）**：新增 `BoundedProtocolWriter` 作为所有 Host→Plugin frame 的唯一 owner。单帧继续遵守既有 20MB protocol 上限；aggregate 待写队列新增 `MAX_PROTOCOL_QUEUED_BYTES=32MB` 与 `MAX_PROTOCOL_QUEUED_FRAMES=128`，超过即 `PLUGIN_BACKEND_BACKPRESSURE_LIMIT`。writer 按 Promise tail 串行发送；底层 `Writable.write()` 返回 false 时等待 `drain`，stdin `error/close` 或同步 write 异常都会 reject，最终由 §7.134 的 `protocolFailure()` fail pending lifecycle 并终止该 Plugin instance。原 lifecycle `request()`、`sendStorageResult()`、`sendIntentResult()` 已全部移除裸 `child.stdin.write()`。

同时 Plugin-originated Host RPC 增加 `MAX_PLUGIN_HOST_OPERATIONS=32`。`trackHostOperation()` 改接 operation factory，在检查 active count 后才调用 `handleStorage/handleIntent`，因此第 33 个请求不会先启动 Backend storage/AppIntent work 再被拒绝。新增 `tests/backend/plugin-backend-writer-backpressure.regression.ts`：使用 8-byte highWaterMark 的延迟 Writable 证明 frame 串行且 backpressure 未 drain 前下一帧不会写入；小 aggregate queue 的第三帧明确 fail-closed，完成后 counters 回到 0；并锁定生产代码无任何 `child.stdin.write()`、Host RPC 使用 factory-based concurrency gate。focused regression 与 Backend TypeScript 均通过。

---

### 7.136 `af543606` 默认模型 optimistic state 只缓存 modelId，跨 Provider 同名模型会解析/过滤到错误 Provider（P1 · ✅ 已修复 2026-09-24）
...
**修复（2026-09-24）**：复核当前 HEAD 后确认 `ModelProviderSettings.vue` 已经使用完整 `optimisticDefaultModelKey = providerId\0modelId`，`defaultModelKey` 只按完整 option key 查找，且 props watcher 同时观察 `defaultProviderId` 与 `defaultModelId`；因此问题记录中“跨 Provider 同名模型取第一个 id”与“Provider 改变但 modelId 不变不触发 watcher”已在后续代码中消失。本轮补齐剩余的 mutation-failure 生命周期：组件新增 `settingsBusy` prop，父级传入真实 `settingsMutationBusy`；当 settings write 从 busy→idle 时，无论成功或失败，optimistic key 都重新读取当前 requested props。成功时 props 已更新为新复合 key；失败时 props 未变，UI 自动回滚旧复合 key，不再永久保留未提交 optimistic selection。

新增 `tests/backend/default-model-optimistic-identity.regression.ts`，锁定 optimistic state 只能使用复合 key、props watcher 必须观察 Provider+Model、default option 必须按完整 key 匹配、settings mutation idle 时必须回到 requested props，并确认父级实际传递 `settingsMutationBusy`。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.137 `a6371f9` 重新打开 §7.38：Launcher 从“长按 320ms 才拖动”回退为 4px 位移即拖动（P1 · ✅ 已修复 2026-09-24）
...
**修复（2026-09-24）**：直接从历史修复提交 `3016ffb` 恢复 §7.38 已经实测过的交互参数与状态机，而不是重新定义拖动 UX。`AgentLauncher.vue` 重新采用 `HOLD_MS=320`、`MOVE_CANCEL_PX=10`、`RESET_VISIBLE_MS=7000`：pointer down 后只启动 hold timer；320ms 前移动超过 10px 会标记本次 gesture 已移动并取消 hold，但不会调用 `setLauncherPosition()`；只有 hold 成功后 pointer move 才更新位置。普通 down/up 在未移动且未进入 hold drag 时直接打开 Hub，快速滑动不会打开 Hub也不会移动 Launcher。

拖动结束后若位置非默认值，会显示 7 秒 `data-agent-launcher-reset` 可聚焦按钮；按钮文案/tooltip 已恢复到 en-US / zh-CN / ja-JP，右键 reset 仍保留为补充。`pointercancel` 若已经发生实际 drag，则先把 launcher 恢复到 pointer down 起点且不 emit `layoutChange`，保证取消手势不会被后续布局持久化。现有 viewport resize clamp 与当前 Launcher 视觉样式全部保留。新增 `tests/backend/agent-launcher-long-press.regression.ts` 锁定 320ms/10px/7s 常量、hold gate、可见 reset、cancel rollback，并禁止 `DRAG_THRESHOLD_PX=4` 与旧 `@click=handleClick` 回归。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.138 Provider discovery 最多返回 1,000 模型，但“导入全部/一键添加全部”会越过 100-model 持久化上限（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：Provider 模型数量上限现在由共享 protocol 常量 `AGENT_PROVIDER_MODEL_LIMIT=100` 定义。Backend `validateProviderInput()` 不再使用裸 `100`，而是直接引用该 contract；Frontend `ModelProviderSettings.vue` 同样用这个常量计算 `MAX_PROVIDER_MODELS` 与剩余容量，避免保存端与 UI 再次漂移。

新建 Provider 的 `submitModal()` 在“导入全部”模式下先将 endpoint discovery 裁到 `MAX_PROVIDER_MODELS`，并复用现有 `modelLimitReached` feedback 告知被跳过数量，因此 endpoint 返回 101–1000 个模型时仍只提交合法的最多 100 条；已有 Provider 的 `addAllDiscovered()` / `addSelectedDiscovered()` 原本已有 `remainingModelCapacity()` 裁剪，本轮统一到共享上限。新增 `tests/backend/provider-model-limit-contract.regression.ts`，锁定 protocol/Backend/Frontend 使用同一常量，并验证 101→100、1000→100、已有90+新11→只加10、恰好100→100。focused regression、Backend TypeScript 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.139 Runner coding projection 的 count/relevance 截断没有可靠暴露 `truncated`，Repo Map 还会在相关性排序前提前停止扫描（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`workspace_repo_map` 的候选发现与 symbol 投影现在解耦。Runner 会先遍历 scope 内全部已索引 TypeScript/JavaScript 文件，获取 path/imports 并使用原始 source content 做轻量 query relevance；排序完成后才选取 `maxFiles` 个 top candidate，再按全局 `maxSymbols` 预算收集 symbols。因此路径排序靠前的普通文件不再能在相关性排序前耗尽 budget、阻止后排精确匹配文件进入候选。`collectSymbols()` 改为内部多探测 1 条并返回 `{values,truncated}`，Repo Map 将 scan/file-count/symbol-count/output-byte 任一截断统一传播到最终 `truncated`。

`workspace_code_intel` 同时修正四种 action 的 count contract：symbols 直接使用 `collectSymbols` 的 has-more；diagnostics 在 slice 前比较原始数量；definition 对完整 declarations projection 先判 count 再 slice；references 最多探测到 `maxResults+1` 后才停止并设置 `countTruncated`。最终 `truncated = countTruncated || bounded.truncated`，不再比较已经裁过的 `results.length`。

新增 `tests/backend/workspace-code-intel-truncation.regression.ts`：构造路径靠前的普通文件和靠后的 `z-target.ts`，在 `maxFiles=1/maxSymbols=1` 时 query `needleTarget` 必须仍返回后者；同一文件多个 symbols 在 `maxResults=1` 时必须标截断；多 references 同样只返 1 条且 `truncated=true`。focused regression 与 Agent Runner TypeScript 均通过。

---

### 7.140 MCP HTTP/SSE response 没有 pre-decode 字节上限，10MB `MCP_OUTPUT_TOO_LARGE` 只能限制解码后的业务结果（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`SafeMcpFetch` 现在在把 `Response` 交给 MCP SDK 之前统一调用 `boundMcpResponse()`。普通 JSON/错误 response 先检查合法 `Content-Length`，随后 body 经过 `boundedByteStream()`；总 wire 上限定义为 `MCP_WIRE_RESPONSE_LIMIT_BYTES = 10MB + 256KB`，给 JSON-RPC envelope 留固定余量，但不再允许 SDK 先无限 `response.json()` 后才在 `jsonValue()` 处发现超限。

SSE 使用独立 `boundedSseStream()`：不设置 response 总累计上限，避免长寿命连接因为许多合法小事件累计超过 10MB 就被误杀；而是在原始 UTF-8 bytes 进入 SDK `TextDecoderStream/EventSourceParserStream` 前，按 `\n\n` / `\r\n\r\n` event boundary 维护单 event byte count，超过 `MCP_SSE_EVENT_LIMIT_BYTES = 10MB + 64KB` 立即抛 `MCP_SSE_EVENT_TOO_LARGE`。因此单个恶意 event 不会先在 SDK 内完整 materialize/JSON.parse，合法长流仍可持续。原 `jsonValue()` 的 10MB serialized-object check 保留为第二层业务 projection 防线。

新增 `tests/backend/mcp-wire-response-limit.regression.ts`：声明超限 Content-Length 在读 body 前拒绝；无 Content-Length JSON 流跨 wire cap 后停止继续 pull；无 event delimiter 的超大 SSE 同样在完整流读取前 fail；同时 24 个各 512KB 的独立 SSE event 累计超过 JSON 总 cap 仍能全部通过，验证 per-event 而非 total-stream 语义。focused regression 与 Backend TypeScript 均通过。

---

### 7.141 Provider version 推进后 Subagent Profile 的旧模型 ref 在 UI 中消失，正常重选还会保留隐藏 stale ref，导致 Profile 无法保存/运行（P1 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`SubagentSettings.vue` 现在把完整冻结 ref identity（含 `configurationVersion`）与可迁移的 model family identity（`providerId+modelId`）分开。`staleModelRefs(profile)` 会把 default/allowed 中无法精确命中当前 enabled Provider option 的 ref 明确列出来；每个 stale ref 在 Profile 卡片顶部显示旧 configurationVersion，而不是静默从 select/checkbox 中消失。若同一 provider/model 当前仍存在，用户必须显式点击“重新绑定到当前配置”，`replaceModelFamily()` 会原子删除该 family 的所有旧版本并加入当前 ref，同时在 default 属于同 family 时同步替换 default。

普通 `setDefaultModel()` / `toggleAllowedModel()` 也统一按 model family 去重：加入当前 v2 前先移除同 provider/model 的 v1/vN，杜绝正常控件再次制造隐藏双版本。若 Provider/model 已彻底不可用，UI 标为 unavailable，并在 stale ref 不再是默认且 allowed 至少保留一项时提供显式移除入口；用户可先选择新的默认模型，再清除旧 ref。`hasStaleModelRefs` 会阻止 Save，并在保存区显示可操作的 rebind/remove 提示，因此 stale payload 不会再打到 Backend 才得到机器错误；Backend `configurationVersion` fail-closed 冻结语义没有放宽。

新增 `tests/backend/subagent-model-rebind-ui.regression.ts`，锁定 stale detection、family replacement、Save preflight、rebind/remove UI 与 en-US/ja-JP/zh-CN 五组文案；模拟 `P/model@v1 → P/model@v2` replacement 后 payload 只保留 v2。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.142 Settings `?tab=` deep-link 只在 mount 时读取，与 route-name KeepAlive 缓存脱节（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`SettingsPage.vue` 不再依赖一次性的 `onMounted()` 消费 deep-link。新增 `syncRouteTab()`：把 `route.query.tab` 的 string/array 形态统一解析，只有当前 `allTabs` 中的合法值才采用，否则回到 `workspace`；同步时更新 `active` 并恢复 detail view。`watch(() => route.query.tab, syncRouteTab, { immediate:true })` 覆盖首次进入与已激活页面的 query 变化，`onActivated(syncRouteTab)` 则在 route-name KeepAlive 恢复同一 Settings 实例时再次以当前 URL 为权威 reconcile。

`selectTab()` 同时加入反向 URL 同步：本地导航仍立即更新 active/mobile state；若当前 query 与目标 tab 不同，则 `router.replace({ query:{...route.query, tab} })`，保留其它 query 参数并避免同值 replace 循环。这样首次 `/settings?tab=agent`、同页 query 切换、用户点击 tab 后复制 URL、离开后返回 cached Settings、浏览器 back/forward 都使用同一状态源。

新增 `tests/backend/settings-tab-route-sync.regression.ts`，锁定 `useRouter`、route-query immediate watch、`onActivated` reconcile、workspace fallback、query-preserving replace，以及旧 mount-only 逻辑已删除。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.143 Agent Settings 取消 visitedGroups lazy mount 后，未访问的隐藏分组也会立即发起网络请求并弹全局错误（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`AgentSettingsPanel.vue` 的四个二级分组重新恢复 `visitedGroups` 的模板消费。Models/Tools/Runtime/Safety 容器都采用 `v-if="visitedGroups.has(...)"` 与 `v-show="activeGroup === ..."` 组合：初始 `visitedGroups` 只有 `models`，因此首次进入 Agent Settings 时只有模型/预算子树会创建；`selectGroup()` 首次访问其它组时加入 visited，之后即使隐藏也只做 `v-show`，组件实例和草稿继续保活。

这意味着 PluginManagement/MCP/ACP/Workspace/SafetyNetwork 等 request-owning 子组件不再因为父级首次进入 Agent 页面就提前 mount，未访问 Tools/Runtime/Safety 时不会触发其 catalog/integration/runtime/connection 初始化，也不会把隐藏 remote catalog 的失败 toast 泄漏到 Models 页面。新增 `tests/backend/agent-settings-group-lazy-mount.regression.ts`，锁定四组均存在 lazy-mount gate、默认 visited 仅 Models、点击后才加入 visited。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.144 `a6371f9` 删除 Agent panel 的 DOM id，Settings tab 的 `aria-controls` 现在指向不存在的目标（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`SettingsPage.vue` 现在对 `<AgentSettingsPanel>` 显式传入 `id="settings-panel-agent"`。该组件保持单一根节点，Vue 会把非 prop attributes 透传到根 DOM，因此现有 mobile pills 与 desktop rail 上生成的 `aria-controls="settings-panel-agent"` 都重新拥有真实、唯一的 panel target；不需要在子组件内部硬编码父页面的导航 id。

新增 `tests/backend/settings-agent-aria-controls.regression.ts`，锁定动态 `aria-controls` contract 与 Agent panel target 同时存在。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.145 Settings 页面把导航声明为 ARIA tabs，却没有 tab widget 的键盘/焦点模型（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：Settings 两套响应式导航继续采用 ARIA tabs pattern，但现在拥有完整 composite focus model。每个 tab 都有布局唯一 id，并使用 `:tabindex="active === item.value ? 0 : -1"`；mobile tablist 为横向，Left/Right 循环移动；desktop rail 显式声明 `aria-orientation="vertical"`，Up/Down 循环移动；两者都支持 Home/End。`handleTabKeydown()` 在识别到导航键后 `preventDefault()`、调用统一 `selectTab(target)` 更新 active/route，再通过对应 mobile/desktop tab id 把 focus 移到新 active tab，采用 selection-follows-focus 语义。

8 个 Settings panel 继续使用稳定 `settings-panel-*` id，并全部增加 `role="tabpanel"` 与当前 tab 翻译生成的 `aria-label`，因此 tab 的 `aria-controls` 有真实 panel 目标，panel 本身也暴露明确可访问名称。新增 `tests/backend/settings-tabs-a11y.regression.ts`，锁定 roving tabindex、四个方向键、Home/End、desktop orientation、focus handoff 以及 8 个 tabpanel contract。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.146 `a6371f9` 用无运行语义的假 i18n 引用绕过 §7.40 dead-key 可达性门禁（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：删除 `AgentSettingsPanel.vue` 中完全未被消费的 `_legacyPluginGroupKey`，并从 en-US / ja-JP / zh-CN 同步删除 `agent.settings.groups.plugins`。`AgentSettingsPanel` 原先把 `agent.settings.navigation` 直接作为 aria-label 文本输出，本轮也改成真实 `$t('agent.settings.navigation')` lookup，避免可访问名称暴露内部 key。

`check-agent-i18n.mjs` 的 reachability 现在只扫描 Frontend / Backend / Agent Runner 运行源码，不再让 tests 或 scripts 本身成为 locale key 的 caller。reachability helper 直接内联并从该门禁脚本导出：先过滤注释，再识别运行源码的 exact/dynamic `agent.*` 引用；对 `const/let name = 'agent.*'` 这种简单绑定，如果 identifier 在文件里只出现声明这一处，就在 reachability 扫描前剔除，因此 dummy variable 无法续命。运行时对象映射、Backend `userSummary` key、ternary lookup 与动态 template prefix 仍可保留。完整 gate 在收紧后还发现 `agent.tasks.runId` 没有任何运行 caller，本轮一并从三语字典和 brand allowlist 删除。

新增 `tests/backend/agent-i18n-reachability.regression.mjs`，覆盖：未消费 const 不可达、comment-only 不可达、真实 `t/$t`、runtime mapping、ternary key 和 dynamic prefix 仍可达，并锁定 `groups.plugins` 已从组件/三语字典消失。`node scripts/check-agent-i18n.mjs`、focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.147 `97560f1` 把 MCP Unix-seconds 时间戳当毫秒传给 Date，最近刷新时间会显示到 1970 年（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：在 Agent 共享 `locale-format.ts` 新增 `dateFromUnixSeconds(value)`，只接受有限 number，并统一按 `value * 1000` 构造 `Date`；`null / undefined / NaN / invalid Date` 返回 `null`。`McpIntegrationSettings.vue` 的 `formatTime()` 现在只调用该 helper，再交给 `formatAgentTime()` 做 locale 展示，不再把 Unix-seconds 直接传给 `new Date(number)`。

`AgentIntegrationViewDto` 的 refresh 时间字段旁新增 protocol 注释，明确 `lastAttemptAt / lastSuccessAt / nextRetryAt` 是 Unix epoch seconds，和 Backend `clock.nowUnixSeconds()` 的写入语义对齐。新增 `tests/backend/mcp-unix-seconds-format.regression.ts`，固定 `1704067200` 并断言转换结果为 `2024-01-01T00:00:00.000Z` / `1704067200000ms`，同时覆盖 `null / undefined / NaN`，并锁定 MCP 组件已使用共享 helper、协议已声明单位。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.148 `97560f1` 多个 Settings 子组件把父级 async mutation 当成同步成功，失败时仍 toast“已保存”并保留未提交本地状态（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：父级 `AgentSettingsPanel.patchSection()` 现在显式返回 `Promise<boolean>`：只有 `agentApi.patchSettings()` 成功、authoritative `settings` 已更新并完成 post-commit storage sync 调度后才返回 `true`；busy / validation / network / 409 等失败仍由统一 `execute()` 报错，并向子级返回 `false`。Browser 与 ACP Profile 不再用普通 Vue emit 表达需要确认的持久化命令，而是接收 async callback。两者都先构造 next snapshot，await commit 成功后才写本地 draft；新增 modal 也只有成功后关闭，失败时表单继续保留且 authoritative props 不被 optimistic mutation 覆盖。

Model fallback 同样新增 `saveFallbackModels()` callback，新增/删除/排序，以及移除 Provider model 后的 fallback 自动清理都 await 同一 commit boundary。用户直接操作 fallback 时父级负责唯一一次 `saveNoticeFallback` success；模型移除后的后台清理传 `success=null`，避免和模型保存本身形成双 success。新增 `tests/backend/agent-settings-async-commit-boundary.regression.ts`，锁定 `patchSection` boolean 结果、Browser/ACP 无 save emit、modal 仅成功关闭、fallback 无 fire-and-forget emit / 本地提前 success。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.149 `97560f1` 设置卡片重构删除既有 Browser / ACP Profile / MCP 配置的编辑入口（P1 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：Browser Target 与 Endpoint 卡片都重新提供 edit action，并复用原有 modal 作为 edit mode。Target edit 以当前 `id / allowedUrlPatterns` 初始化，提交时保留现有 endpoints，只替换 target identity/patterns；Endpoint edit 以当前 endpoint 初始化并完整覆盖 `url / scope / via / priority / allowPlaintext / verifyTls`。两类保存继续走 §7.148 建立的 async `props.save()` commit boundary，失败不关闭 modal、不要求 delete/recreate。

ACP Profile 也复用新增 Profile modal 做 edit：以现有 `id / cwd / argvText` 初始化，提交后原位替换对应 draft。若该 Profile 当前仍被 ACP Integration 的 `profileId` 引用，则 edit mode 会锁住 ID，仅允许修改 argv/cwd，从 UI 层阻止 rename 制造悬空引用；未被引用的 Profile 仍可正常 rename。MCP Integration 增加 edit action，modal 从当前 `AgentIntegrationViewDto` 初始化 displayName/endpoint/trust/enabled，保存明确调用 typed `agentApi.updateIntegration()` 并保留原 `privateHostExceptions / protocolVersion`；credential 为空时不会清空已有凭据，有新值时仍可同时替换。

新增 `tests/backend/agent-settings-edit-paths.regression.ts`，锁定三类 edit action、Browser 完整 endpoint 字段覆盖、ACP 引用时 ID lock、MCP update path 与 config/credential 保留语义。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.150 Plugin 仓库分组只按 URL path“owner”聚合，可把不同 host/source 合成同一个“官方仓库”头部（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：把 Plugin catalog provenance 分组抽成 `plugin-catalog-grouping.ts` 纯函数。每个 source 先 canonicalize `repositoryUrl`（移除 fragment、由 URL parser 归一化 host/URL），group key 使用 `official|remote + NUL + canonical repository URL`，因此不同 host 同 owner 不会碰撞，同一 URL 若分别来自 official 与用户 remote 配置也不会混成一个 provenance。显示 label 改为 `hostname/owner`，owner 不再承担 identity 职责。

`PluginManagementSettings.vue` 现在直接消费该 helper：`group.official` 在创建时即由唯一 source 决定，不再出现组内 OR；header 复制/展示 URL 使用 `group.sourceUrl`，不再取 `catalogs[0]`；折叠状态与 Vue key 都改用 `group.key`，所以同名 owner 的多个 source 可独立展开。package 行仍携带各自真实 `catalog + official`，和安装/信任流程保持一致。新增 `tests/backend/plugin-catalog-provenance-grouping.regression.ts`，覆盖 GitHub/GitLab/自定义域同名 owner，以及相同 URL 的 remote-first + official-later，断言 group 数量、header sourceUrl、official 标记与 package provenance 均不交叉污染。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.151 `97560f1` ACP 新增 Profile 的“shell command string”正则 tokenizer 会静默改写 argv 语义（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：移除 `AcpRuntimeSettings.vue` 内部的正则 tokenizer，新增纯函数 `parseAcpCommandToArgv()`。JSON 输入只要以 `[` 开始就按 JSON string array 解析；结构错误、非字符串元素或 NUL 直接返回 `null`，不会再退回 command tokenizer。普通命令输入使用显式状态机，只做 argv lexical parsing：空白在 quote 外分词，单双引号去壳，相邻 quoted/unquoted 片段合并为一个 word，quote 外反斜杠转义下一字符，双引号内按 shell 风格处理 `\" / \\ / \$ / \``；`""`会保留为空参数。未闭合 quote 与尾部反斜杠都返回 invalid，组件的`isArgvValid`与 submit 路径都显式处理`null`。

三语 `argvHint` 同步改为“支持用于 argv 的 shell-style quoting/escaping，但不会执行 shell expansion；也可输入 JSON 字符串数组”，避免把 lexical parser 描述成真正 shell。新增 `tests/backend/acp-command-argv.regression.ts`，覆盖 escaped space、单双引号、`foo" bar"baz` 相邻拼接、escaped quote、空参数、JSON argv、malformed JSON、未闭合 quote、trailing escape 与非字符串 JSON 元素。focused regression、完整 Agent i18n gate、Frontend `vue-tsc --noEmit` 均通过。

---

### 7.152 `a6371f9` 移动端 Settings 总览头部误用 Agent 专属描述，整页目录被说明成 AI 设置（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：`SettingsPage.vue` 的移动端 catalog header 现在使用既有 `t('settings.mobile.settingsOverview')`，只描述“设置总览”本身；不再把 `settings.descriptions.agent` 作为全目录副标题。`tabGroups` 中 Agent item 的 `descriptionKey` 保持 `settings.descriptions.agent`，所有 catalog item 仍统一通过 `t(item.descriptionKey)` 渲染，因此通用 header 与具体 tab 描述的职责重新分离。

新增 `tests/backend/settings-mobile-overview-copy.regression.ts`，在 `data-testid="settings-mobile-catalog"` 范围内锁定 overview key 存在、Agent 专属 key 不出现在 header/catalog 固定文案位置，同时确认 item 仍使用动态 `descriptionKey` 且 Agent item 仍声明自身描述。focused regression 与 Frontend `vue-tsc --noEmit` 均通过。

---

### 7.153 `a6371f9` 每个 Provider 抽屉都显示全局 defaultModelId，非默认 Provider 会展示别家模型（P2 · ✅ 已修复 2026-09-25）
...
**修复（2026-09-25）**：Provider 卡片 footer 不再直接消费全局 `defaultModelId`，也不再用 `provider.models[0]` 作为伪默认值。新增纯函数 `resolveProviderDefaultModelStatus()`，输入当前 `providerId`、该 Provider 的 model id 集合以及全局 `defaultProviderId/defaultModelId` pair：非默认 Provider 或无全局默认时返回 `none`；完整 pair 命中当前 Provider 且 model 存在时返回 `default`；pair 指向当前 Provider 但 model 已不存在时返回 `stale` 并保留原 model id 作为配置漂移证据。

`ModelProviderSettings.vue` 的 footer 现在只在 resolver 非 `none` 时显示“默认模型”；`stale` 使用 warning icon 与新增三语 `defaultModelMissing` 文案，非默认 Provider 只保留自己的 configured model count。新增 `tests/backend/provider-default-model-status.regression.ts`，覆盖 Provider A=`gpt-4o` / Provider B=`claude-3-5-sonnet`、两个 Provider 同名 `gpt-4o`、无全局默认，以及默认 pair 指向当前 Provider 但 model 已缺失四种情况，并锁定旧 `defaultModelId || provider.models[0]?.id` 表达式已消失。focused regression、完整 Agent i18n gate 与 Frontend `vue-tsc --noEmit` 均通过。
