...
7. 大 owner 只按已确认职责边界继续拆分：`RootToolExecutionCoordinator` 在 governed mutation 已共享后只继续按剩余 Root tool projection/continuation 职责自然缩小。`PluginInstallService` 的 package/install transaction、runtime lifecycle、data-management collaborator 拆分，Browser Tool family 的 shared `BrowserSessionBindingAuthority` 与 lifecycle/observation/interaction/transfer factory 拆分，以及 `SubagentParticipantExecutor` 的 model/tool/completion collaborator 拆分均已完成；后续不得重新合并。StateCommit 不拆成多个 durable mutation authority。
