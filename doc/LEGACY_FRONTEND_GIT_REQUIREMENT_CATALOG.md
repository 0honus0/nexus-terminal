...
**Refactored design/implementation:** `TransferController` owns logical upload tasks/batches/conflict UX; the Workspace transfer adapter owns upload start/prepare protocol, bounded raw-file streaming and adaptive active-file scheduling with hardware-aware stream limits, file-size reservation weights, bounded head-of-line bypass and anti-starvation; Backend `StreamUploadOperationService` validates ordered chunks and declared size, writes to a private `.part` file, verifies its size and replaces the destination only on successful finalization; `SshSftpChannelPool` isolates the `transfer` filesystem role from control filesystem operations. One raw `/ws/uploads` socket is used per active file, browser and server queues are bounded, and abnormal data-stream closure routes through Backend abort cleanup instead of leaving a ghost `.part`/running task. Main-Workspace reconnect semantics are accounted for separately but consistently under `GREQ-XFER-002`.
