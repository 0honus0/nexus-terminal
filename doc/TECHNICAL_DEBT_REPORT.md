...
### 第六项推进记录（2026-04-23 第二十三批并行落地）

- 量化结果：
  - `import/no-cycle` 受控豁免：**维持 0**
  - `SFTP 服务体量`：`sftp.service.ts` **243 -> 243**（第二十三批保持不变；累计 **1884 -> 243**）
  - `认证控制器体量`：`auth.controller.ts` **1399 -> 1399**（第二十三批保持不变；累计 **1592 -> 1399**）
  - 回归测试补充：**动作层类型与行为断言补齐**
- 并行子任务 BO（登录 2FA 用户查询动作化）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.ts`
  - 结果：新增 `LoginTwoFactorUserRow`、`buildLoginTwoFactorUserQueryAction`、`resolveLoginTwoFactorUserLookupAction`，下沉数据库查询参数与用户校验动作。
- 并行子任务 BP（登录 2FA verify-outcome 动作化）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.ts`
  - 结果：新增 `resolveLoginTwoFactorVerifiedOutcomeAction`，统一 `verified/time_skew/invalid` 对应的日志、attempt 与响应构造。
- 并行子任务 BQ（第二十三批门禁固化）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.ts`
  - 结果：修复 outcome 响应联合类型收敛问题，`typecheck:backend` 与认证链路测试通过。

### 第六项推进记录（2026-04-23 第二十四批并行落地）

- 量化结果：
  - `import/no-cycle` 受控豁免：**维持 0**
  - `SFTP 服务体量`：`sftp.service.ts` **243 -> 243**（第二十四批保持不变；累计 **1884 -> 243**）
  - `认证控制器体量`：`auth.controller.ts` **1399 -> 1366**（第二十四批净减少 33 行；累计 **1592 -> 1366**）
  - 回归测试补充：**1 文件 / 3 组 outcome 场景断言**
- 并行子任务 BR（登录 2FA 数据库查询与用户校验动作接线）：
  - 变更文件：`packages/backend/src/auth/auth.controller.ts`
  - 结果：控制器改为通过 `buildLoginTwoFactorUserQueryAction + resolveLoginTwoFactorUserLookupAction` 获取查询参数并处理缺失用户/密钥失败分支，移除内联 SQL 与校验细节。
- 并行子任务 BS（登录 2FA verify-result 统一执行器接线）：
  - 变更文件：`packages/backend/src/auth/auth.controller.ts`
  - 结果：控制器改为消费 `resolveLoginTwoFactorVerifiedOutcomeAction`，统一处理 `verified/time_skew/invalid` 的日志、attempt 记录、响应与会话完成动作，主流程线性化。
- 并行子任务 BT（第二十四批回归补测与门禁固化）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.test.ts`
  - 结果：补齐 user-query/user-lookup 与 verified-outcome（三分支）断言，`typecheck:backend + 关键认证链路测试 + quality:check` 全绿。

### 第六项推进记录（2026-04-23 第二十五批并行落地）

- 量化结果：
  - `import/no-cycle` 受控豁免：**维持 0**
  - `SFTP 服务体量`：`sftp.service.ts` **243 -> 243**（第二十五批保持不变；累计 **1884 -> 243**）
  - `认证控制器体量`：`auth.controller.ts` **1366 -> 1363**（第二十五批净减少 3 行；累计 **1592 -> 1363**）
  - 回归测试补充：**2 文件 / 3 组断言**
- 并行子任务 BU（登录 2FA 控制器副作用执行器收敛）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.ts`、`packages/backend/src/auth/auth.controller.ts`
  - 结果：新增 `applyLoginTwoFactorAttemptAction`，控制器不再直接判断 attempt kind，失败/成功记录改为统一动作执行器分发。
- 并行子任务 BV（认证控制器预处理流程切片）：
  - 变更文件：`packages/backend/src/auth/auth-login-2fa-flow.utils.ts`、`packages/backend/src/auth/auth.controller.ts`
  - 结果：新增 `resolveLogin2FAVerificationPrecheck` 组合 helper，一次完成 pending 校验与 token 校验，控制器预处理分支收敛。
- 并行子任务 BW（第二十五批回归补测与门禁固化）：
  - 变更文件：`packages/backend/src/auth/auth-login-2fa-flow.utils.test.ts`、`packages/backend/src/auth/auth-login-two-factor-actions.utils.test.ts`
  - 结果：补充 precheck 与 attempt 执行器测试，`typecheck:backend + 关键认证链路测试 + quality:check` 全绿。

### 第六项推进记录（2026-04-24 第二十六批并行落地）

- 量化结果：
  - `import/no-cycle` 受控豁免：**维持 0**
  - `SFTP 服务体量`：`sftp.service.ts` **243 -> 243**（第二十六批保持不变；累计 **1884 -> 243**）
  - `认证控制器体量`：`auth.controller.ts` **1363 -> 1363**（第二十六批保持不变；累计 **1592 -> 1363**）
  - 回归验证：**认证链路定向测试 + quality:check**
- 并行子任务 BX（登录 2FA side-effect 服务对象去重封装）：
  - 变更文件：`packages/backend/src/auth/auth.controller.ts`
  - 结果：提取 `loginTwoFactorSideEffectServices` 局部常量，移除 `verifyLogin2FA` 内重复服务对象字面量。
- 并行子任务 BY（认证控制器局部流程命名统一）：
  - 变更文件：`packages/backend/src/auth/auth.controller.ts`
  - 结果：统一 `verificationPrecheckAction`、`verificationOutcomeAction` 命名风格，保持语义不变并降低局部认知负担。
- 并行子任务 BZ（第二十六批回归补测与门禁固化）：
  - 变更文件：`packages/backend/src/auth/auth.controller.ts`
...
> PR #22 修复了 13 项（H1-H4/H6/M3/M4/M7/M8/M9/M10/L3/L10）。后续 PR 追加修复 6 项（M2/M5/M6/L1/L2/L9），再追加修复 L8（大组件拆分）。剩余 M1（FileManager 拆分）、L4（路线图功能）、L6（移动端体验）待后续处理。
