...
> **生成时间**：2025-12-23 | **更新时间**：2026-04-23（第六项第二十三批并行落地）
...
| 6) 核心文件体量偏大且存在多处受控循环依赖豁免（`import/no-cycle`）                        | 长期维护复杂度上升，局部改动回归半径扩大                                 | P3     | 拆分超大模块（优先 FileManager/SFTP/认证链路），逐步消减循环依赖                    | 关键模块单文件体量下降，循环依赖豁免数量持续收敛               | 🟡 进行中（第二十三批完成） |
...
- 量化结果：
  ...
  - 回归测试补充：**2 文件 / 5 类动作断言**
- 并行子任务 BO（`verifyLogin2FA` 正常路径分支压平）：
  - 变更文件：`packages/backend/src/auth/auth.controller.ts`
  - 结果：将登录 2FA 的 dev 诊断日志、失败记录参数构造与成功完成参数构造改为动作化调用，控制器内联分支进一步压平，HTTP 编排职责更集中。
- 并行子任务 BP（登录 2FA 动作层类型收敛）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.ts`
  - 结果：新增 `LoginTwoFactorAttemptAction`、成功/失败 attempt payload、missing-secret/fallback-response/session-completion 动作类型，缩小控制器端条件歧义。
- 并行子任务 BQ（第二十三批回归补测与门禁固化）：
  - 变更文件：`packages/backend/src/auth/auth-login-two-factor-actions.utils.test.ts`、`packages/backend/src/auth/auth.controller.test.ts`
  - 结果：补充 attempt action、诊断日志动作与 invalid token 失败记录断言，`typecheck + 关键认证链路测试 + quality:check` 全绿。

### 第六项下一批并行子任务清单（第二十四批）

1. 任务 BR：登录 2FA 数据库查询与用户校验动作化
   - 目标：将 `getDb + user/two_factor_secret` 校验收敛为动作层入口，继续减少控制器内 DB 细节。
   - 验收：缺失用户/密钥场景响应语义不变，控制器中的查询+校验代码减少。
2. 任务 BS：登录 2FA verify-result 到 side-effect 的统一执行器
   - 目标：将 `verified/time-skew/invalid` 对应的日志、事件记录、响应发送收敛为统一执行器。
   - 验收：三类分支行为不变，控制器主流程进一步线性化。
3. 任务 BT：第二十四批回归补测与门禁固化
   - 目标：补齐第二十四批新增动作层测试并验证门禁稳定。
