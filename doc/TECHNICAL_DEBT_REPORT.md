...
> **生成时间**：2025-12-23 | **更新时间**：2026-04-23（第六项第七批并行落地）
...
| 6) 核心文件体量偏大且存在多处受控循环依赖豁免（`import/no-cycle`）                        | 长期维护复杂度上升，局部改动回归半径扩大                                 | P3     | 拆分超大模块（优先 FileManager/SFTP/认证链路），逐步消减循环依赖                    | 关键模块单文件体量下降，循环依赖豁免数量持续收敛               | 🟡 进行中（第七批完成） |
...
1. 任务 V：`sftp.service.ts` 下载/上传入口继续下沉（保留协议边界）
   - 目标：将高分支的下载、上传消息编排拆入独立执行器，进一步收敛服务主类体量。
   - 验收：二进制/文本上传下载行为与错误码保持一致，新增执行器单测覆盖成功/失败/未就绪分支。
2. 任务 W：认证控制器 passkey 注册与断言校验动作分层
   - 目标：将 passkey challenge 生成、校验结果归并、审计事件写入动作下沉，减少控制器内联分支。
   - 验收：现有 passkey 测试保持全绿，新增动作层测试覆盖 challenge 过期与验证失败路径。
3. 任务 X：第八批新增回归测试（SFTP 上传下载动作层 + Passkey 动作层）
   - 目标：为第八批职责下沉提供回归保护并稳定改造节奏。
