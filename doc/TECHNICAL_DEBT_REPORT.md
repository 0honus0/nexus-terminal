...
> **状态**：🟡 收敛中 | **更新时间**：2026-05-02 | **收敛率**：29/84 已修复（35%）
...
| 代码质量 | 2→1      | 10        | 20        | 15→12     | **47**       | **4**  |
| 边界条件 | 3→0      | 6→1       | 10→4      | 9→3       | **28**       | **20** |
| 测试覆盖 | —        | —         | —         | —         | **独立章节** | 2      |
| **合计** | **6→1**  | **21→11** | **30→24** | **27→15** | **84**       | **29** |
...
| ID   | 问题                                                      | 位置                                                      | 修复建议                                                         |
| ---- | --------------------------------------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------- |
| H-6  | 7 个空 catch 块静默吞错（含迁移失败）                     | `migrations.ts:531` 等                                    | 至少记录日志，迁移失败应终止启动                                 |
...
| M-3  | `useWebSocketConnection.ts` 641 行                    | 前端 composable           | 提取消息解析、重连逻辑为子模块                        |
| M-4  | `useSshTerminal.ts` 522 行                            | 前端 composable           | 提取缓冲管理、事件处理为子模块                        |
...
| M-26     | 无端口范围验证（1-65535）                                 | `connections.controller.ts`               | 添加 `port >= 1 && port <= 65535` 校验                  |
| M-27     | 无主机名格式验证                                          | `connections.controller.ts`               | 添加最大 253 字符 + 合法主机名/IP 正则                  |
| M-28     | 注册无用户名长度/密码复杂度限制                           | `auth.controller.ts`                      | 用户名 3-64 字符 + 字母数字；密码含字母和数字           |
| M-29     | `ssh:resize` 无 cols/rows 上限                            | `ssh.handler.ts`                          | 添加 `cols <= 1000 && rows <= 500`                      |
...
**文档维护者**：工程治理
**最后更新**：2026-05-02（84 项全面修复 + Codex 审查补漏 7 项，累计 29/84 已修复，收敛率 35%）
