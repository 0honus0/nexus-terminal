...
const ensureDefaultRemoteHtmlPresetsUrl = async (settings: AppearanceSettings): Promise<void> => {
  const storedUrl = typeof settings.remoteHtmlPresetsUrl === 'string' ? settings.remoteHtmlPresetsUrl.trim() : '';
...
export const updateSettings = async (settingsDto: UpdateAppearanceDto): Promise<boolean> => {
  ...
  if (settingsDto.hasOwnProperty('terminal_custom_html')) {
    if (
      ...
      if (typeof settingsDto.terminal_custom_html === 'string' && settingsDto.terminal_custom_html.length > 10240) {
        // 10KB 限制
...
export const listPresetHtmlThemes = async (): Promise<Array<{ name: string; type: 'preset' }>> => {
  try {
    ...
    return files.filter((file) => file.endsWith('.html')).map((name) => ({ name, type: 'preset' as const })); // Add type
  } catch (error: any) {
...
export const listUserCustomHtmlThemes = async (): Promise<Array<{ name: string; type: 'custom' }>> => {
  try {
    ...
    return files.filter((file) => file.endsWith('.html')).map((name) => ({ name, type: 'custom' as const })); // Add type
  } catch (error: any) {
...
export const getRemoteHtmlPresetContent = async (fileUrl: string): Promise<string> => {
  ...
  if (
    ...
    parsedUrl.port ||
    !parsedUrl.pathname.toLowerCase().endsWith('.html')
  ) {
