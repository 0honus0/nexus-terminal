...
export class NotificationService {
  ...
  private async _testTelegramSetting(config: TelegramConfig): Promise<{ success: boolean; message: string }> {
    ...
    const messageText = this._renderTemplate(templateToUse, templateDataTelegramTest, defaultMessageTemplateFromI18n);
  ...
  private async _sendWebhook(
    ...
    const templateDataWebhook: Record<string, string> = {
      ...
      timestamp: formatInTimeZone(new Date(translatedPayload.timestamp), userTimezone, "yyyy-MM-dd'T'HH:mm:ss.SSSXXX"),
