...
export class WorkspaceShellIntegrationService {
  ...
  private async handlePrompt(sessionId: string, state: ShellState) {
    ...
    if (!pending.executing) {
      ...
      const historyToken = `__NEXUS_INTERNAL_CD_${randomUUID().replace(/-/g, '')}__`;
      shell.write(
        `\x15cd ${quotePosixShellArg(pending.requestedPath)}; : ${quotePosixShellArg(historyToken)}; ${this.buildHistoryCleanup(state.shellKind, historyToken)}\r`,
      );
      return;
    }
    this.clearPending(state);
  ...
  private buildProbeCommand(start: string, end: string) {
    const historyToken = `__NEXUS_INTERNAL_PROBE_${randomUUID().replace(/-/g, '')}__`;
    return (
      [
        ...
        `: ${quotePosixShellArg(historyToken)};`,
        this.buildHistoryCleanupFromProbe(historyToken),
        'stty echo 2>/dev/null;',
        "printf '\\n'",
      ].join(' ') + '\r'
    );
  ...
  private buildHookCommand(kind: 'bash' | 'zsh', start: string, end: string) {
    const historyToken = `__NEXUS_INTERNAL_HOOK_${randomUUID().replace(/-/g, '')}__`;
    ...
    return (
      [
        ...
        `: ${quotePosixShellArg(historyToken)};`,
        this.buildHistoryCleanup(kind, historyToken),
        'stty echo 2>/dev/null;',
        "printf '\\n'",
      ].join(' ') + '\r'
    );
  ...
  private buildHistoryCleanup(kind: WorkspaceShellKind | undefined, token: string): string {
    const quotedToken = quotePosixShellArg(token);
    if (kind === 'bash') {
      return `case "$(builtin history 1 2>/dev/null)" in *${quotedToken}*) builtin history -d "$HISTCMD" 2>/dev/null || true ;; esac;`;
    }
    if (kind === 'zsh') {
      return `zmodload zsh/parameter 2>/dev/null && case "${'${history[$HISTCMD]-}'}" in *${quotedToken}*) unset "history[$HISTCMD]" 2>/dev/null ;; esac || true;`;
    }
    return ':';
  }

  private buildHistoryCleanupFromProbe(token: string): string {
    const quotedToken = quotePosixShellArg(token);
    return [
      'case "$__nexus_shell_kind" in',
      `bash) case "$(builtin history 1 2>/dev/null)" in *${quotedToken}*) builtin history -d "$HISTCMD" 2>/dev/null || true ;; esac ;;`,
      `zsh) zmodload zsh/parameter 2>/dev/null && case "${'${history[$HISTCMD]-}'}" in *${quotedToken}*) unset "history[$HISTCMD]" 2>/dev/null ;; esac || true ;;`,
      'esac;',
    ].join(' ');
  }
}
