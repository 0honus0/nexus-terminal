...
import { getDbInstance, runDb, getDb as getDbRow, allDb } from '../../infrastructure/database/connection';
import type { RdpConnectionOptions } from '../../shared/types/connection.types';
...
interface FullConnectionDbRow extends Omit<FullConnectionData, 'jump_chain' | 'rdp_options' | 'tag_ids'> {
...
export const createConnection = async (
  data: Omit<FullConnectionData, 'id' | 'created_at' | 'updated_at' | 'last_connected_at' | 'tag_ids'>,
): Promise<number> => {
...
export const updateConnectionTags = async (connectionId: number, tagIds: number[]): Promise<boolean> => {
  ...
  // 1. 检查连接是否存在
  try {
    const connectionExists = await getDbRow<{ id: number }>(db, `SELECT id FROM connections WHERE id = ?`, [
      connectionId,
    ]);
    if (!connectionExists) {
  ...
  try {
    ...
    if (tagIds.length > 0) {
      ...
      const insertPromises = validTagIds.map((tagId) => runDb(db, insertSql, [connectionId, tagId]));
...
export const bulkInsertConnections = async (
  ...
  connections: Array<
    Omit<FullConnectionData, 'id' | 'created_at' | 'updated_at' | 'last_connected_at'> & { tag_ids?: number[] }
  >,
