  ...
  async recordFailedAttempt(ip: string): Promise<void> {
    ...
    try {
      ...
      if (entry) {
        ...
        await runDb(db, 'UPDATE ip_blacklist SET attempts = ?, last_attempt_at = ?, blocked_until = ? WHERE ip = ?', [
          newAttempts,
          now,
          blockedUntil,
          ip,
        ...
        await runDb(db, 'INSERT INTO ip_blacklist (ip, attempts, last_attempt_at, blocked_until) VALUES (?, ?, ?, ?)', [
          ip,
          attempts,
          now,
          blockedUntil,
