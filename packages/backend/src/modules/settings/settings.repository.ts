...
import { SidebarConfig, LayoutNode, PaneName } from '../../shared/types/settings.types';
import { CaptchaSettings } from '../../shared/types/settings.types';
...
export const ensureDefaultSettingsExist = async (db: Database): Promise<void> => {
  type OmitIdRecursive<T> = T extends object ? { [K in keyof Omit<T, 'id'>]: OmitIdRecursive<T[K]> } : T;
