...
const getSftpStats = (sftp: SFTPWrapper, remotePath: string): Promise<Stats> => new Promise((resolve, reject) => {
...
const getSftpRealPath = (sftp: SFTPWrapper, remotePath: string): Promise<string> => new Promise((resolve, reject) => {
...
const readSftpDirectory = (sftp: SFTPWrapper, remotePath: string): Promise<any[]> => new Promise((resolve, reject) => {
...
const addDirectoryToArchive = async (
    ...
    for (const entry of entries) {
        ...
        const targetStats: Stats = entry.attrs.isSymbolicLink()
            ? await getSftpStats(sftp, currentRemotePath)
            : entry.attrs;
