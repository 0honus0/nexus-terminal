  ...
  try {
    ...
    if (typedRawConnInfo.proxy_db_id) {
      ...
      const proxyType =
        ...
        (() => {
          throw new Error(`Proxy for Connection ID ${connectionId} (actual_proxy_server_type) has null type.`);
        })();
      const proxyHost =
    ...
    if (typedRawConnInfo.jump_chain) {
      try {
        ...
        if (Array.isArray(jumpHostConnectionIds) && jumpHostConnectionIds.length > 0) {
          const hopTargetDetailsList = await Promise.all(
            jumpHostConnectionIds.map(async (hopConnectionId, index) => {
              if (typeof hopConnectionId !== 'number') {
                throw new Error(
                  `Jump host ID at index ${index} in jump_chain for connection ${connectionId} is not a number. Found: ${hopConnectionId}`,
                );
              }
    ...
    const eventHandlers = {
      ready: () => {
        ...
        if (isFinalClient && connectionIdForUpdate !== null && connectionIdForUpdate !== -1) {
          ...
          setImmediate(() => {
            void ConnectionRepository.updateLastConnected(connectionIdForUpdate, currentTimeSeconds).catch(
              (updateError) => {
                console.error(
...
const _establishDirectSshConnection = (connDetails: DecryptedConnectionDetails, timeout: number): Promise<Client> => {
...
const _connectViaHttpProxy = (
  ...
  return new Promise((resolve, reject) => {
    ...
    if (proxyDetails.username) {
      const auth =
        'Basic ' + Buffer.from(proxyDetails.username + ':' + (proxyDetails.password || '')).toString('base64');
      reqOptions.headers = {
        ...reqOptions.headers,
...
const _establishProxyConnection = async (connDetails: DecryptedConnectionDetails, timeout: number): Promise<Client> => {
  ...
  return new Promise<Client>(async (resolveOuter, rejectOuter) => {
    ...
    if (hopIndex === jumpChainDetails.length) {
      ...
      _setupSshClientListenersAndConnect(
        ...
        .catch((err) => {
          cleanupAndReject(
            new Error(`Final target connection error for ${finalTargetDetails.name} (via jump chain): ${err.message}`),
            finalClient,
          );
        });
