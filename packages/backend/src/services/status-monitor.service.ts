  ...
  private async fetchServerStatus(sshClient: Client, sessionId: string): Promise<ServerStatus> {
    ...
    try {
      ...
      try {
        let cpuModelOutput = '';
        try {
          cpuModelOutput = await this.executeSshCommand(
            sshClient,
        ...
        if (currentCpuTimes) {
          ...
          if (prevCpuStats && prevCpuStats.timestamp < now) {
            ...
            if (totalDiff > 0 && timeDiffMs > 100) {
              ...
              status.cpuPercent = parseFloat(
                Math.max(0, Math.min(100, usageRatio * 100)).toFixed(1)
              );
            } else {
      ...
      try {
        ...
        const match = uptimeOutput.match(
      ...
      try {
        ...
        if (currentStats) {
          const defaultInterface =
            (await this.getDefaultInterface(sshClient)) ||
            Object.keys(currentStats).find((iface) => iface !== 'lo'); // Detect or fallback excluding loopback
          ...
          if (defaultInterface && currentStats[defaultInterface]) {
            ...
            if (prevStats && prevStats.timestamp < timestamp) {
              ...
              if (timeDiffSeconds > 0.1) {
                status.netRxRate = Math.max(
                  0,
                  Math.round((currentRx - prevStats.rx) / timeDiffSeconds)
                );
                status.netTxRate = Math.max(
                  0,
                  Math.round((currentTx - prevStats.tx) / timeDiffSeconds)
                );
              } else {
                status.netRxRate = 0;
                status.netTxRate = 0;
