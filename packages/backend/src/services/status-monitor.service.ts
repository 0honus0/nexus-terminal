...
export class StatusMonitorService {
  ...
  private async fetchServerStatus(sshClient: Client, sessionId: string): Promise<ServerStatus> {
    ...
    if (cpuTimes) {
      ...
      if (previous && previous.timestamp < timestamp) {
        ...
        if (totalDiff > 0 && idleDiff >= 0) {
          cpuPercent = Number(
            (Math.max(0, Math.min(1, 1 - Math.min(idleDiff, totalDiff) / totalDiff)) * 100).toFixed(1),
          );
        }
      }
    ...
    if (netInterface && netStats[netInterface]) {
      ...
      if (previous && previous.timestamp < timestamp) {
        ...
        status.netRxRate =
          elapsedSeconds > 0 ? Math.max(0, Math.round((current.rx_bytes - previous.rx) / elapsedSeconds)) : 0;
        status.netTxRate =
          elapsedSeconds > 0 ? Math.max(0, Math.round((current.tx_bytes - previous.tx) / elapsedSeconds)) : 0;
      } else {
        status.netRxRate = 0;
        status.netTxRate = 0;
      }
  ...
  private parseDiskUsage(output: string): Pick<ServerStatus, 'diskTotal' | 'diskUsed' | 'diskPercent'> {
    const lines = output
      ...
      .filter(Boolean);
    const dataLine =
      lines.slice(1).find((line) => line.endsWith(' /')) ?? (lines.length > 0 ? lines[lines.length - 1] : '');
