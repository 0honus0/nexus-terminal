    ...
    SharedFactRepositoryPort
{
  ...
  async completeJoinResume(
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (!work || work.status !== 'claimed' || work.owner_epoch !== ownerEpoch)
      ...
      if (parentDelegationId !== null) {
        ...
        if (
          !parent ||
          parent.child_runtime_id !== runtimeId ||
  ...
  async settleWork(
    ...
    const current = await this.db.queryOne<{ status: string }>('SELECT status FROM agent_scheduler_work WHERE id = ?', [
      workId,
    ]);
    ...
    return this.db.transaction(async (tx) => {
      ...
      for (const row of rows) {
        const terminalDelegation =
          ...
          row.delegation_status === 'failed' ||
          row.delegation_status === 'cancelled';
