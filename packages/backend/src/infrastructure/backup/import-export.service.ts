...
import { decrypt, getEncryptionKeyBuffer as getCryptoKeyBuffer } from '../../shared/security/crypto';
import { getAllDecryptedSshKeys, DecryptedSshKeyDetails } from '../../modules/ssh-keys/ssh_key.service';
  ...
  try {
    ...
    for (const sshKey of allSshKeys) {
      if (sshKey.name && sshKey.privateKey) {
        ...
        await zipWriter.add(`ssh_keys/${sanitizedKeyName}.txt`, new TextReader(sshKey.privateKey));
      } else {
...
export const importConnections = async (fileBuffer: Buffer): Promise<ImportResult> => {
  ...
  try {
    ...
    const connectionsToInsert: Array<
      Omit<ConnectionRepository.FullConnectionData, 'id' | 'created_at' | 'updated_at' | 'last_connected_at'> & {
        tag_ids?: number[];
      }
    > = [];
