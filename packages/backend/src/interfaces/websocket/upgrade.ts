...
import { checkIpWhitelist } from '../http/auth/ipWhitelist.middleware';
    ...
    sessionParser(request, sessionResponse, () => {
      ...
      if (pathname === '/rdp-proxy' || pathname === '/ws/rdp-proxy') {
        ...
        if (
          ...
          (dpi !== undefined && typeof dpi !== 'string')
        ) {
        ...
        const uploadSessionId = typeof parsedUrl.query.sessionId === 'string' ? parsedUrl.query.sessionId.trim() : '';
        ...
        console.log(
          `WebSocket: Handling upload transport upgrade for user ${request.session.username}, session ${uploadSessionId}`,
        );
        wss.handleUpgrade(request, socket, head, (ws) => {
          const extWs = ws as AuthenticatedWebSocket;
          extWs.userId = request.session.userId;
