...
export function handleRdpProxyConnection(ws: AuthenticatedWebSocket, request: WebSocketRequest): void {
  ...
  if (!rdpToken || !rdpWidthStr || !rdpHeightStr) {
    ...
    ws.send(
      JSON.stringify({ type: 'rdp:error', payload: 'Missing RDP connection parameters (token, width, height).' }),
    );
  ...
  console.log(
    `WebSocket: Remote Desktop Proxy for ${ws.username} attempting to connect to ${safeRemoteDesktopTargetUrl}`,
  );
  ...
  rdpWs.on('error', (error) => {
    ...
    console.error(
      `[RDP 代理 RDP WS 错误] 用户: ${ws.username}, 会话: ${ws.sessionId}, 连接到 ${safeRemoteDesktopTargetUrl} 时出错:`,
      error,
    );
  ...
  rdpWs.on('close', (code, reason) => {
    ...
    rdpWsClosed = true;
    console.log(
      `[RDP 代理 RDP WS 关闭] 用户: ${ws.username}, 会话: ${ws.sessionId}, 到 ${safeRemoteDesktopTargetUrl} 的连接已关闭。代码: ${code}, 原因: ${reason.toString()}`,
    );
    if (!clientWsClosed && ws.readyState !== WebSocket.CLOSED && ws.readyState !== WebSocket.CLOSING) {
  ...
  rdpWs.on('open', () => {
    clearTimeout(connectTimeout);
    console.log(
      `[RDP 代理 RDP WS 打开] 用户: ${ws.username}, 会话: ${ws.sessionId}, 到 ${safeRemoteDesktopTargetUrl} 的连接已建立。开始转发消息。`,
    );
