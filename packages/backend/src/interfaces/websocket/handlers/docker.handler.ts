...
import { isSafeDockerIdentifier } from '../../../platform/execution/posix-shell';
...
export async function handleDockerGetStatus(ws: AuthenticatedWebSocket, sessionId: string | undefined): Promise<void> {
  ...
  try {
    ...
    const isUnavailable =
      errorMessage.includes('command not found') || errorMessage.includes('Cannot connect to the Docker daemon');
    if (ws.readyState === WebSocket.OPEN) {
      if (isUnavailable) {
  ...
  if (
    ...
    if (ws.readyState === WebSocket.OPEN)
      ws.send(
        JSON.stringify({
          type: 'docker:command:error',
          payload: { command: command, message: 'Invalid containerId or command.' },
        }),
      );
    return;
  }
  ...
  try {
    ...
    if (ws.readyState === WebSocket.OPEN)
      ws.send(
        JSON.stringify({
          type: 'docker:command:error',
          payload: { command, containerId, message: `Failed to execute remote command: ${error.message}` },
        }),
      );
  }
}
    ...
    if (execResult.stderr) {
      ...
      if (ws.readyState === WebSocket.OPEN)
        ws.send(
          JSON.stringify({
            type: 'docker:stats:error',
            payload: { containerId, message: execResult.stderr.trim() || 'Error executing stats command.' },
          }),
        );
      return;
    }
    ...
    if (!execResult.stdout) {
      ...
      if (!execResult.stderr && ws.readyState === WebSocket.OPEN) {
        ws.send(
          JSON.stringify({
            type: 'docker:stats:error',
            payload: { containerId, message: 'No stats data received (container might be stopped).' },
          }),
        );
      }
      return;
