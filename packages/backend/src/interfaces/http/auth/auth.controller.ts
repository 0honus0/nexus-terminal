...
import { getDbInstance, runDb, getDb, allDb } from '../../../infrastructure/database/connection';
...
import { passkeyService } from '../../../modules/passkey/passkey.service'; // +++ Passkey Service
import { passkeyRepository } from '../../../modules/passkey/passkey.repository'; // +++ Passkey Repository
import { userRepository } from '../../../modules/user/user.repository'; // For passkey auth success
...
export const generatePasskeyRegistrationOptionsHandler = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `[AuthController] Generated Passkey registration options for user ${username}, challenge: ${options.challenge.substring(0, 10)}...`,
    );
    res.json(options);
  } catch (error: any) {
 ...
 */
type PasskeyRegistrationRequestBody = {
  username?: string;
  registrationResponse?: RegistrationResponseJSON;
};

export const verifyPasskeyRegistrationHandler = async (
  req: Request<Record<string, never>, unknown, PasskeyRegistrationRequestBody>,
  res: Response,
): Promise<void> => {
  ...
  try {
    ...
    if (verification.verified && verification.newPasskeyToSave) {
      ...
      console.log(
        `[AuthController] 用户 ${userHandle} 的 Passkey 注册成功并已保存。 CredentialID: ${verification.newPasskeyToSave.credential_id}`,
      );
      ...
      res.status(400).json({ verified: false, message: 'Passkey 注册验证失败。' });
    }
  } catch (error: any) {
...
export const generatePasskeyAuthenticationOptionsHandler = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.log(
      `[AuthController] Generated Passkey authentication options (username: ${username || 'any'}), challenge: ${options.challenge.substring(0, 10)}...`,
    );
    res.json(options);
  } catch (error: any) {
    console.error(
 ...
 */
type PasskeyAuthenticationRequestBody = {
  username?: string;
...
export const verifyPasskeyAuthenticationHandler = async (
  req: Request<Record<string, never>, unknown, PasskeyAuthenticationRequestBody>,
  res: Response,
): Promise<void> => {
  ...
  try {
    ...
    if (verification.verified && verification.userId && verification.passkey) {
      ...
      console.log(
        `[AuthController] 用户 ${user.username} (ID: ${user.id}) 通过 Passkey (ID: ${verification.passkey.id}) 认证成功。`,
      );
...
export const deleteUserPasskeyHandler = async (
  ...
  try {
    ...
    if (wasDeleted) {
      console.log(
        `[AuthController] 用户 ${username} (ID: ${userId}) 成功删除了 Passkey (CredentialID: ${credentialID})。`,
      );
      ...
      console.warn(
        `[AuthController] 用户 ${username} (ID: ${userId}) 删除 Passkey (CredentialID: ${credentialID}) 失败，但未抛出错误。`,
      );
      res.status(404).json({ message: 'Passkey 未找到或无法删除。' });
    }
  } catch (error: any) {
    console.error(
      `[AuthController] 用户 ${username} (ID: ${userId}) 删除 Passkey (CredentialID: ${credentialID}) 时出错:`,
      error.message,
      error.stack,
    );
...
export const updateUserPasskeyNameHandler = async (
  ...
  try {
    ...
    console.log(
      `[AuthController] 用户 ${username} (ID: ${userId}) 成功更新了 Passkey (CredentialID: ${credentialID}) 的名称为 "${trimmedName}"。`,
    );
    ...
    console.error(
      `[AuthController] 用户 ${username} (ID: ${userId}) 更新 Passkey (CredentialID: ${credentialID}) 名称时出错:`,
      error.message,
      error.stack,
    );
    if (error.message === 'Passkey not found.') {
...
export const changePassword = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const result = await runDb(db, 'UPDATE users SET hashed_password = ?, updated_at = ? WHERE id = ?', [
      newHashedPassword,
      now,
      userId,
    ]);
...
export const verifyAndActivate2FA = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (verified) {
      const now = Math.floor(Date.now() / 1000);
      const result = await runDb(db, 'UPDATE users SET two_factor_secret = ?, updated_at = ? WHERE id = ?', [
        tempSecret,
        now,
        userId,
      ]);
...
export const disable2FA = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const now = Math.floor(Date.now() / 1000);
    const result = await runDb(db, 'UPDATE users SET two_factor_secret = NULL, updated_at = ? WHERE id = ?', [
      now,
      userId,
    ]);
