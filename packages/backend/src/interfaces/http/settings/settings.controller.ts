...
import { exportConnectionsAsEncryptedZip } from '../../../infrastructure/backup/import-export.service';
import bcrypt from 'bcrypt';
import { getDb, getDbInstance } from '../../../infrastructure/database/connection';
import {
  BackupPasswordRequiredError,
  InvalidBackupPasswordError,
  createFullBackup,
  importFullBackup,
} from '../../../infrastructure/backup/full-backup.service';
import { UpdateSidebarConfigDto, UpdateCaptchaSettingsDto, CaptchaSettings } from '../../../shared/types/settings.types';
import { AppearanceSettings, UpdateAppearanceDto } from '../../../shared/types/appearance.types';
import {
  getAppearanceSettings,
  updateAppearanceSettings as updateAppearanceSettingsInRepo,
} from '../../../modules/appearance/appearance.repository';
...
export const settingsController = {
    ...
    try {
      ...
      if (!(
        ...
        Object.values(fullConfig.shortcuts).every(
          (sc: any) =>
            typeof sc === 'object' && sc !== null && (sc.shortcut === undefined || typeof sc.shortcut === 'string'),
        )
      )) {
  ...
  async exportAllConnections(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        error.message &&
        (error.message.includes('ENCRYPTION_KEY is not set') ||
          error.message.includes('Failed to decode ENCRYPTION_KEY') ||
          error.message.includes('Invalid ENCRYPTION_KEY length'))
      ) {
  ...
  async exportFullBackup(req: Request, res: Response): Promise<void> {
    try {
      ...
      const user = await getDb<{ hashed_password: string }>(db, 'SELECT hashed_password FROM users WHERE id = ?', [
        userId,
      ]);
