...
const TERMINAL_OUTPUT_BATCH_DELAY_MS = 4;
...
const reconcileStreamFlow = (state: ClientState): void => {
    ...
    const shouldPause = transport.networkBackpressureActive
        || transport.applicationBackpressureActive
        || state.terminalOutputHold === true;
...
export const flushTerminalOutput = (state: ClientState): void => {
    ...
    const payload = transport.chunks.length === 1
        ? transport.chunks[0]
        : Buffer.concat(transport.chunks, transport.bytes);
    transport.chunks = [];
    transport.bytes = 0;
