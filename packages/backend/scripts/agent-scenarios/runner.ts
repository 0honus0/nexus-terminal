...
import { SqliteCheckpointRepository } from '../../src/infrastructure/agent/repositories/sqlite-checkpoint.repository';
...
import { BrowserRuntimeAdapter } from '../../src/infrastructure/agent/integrations/browser-runtime.adapter';
...
import { SqliteWorkspaceRepository } from '../../src/infrastructure/agent/workspace-runtime/sqlite-workspace.repository';
import {
...
import { SshSuspendService } from '../../src/modules/ssh-suspend/ssh-suspend.service';
import { WorkspaceProtocolSession } from '../../src/interfaces/websocket/workspace-protocol.session';
...
import { createBrowserTools } from '../../src/modules/agent/tools/host/browser-tools';
import { hashOperation } from '../../src/modules/agent/operation-hash';
import {
  createWorkspaceCheckpointArchive,
  restoreWorkspaceCheckpointArchive,
} from '../../../agent-runner/src/controller/workspace-checkpoint-archive';
...
const browserTargetScopedRevisionScenario: Scenario = async () => {
  ...
  let session: import('../../src/modules/agent/ai/integrations.types').BrowserSessionView | null = null;
  let closeCount = 0;
  const gateway = {
    createSession: async (request: import('../../src/modules/agent/ai/integrations.types').BrowserSessionRequest) => {
      session = {
...
const browserInteractionPrimitivesScenario: Scenario = async () => {
  ...
  try {
    ...
    return [
      ...
      { name: 'browser_console_oversize_cursor_progress', value: tinyConsolePage.nextCursor === 4 ? 1 : 0, unit: 'cases' },
