<template>
  <div data-testid="quick-commands-view" class="quick-commands-root flex flex-col h-full overflow-hidden bg-background">
    ...
    <div class="flex flex-col flex-grow overflow-hidden bg-background">
      <!-- Controls Area -->
      <div class="quick-commands-controls flex items-center p-2 flex-shrink-0 gap-2 bg-background">
        ...
        <input
          ...
          ref="searchInputRef"
          class="quick-commands-search flex-grow min-w-0 px-4 py-1.5 border border-border/50 rounded-lg bg-input text-foreground text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition duration-150 ease-in-out"
        />
        <!-- Sort Button -->
        <button
          @click="toggleSortBy"
        ...
        <button
          @click="toggleCommandDisplayMode"
          class="quick-commands-control-button w-8 h-8 border border-border/50 rounded-lg text-text-secondary hover:bg-border hover:text-foreground transition-colors duration-150 flex-shrink-0 flex items-center justify-center"
        ...
        <button
          ...
          @click="openAddForm"
          class="quick-commands-control-button w-8 h-8 bg-primary text-white border-none rounded-lg text-sm font-semibold cursor-pointer shadow-md transition-colors duration-200 ease-in-out hover:bg-button-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary flex-shrink-0 flex items-center justify-center"
      ...
      <!-- List Area -->
      <div class="quick-commands-list-area flex-grow overflow-y-auto p-2">
        <!-- Loading State -->
        <div
        ...
        >
          ...
          <div v-if="showQuickCommandTagsBoolean">
            <div v-for="groupData in filteredAndGroupedCommands" :key="groupData.groupName" class="mb-1 last:mb-0">
              ...
              <div
                :data-testid="
                  groupData.tagId === null ? 'quick-command-group-untagged' : `quick-command-group-${groupData.tagId}`
                "
              ...
              >
                <i
                  :class="[
                    ...
                    'quick-command-group-chevron text-center text-text-secondary group-hover:text-foreground transition-transform duration-200 ease-in-out',
              ...
              <ul
                ...
                class="quick-command-group-list list-none p-0 m-0 pl-3"
              >
                <li
                  v-for="cmd in groupData.commands"
                  :key="cmd.id"
                  :data-command-id="cmd.id"
                  class="quick-command-row group flex justify-between items-center mb-1 cursor-pointer rounded-md hover:bg-primary/10 transition-colors duration-150"
                  :style="{
                ...
                >
                  ...
                  <div class="quick-command-info flex flex-col overflow-hidden flex-grow min-w-0">
                    <span
                      ...
                      :class="
                        commandDisplayMode === 'name' ? 'quick-command-display-name' : 'quick-command-display-code'
                      "
          ...
          <ul v-else class="list-none p-0 m-0">
            ...
            >
              ...
              <div class="quick-command-info flex flex-col overflow-hidden flex-grow min-w-0">
                <span
                  class="quick-command-display-text truncate"
                  :class="commandDisplayMode === 'name' ? 'quick-command-display-name' : 'quick-command-display-code'"
                  :title="getCommandDisplayText(cmd)"
                  >{{ getCommandDisplayText(cmd) }}</span
                >
              </div>
    ...
    <!-- 添加/编辑表单模态框 -->
    <AddEditQuickCommandForm v-if="isFormVisible" :command-to-edit="commandToEdit" @close="closeForm" />
...
<script setup lang="ts">
  ...
  const loadCommandDisplayMode = (): string | null => {
    try {
      return typeof window !== 'undefined' ? window.localStorage.getItem(QUICK_COMMAND_DISPLAY_MODE_KEY) : null;
    } catch {
      return null;
    }
  };
  ...
  const quickCommandScaleSaver = createLatestValueSaver<number>({
    ...
    onPendingChange: (pending) => {
      quickCommandScaleSyncLocked.value = pending;
    },
  ...
  const commandDisplayModeTitle = computed(() =>
    commandDisplayMode.value === 'name'
  ...
  const handleQuickCommandMenuAction = async (action: QuickCommandMenuAction, command: QuickCommandFE) => {
    ...
    if (action === 'sendToAllSessions') {
      const activeSshSessions = Array.from(sessionStore.sessions.values()).filter((s: SessionState) => {
...
<style scoped>
  ...
  .quick-command-list-small-type .quick-command-display-name {
    font-family:
      ui-rounded, 'SF Pro Rounded', 'Avenir Next Rounded', 'Segoe UI Rounded', 'Arial Rounded MT Bold',
  ...
  @container quick-commands-pane (max-width: 340px) {
    ...
    .quick-commands-control-button {
      width: 1.8rem !important;
      height: 1.8rem !important;
    }
    .quick-commands-control-button i {
