<template>
  <div data-testid="audit-log-view" class="p-4 bg-background text-foreground">
    <!-- Outer container with padding -->
    <div class="max-w-7xl mx-auto">
      <!-- Inner container for max-width (slightly wider for table) and centering -->
      <h1 class="text-xl font-semibold text-foreground mb-4 pb-2 border-b border-border">
        <!-- Title styling -->
        {{ $t('auditLog.title') }}
      </h1>
      ...
      <div v-else-if="!store.isLoading && !store.error && logs.length > 0">
        <!-- Pagination Controls -->
        <nav aria-label="Audit Log Pagination" v-if="totalPages > 1" class="mb-4 flex justify-center">
          <!-- Removed mt-6, added mb-4 -->
          <ul class="inline-flex items-center -space-x-px">
            <li>
              <a
                ...
                :class="[
                  'px-3 py-2 ml-0 leading-tight text-text-secondary bg-background border border-border rounded-l-lg hover:bg-header hover:text-foreground',
            ...
            <li v-for="page in paginationRange" :key="page">
              <a
                ...
                :class="[
                  'px-3 py-2 leading-tight border border-border',
                  page === currentPage
                    ? 'text-button-text bg-button border-button hover:bg-button-hover'
                    : 'text-text-secondary bg-background hover:bg-header hover:text-foreground',
                ]"
              >
                {{ page }}
              </a>
              <span v-else class="px-3 py-2 leading-tight text-text-secondary bg-background border border-border"
                >...</span
              >
            </li>
            <li>
              <a
                ...
                @click.prevent="changePage(currentPage + 1)"
                :class="[
                  'px-3 py-2 leading-tight text-text-secondary bg-background border border-border rounded-r-lg hover:bg-header hover:text-foreground',
        ...
        <div class="text-right text-text-secondary text-sm mb-4">
          <!-- Changed text-center to text-right, removed mt-3, added mb-4 -->
          {{ $t('auditLog.paginationInfo', { currentPage, totalPages, totalLogs }) }}
        </div>
        <div class="border border-border rounded-lg overflow-hidden shadow-sm bg-background">
          <!-- Removed mt-4 -->
            ...
            <table class="min-w-full divide-y divide-border text-sm">
              ...
              <thead class="bg-header">
                <tr>
                  <th
                    scope="col"
                    class="px-6 py-3 text-left font-medium text-text-secondary tracking-wider whitespace-nowrap"
                  >
                  ...
                  <th scope="col" class="px-6 py-3 text-left font-medium text-text-secondary tracking-wider">
