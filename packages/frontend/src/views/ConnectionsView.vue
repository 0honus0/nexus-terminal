<script setup lang="ts">
  ...
  const getTagNames = (tagIds: number[] | undefined): string[] => {
    ...
    return tagIds.map((id) => allTags.find((tag) => tag.id === id)?.name).filter((name): name is string => !!name);
  };
    ...
    <div class="max-w-screen-lg mx-auto">
      ...
      <div class="bg-card text-card-foreground shadow rounded-lg overflow-hidden border border-border min-h-[400px]">
        ...
        >
          ...
          >
            ...
            <button
              ...
              :disabled="
                isTestingAll || isLoadingConnections || !filteredAndSortedConnections.some((c) => c.type === 'SSH')
              "
            ...
            <button
              ...
              :disabled="
                isConnectingAll || isLoadingConnections || !filteredAndSortedConnections.some((c) => c.type === 'SSH')
              "
              class="h-8 px-3 py-1.5 text-sm bg-button text-button-text rounded-md shadow-sm hover:bg-button-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition duration-150 ease-in-out flex items-center justify-center flex-shrink-0 ml-2 sm:ml-0"
            >
        ...
        <div class="p-4">
            ...
            >
              ...
              <div class="flex items-center space-x-2 flex-shrink-0">
                ...
                >
                  ...
                  ></i>
                  <span v-if="getSingleTestButtonInfo(conn.id, conn.type).textKey !== 'connections.actions.testing'">{{
          ...
          <div
            v-else-if="!isLoadingConnections && searchQuery && filteredAndSortedConnections.length === 0"
            class="text-center text-text-secondary"
          >
            {{ t('dashboard.noConnectionsMatchSearch', '没有连接匹配搜索条件') }}
          </div>
          <div
            v-else-if="!isLoadingConnections && selectedTagId !== null && filteredAndSortedConnections.length === 0"
            class="text-center text-text-secondary"
          >
