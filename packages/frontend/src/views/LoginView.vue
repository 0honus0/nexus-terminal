...
const { isLoading, error, loginRequires2FA, publicCaptchaConfig, hasPasskeysAvailable } =
  storeToRefs(authStore); // Get publicCaptchaConfig and hasPasskeysAvailable
    ...
    >
      ...
      >
        ...
        <div class="z-10 text-center">
          ...
          <p class="text-base opacity-80">{{ t('slogan') }}</p>
          <!-- Example Slogan -->
        </div>
      </div>
      ...
      <div class="w-full md:w-3/5 flex flex-col justify-center p-8 sm:p-12">
        ...
        <h2 class="text-2xl font-semibold mb-6 text-center text-foreground">
          {{ t('login.title') }}
        </h2>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-5">
          ...
          <div v-if="!loginRequires2FA" class="space-y-6">
            ...
            <div class="flex items-center">
              ...
              />
              <label for="rememberMe" class="text-sm text-text-secondary cursor-pointer">{{
          ...
          >
            ...
            <div
              v-if="
                publicCaptchaConfig?.provider === 'hcaptcha' && publicCaptchaConfig.hcaptchaSiteKey
              "
            >
              <VueHcaptcha
                ref="hcaptchaWidget"
            ...
            <div
              v-else-if="
                publicCaptchaConfig?.provider === 'recaptcha' &&
                publicCaptchaConfig.recaptchaSiteKey
              "
            >
              <VueRecaptcha
                ref="recaptchaWidget"
          ...
          <!-- General Login Error -->
          <div v-if="error" class="text-error text-center text-sm -mt-2 mb-2">
            <!-- Adjusted margin -->
            {{ error }}
          </div>
