...
const { orderedTabs: globalEditorTabs, activeTabId: globalActiveEditorTabId } =
...
onMounted(() => {
  ...
  // +++ 订阅工作区事件 +++
  subscribeToWorkspaceEvents('terminal:sendCommand', (payload) =>
  ...
  subscribeToWorkspaceEvents('editor:activateTab', (payload) =>
    handleActivateEditorTab(payload.tabId)
  );
  ...
  subscribeToWorkspaceEvents('editor:closeOtherTabs', (payload) =>
    handleCloseOtherEditorTabs(payload.tabId)
  );
  subscribeToWorkspaceEvents('editor:closeTabsToRight', (payload) =>
    handleCloseEditorTabsToRight(payload.tabId)
  );
  subscribeToWorkspaceEvents('editor:closeTabsToLeft', (payload) =>
  ...
  subscribeToWorkspaceEvents('connection:openNewSession', (payload) =>
    handleOpenNewSession(payload.connectionId)
  );
  subscribeToWorkspaceEvents('connection:requestAdd', handleRequestAddConnection);
  subscribeToWorkspaceEvents('connection:requestEdit', (payload) =>
  ...
  subscribeToWorkspaceEvents('session:activate', (payload) =>
    sessionStore.activateSession(payload.sessionId)
  );
  subscribeToWorkspaceEvents('session:close', (payload) =>
    sessionStore.closeSession(payload.sessionId)
  );
  subscribeToWorkspaceEvents('session:closeOthers', (payload) =>
    handleCloseOtherSessions(payload.targetSessionId)
  );
  subscribeToWorkspaceEvents('session:closeToRight', (payload) =>
    handleCloseSessionsToRight(payload.targetSessionId)
  );
  subscribeToWorkspaceEvents('session:closeToLeft', (payload) =>
...
onBeforeUnmount(() => {
  ...
  // +++ 取消订阅工作区事件 +++
  unsubscribeFromWorkspaceEvents('terminal:sendCommand', (payload) =>
    handleSendCommand(payload.command, payload.sessionId)
  );
  ...
  unsubscribeFromWorkspaceEvents('editor:closeTab', (payload) =>
    handleCloseEditorTab(payload.tabId)
  );
  unsubscribeFromWorkspaceEvents('editor:activateTab', (payload) =>
    handleActivateEditorTab(payload.tabId)
  );
  ...
  unsubscribeFromWorkspaceEvents('editor:closeOtherTabs', (payload) =>
    handleCloseOtherEditorTabs(payload.tabId)
  );
  unsubscribeFromWorkspaceEvents('editor:closeTabsToRight', (payload) =>
    handleCloseEditorTabsToRight(payload.tabId)
  );
  unsubscribeFromWorkspaceEvents('editor:closeTabsToLeft', (payload) =>
    handleCloseEditorTabsToLeft(payload.tabId)
  );
  ...
  unsubscribeFromWorkspaceEvents('connection:openNewSession', (payload) =>
    handleOpenNewSession(payload.connectionId)
  );
  unsubscribeFromWorkspaceEvents('connection:requestAdd', handleRequestAddConnection);
  unsubscribeFromWorkspaceEvents('connection:requestEdit', (payload) =>
    handleRequestEditConnection(payload.connectionInfo)
  );
  ...
  unsubscribeFromWorkspaceEvents('session:activate', (payload) =>
    sessionStore.activateSession(payload.sessionId)
  );
  unsubscribeFromWorkspaceEvents('session:close', (payload) =>
    sessionStore.closeSession(payload.sessionId)
  );
  unsubscribeFromWorkspaceEvents('session:closeOthers', (payload) =>
    handleCloseOtherSessions(payload.targetSessionId)
  );
  unsubscribeFromWorkspaceEvents('session:closeToRight', (payload) =>
    handleCloseSessionsToRight(payload.targetSessionId)
  );
  unsubscribeFromWorkspaceEvents('session:closeToLeft', (payload) =>
    handleCloseSessionsToLeft(payload.targetSessionId)
  );
...
const handleSendCommand = (command: string, targetSessionId?: string) => {
  ...
  if (
    ...
    if (connectionInfo) {
      ...
      console.error(
        `[WorkspaceView] handleSendCommand: 未找到 ID 为 ${sessionToCommand.connectionId} 的连接信息。`
      );
    }
    return;
  }
  ...
  if (terminalManager && typeof terminalManager.sendData === 'function') {
    ...
    console.warn(
      `[WorkspaceView] Cannot send command for session ${sessionToCommand.sessionId}, terminal manager or sendData method not available.`
    );
  }
};
...
const handleTerminalInput = (payload: { sessionId: string; data: string }) => {
  ...
  if (!session || !manager) {
    console.warn(
      `[WorkspaceView] handleTerminalInput: 未找到会话 ${sessionId} 或其 terminalManager`
    );
    return;
  }
  if (data === '\r' && manager.isSshConnected && !manager.isSshConnected.value) {
    ...
    if (connectionInfo) {
      ...
      console.error(
        `[WorkspaceView] handleTerminalInput: 未找到 ID 为 ${session.connectionId} 的连接信息。`
      );
    }
  } else {
...
const handleFindNext = () => {
  ...
  if (manager && currentSearchTerm.value) {
    ...
    const mode = isMobile.value ? 'Mobile' : 'Desktop';
    console.warn(
      `[WorkspaceView ${mode}] Cannot findNext, no active session manager or search term.`
    );
  }
};
...
const handleFindPrevious = () => {
  ...
  if (manager && currentSearchTerm.value) {
    ...
    const mode = isMobile.value ? 'Mobile' : 'Desktop';
    console.warn(
      `[WorkspaceView ${mode}] Cannot findPrevious, no active session manager or search term.`
    );
  }
};
...
const handleClearTerminal = () => {
  ...
  if (
    ...
    console.warn(
      `[WorkspaceView ${mode}] Cannot clear terminal for session ${currentSession.sessionId}, terminal manager, instance, or clear method not available.`
    );
  }
};
...
const handleScrollToBottomRequest = (payload: { sessionId: string }) => {
  ...
  if (terminalManager?.terminalInstance?.value) {
    ...
    console.warn(
      `[WorkspaceView] Cannot scroll to bottom for session ${payload.sessionId}, terminal instance not found.`
    );
  }
};
...
const handleCloseEditorTab = (tabId: string) => {
  ...
  if (isShared) {
    ...
    if (currentActiveSessionId) {
      ...
      console.warn(
        '[WorkspaceView] Cannot close editor tab: No active session in independent mode.'
      );
    }
  }
};
...
const handleActivateEditorTab = (tabId: string) => {
  ...
  if (isShared) {
    ...
    if (currentActiveSessionId) {
      ...
      console.warn(
        '[WorkspaceView] Cannot activate editor tab: No active session in independent mode.'
      );
    }
  }
};
...
const handleUpdateEditorContent = (payload: { tabId: string; content: string }) => {
  ...
  if (isShared) {
    ...
    if (currentActiveSessionId) {
      ...
      console.warn(
        '[WorkspaceView] Cannot update editor content: No active session in independent mode.'
      );
    }
  }
};
...
const handleSaveEditorTab = (tabId: string) => {
  ...
  if (isShared) {
    ...
    if (currentActiveSessionId) {
      ...
      console.warn(
        '[WorkspaceView] Cannot save editor tab: No active session in independent mode.'
      );
    }
  }
};
...
const handleChangeEncoding = (payload: { tabId: string; encoding: string }) => {
  ...
  if (isShared) {
    ...
    if (currentActiveSessionId) {
      ...
      console.warn(
        '[WorkspaceView] Cannot change editor encoding: No active session in independent mode.'
      );
    }
  }
};
...
const handleEditorScrollPositionUpdate = (payload: {
  ...
  if (shareFileEditorTabsBoolean.value) {
    ...
    if (currentActiveSession) {
      ...
      console.warn(
        '[WorkspaceView] Cannot update editor scroll position: No active session in independent mode for tab:',
        tabId
      );
    }
  }
...
const handleVirtualKeyPress = (keySequence: string) => {
  ...
  if (terminalManager && typeof terminalManager.sendData === 'function') {
    ...
    console.warn(
      `[WorkspaceView Mobile] Cannot send virtual key for session ${currentSession.sessionId}, terminal manager or sendData method not available.`
    );
  }
};
...
const handleCloseOtherEditorTabs = (targetTabId: string) => {
  const tabsToClose = editorTabs.value.filter((tab) => tab.id !== targetTabId).map((tab) => tab.id);
...
const handleFileManagerOpenRequest = (payload: { sessionId: string }) => {
  ...
  if (!wsDeps) {
    ...
    console.error(
      `[WorkspaceView] Cannot open file manager: wsDeps are null after attempting retrieval for session ${sessionId}.`
    );
    return;
  }
...
const handleQuickCommandExecuteProcessed = (
        ...
        >
          <h2 class="text-lg font-semibold text-foreground">
            {{ t('fileManager.modalTitle', '文件管理器') }} ({{
              currentFileManagerSessionId
                ? sessionStore.sessions.get(currentFileManagerSessionId)?.connectionName ||
                  currentFileManagerSessionId
                : '未知会话'
            }})
          </h2>
