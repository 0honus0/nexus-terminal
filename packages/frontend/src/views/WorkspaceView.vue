<script setup lang="ts">
  ...
  const preloadPlainTextEditor = () => {
    const preloadTasks: Promise<unknown>[] = [
      ...
      isMobile.value ? import('../components/CodeMirrorMobileEditor.vue') : import('../components/MonacoEditor.vue'),
    ];
    ...
    void Promise.allSettled(preloadTasks).then((results) => {
      ...
      if (failed.length > 0) {
        console.warn(
          `[WorkspaceView] 编辑器核心预加载有 ${failed.length} 个模块失败，将在实际打开文件时再次尝试。`,
          failed,
        );
      }
    });
  ...
  const handleSendCommand = (command: string, targetSessionId?: string) => {
    ...
    if (terminalManager && !terminalManager.isSshConnected.value && !terminalManager.hasSshConnectedOnce.value) {
      console.log(
        `[WorkspaceView] Ignoring command while initial SSH connection is still pending for session ${sessionToCommand.sessionId}.`,
      );
      return;
    }
  ...
  const handleTerminalInput = (payload: { sessionId: string; data: string }) => {
    ...
    if (
      isMobile.value &&
      resolvedSessionId === activeSessionId.value &&
      (isVirtualCtrlActive.value || isVirtualAltActive.value)
    ) {
    ...
    if (!session || !manager) {
      console.warn(
        `[WorkspaceView] handleTerminalInput: 未找到会话 ${payload.sessionId}（解析为 ${resolvedSessionId}）或其 terminalManager`,
      );
      return;
    }
...
<template>
  ...
  <div :class="['workspace-view', { 'with-header': isHeaderVisible, 'is-mobile': isMobile }]">
    ...
    <TerminalTabBar :sessions="sessionTabsWithStatus" :active-session-id="activeSessionId" :is-mobile="isMobile" />
    ...
    <LayoutConfigurator :is-visible="showLayoutConfigurator" @close="handleCloseLayoutConfigurator" />
