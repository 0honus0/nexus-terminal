          ...
          <div class="flex justify-between items-center">
            <div class="session-info flex-grow mr-2">
                ...
                <span
                  ...
                  :title="
                    session.backendSshStatus === 'marked_active'
                      ? undefined
                      : $t('suspendedSshSessions.tooltip.editName')
                  "
              ...
              <div class="text-xs text-muted-color mt-1">
                {{
                  session.backendSshStatus === 'marked_active'
                    ? $t('suspendedSshSessions.label.markedAt')
...
<script setup lang="ts">
  ...
  const resumeSession = async (session: SuspendedSshSession) => {
    ...
    try {
      ...
      console.log(
        '[SuspendedSshSessionsView] Resume request dispatched; completion is handled by the WebSocket notification.',
      );
    } catch (error) {
    ...
    fetchTimeoutId = window.setTimeout(async () => {
      ...
      pollIntervalMs =
        result.status === 429
          ...
          : result.ok
            ? 3000
            : Math.min(Math.max(pollIntervalMs, 10000), 60000);
      schedulePoll();
    }, pollIntervalMs);
  };
