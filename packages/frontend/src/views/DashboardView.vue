<script setup lang="ts">
  ...
  const filteredAndSortedConnections = computed(() => {
    ...
    const filtered = connections.value.filter((connection) => {
      ...
      return [connection.name, connection.username, connection.host, String(connection.port)].some((value) =>
  ...
  const formatRelativeTime = (timestampInSeconds: number | null | undefined): string => {
    ...
    try {
      ...
      const selectedLocale = dateFnsLocales[currentLocale] ?? dateFnsLocales[currentLocale.split('-')[0]] ?? enUS;
  ...
  watch([dashboardShowLocalResourcesBoolean, statusMonitorIntervalSecondsNumber], syncLocalSystemPolling, {
    immediate: true,
  });
  watch([dashboardShowRemoteResourcesBoolean, remoteHostRefreshIntervalSecondsNumber], syncRemoteResourcePolling, {
    immediate: true,
  });
    ...
    <div class="mx-auto w-full max-w-[1680px] space-y-5">
        ...
        <div class="grid gap-4 px-1 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          ...
          >
            ...
            >
              <div>
                <strong
                  data-testid="dashboard-total-connections"
                  class="block text-xl font-semibold leading-none tabular-nums"
                  >{{ connections.length }}</strong
                >
                <div class="mt-1.5 text-[10px] text-text-alt">{{ t('dashboard.totalConnections', '连接总数') }}</div>
              </div>
              <div>
                <strong
                  data-testid="dashboard-tag-count"
                  class="block text-xl font-semibold leading-none tabular-nums"
                  >{{ tags.length }}</strong
                >
                ...
                <div>
                  <div class="flex items-baseline justify-between gap-2">
                    ...
                    <strong class="text-sm font-semibold tabular-nums"
                      >{{ resourcePercent(localSystemStatus.cpuPercent) }}%</strong
                    >
                  </div>
                  <div class="mt-1.5 h-0.5 overflow-hidden rounded-full bg-border/80">
                    <div
                      data-testid="dashboard-local-cpu-bar"
                      class="h-full rounded-full bg-primary"
                      :style="{ width: `${resourcePercent(localSystemStatus.cpuPercent)}%` }"
                    ></div>
                  </div>
                </div>
                <div
                  :title="`${formatMemory(localSystemStatus.memUsed)} / ${formatMemory(localSystemStatus.memTotal)}`"
                >
                  <div class="flex items-baseline justify-between gap-2">
                    <span class="text-[9px] font-medium text-text-alt">{{
                      t('dashboard.resources.memory', '内存')
                    }}</span>
                    <strong class="text-sm font-semibold tabular-nums"
                      >{{ resourcePercent(localSystemStatus.memPercent) }}%</strong
                    >
                  </div>
                  <div class="mt-1.5 h-0.5 overflow-hidden rounded-full bg-border/80">
                    <div
                      data-testid="dashboard-local-memory-bar"
                      class="h-full rounded-full bg-success"
                      :style="{ width: `${resourcePercent(localSystemStatus.memPercent)}%` }"
                    ></div>
                  </div>
                </div>
                <div>
                  <div class="flex items-baseline justify-between gap-2">
                    <span class="text-[9px] font-medium text-text-alt">{{
                      t('dashboard.resources.disk', '根磁盘')
                    }}</span>
                    <strong class="text-sm font-semibold tabular-nums">{{
                      localSystemStatus.diskPercent === undefined
                        ? '—'
                        : `${resourcePercent(localSystemStatus.diskPercent)}%`
                    }}</strong>
                  </div>
                  <div class="mt-1.5 h-0.5 overflow-hidden rounded-full bg-border/80">
                    <div
                      data-testid="dashboard-local-disk-bar"
                      class="h-full rounded-full bg-warning"
                      :style="{ width: `${resourcePercent(localSystemStatus.diskPercent)}%` }"
                    ></div>
                  </div>
                </div>
              </div>
      ...
      >
        <div class="order-1 min-w-0 xl:pr-7">
          <section data-testid="dashboard-connections" class="min-w-0">
            <div class="pb-4">
              <div class="flex items-center justify-between gap-3">
            ...
            >
              ...
              >
                <label class="relative col-span-3 min-w-0 sm:col-span-1">
                  ...
                  <i
                    class="fas fa-search pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-xs text-text-alt"
                    aria-hidden="true"
                  ></i>
                  <input
                    v-model="searchQuery"
                ...
                >
                  <i
                    :class="['fas', isAscending ? 'fa-arrow-up-a-z' : 'fa-arrow-down-z-a', 'text-xs']"
                    aria-hidden="true"
                  ></i>
                </button>
              </div>
                  ...
                  >
                    <div class="min-w-0">
                      <div class="flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">
                        ...
                        <span
                          class="rounded border border-border bg-muted/50 px-1.5 py-0.5 text-[10px] font-medium tracking-wide text-text-alt"
                        >
                          {{ conn.type }}
                        </span>
                      </div>
            ...
            <div class="flex min-w-0 items-center gap-2.5">
              ...
              <div class="min-w-0">
                ...
                <p class="mt-0.5 truncate text-xs text-text-secondary">
                  {{ t('dashboard.resources.sshHint', '已配置 SSH 主机的低频资源快照') }}
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div class="flex min-w-0 items-start justify-between gap-3 pl-1">
                <div class="min-w-0">
                  ...
                  <p
                    class="mt-1 pl-3.5 truncate font-mono text-xs text-text-secondary"
                ...
                <span
                  class="rounded bg-muted/60 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-text-secondary"
                  >SSH</span
                >
              </div>
              ...
              <div v-if="remote.status" class="mt-3 grid grid-cols-3 gap-2 sm:gap-4">
                <div>
                  <div class="flex items-baseline justify-between gap-2">
                    ...
                    <strong class="text-base font-semibold tabular-nums"
                      >{{ resourcePercent(remote.status.cpuPercent) }}%</strong
                    >
                  </div>
                  <div class="mt-2 h-0.5 overflow-hidden rounded-full bg-border/80">
                    <div
                      ...
                      class="h-full rounded-full bg-primary"
                      :style="{ width: `${resourcePercent(remote.status.cpuPercent)}%` }"
                    ></div>
                  </div>
                </div>
                <div :title="`${formatMemory(remote.status.memUsed)} / ${formatMemory(remote.status.memTotal)}`">
                  <div class="flex items-baseline justify-between gap-2">
                    <span class="text-[10px] font-medium text-text-alt">{{
                      t('dashboard.resources.memory', '内存')
                    }}</span>
                    <strong class="text-base font-semibold tabular-nums"
                      >{{ resourcePercent(remote.status.memPercent) }}%</strong
                    >
                  </div>
                  <div class="mt-2 h-0.5 overflow-hidden rounded-full bg-border/80">
                    <div
                      ...
                      class="h-full rounded-full bg-success"
                      :style="{ width: `${resourcePercent(remote.status.memPercent)}%` }"
                    ></div>
                  </div>
                  <div class="mt-1 truncate text-[9px] tabular-nums text-text-alt">
                ...
                <div>
                  <div class="flex items-baseline justify-between gap-2">
                    ...
                    <strong class="text-base font-semibold tabular-nums">{{
                      remote.status.diskPercent === undefined ? '—' : `${resourcePercent(remote.status.diskPercent)}%`
                    }}</strong>
                  </div>
                  <div class="mt-2 h-0.5 overflow-hidden rounded-full bg-border/80">
                    <div
                      ...
                      class="h-full rounded-full bg-warning"
                      :style="{ width: `${resourcePercent(remote.status.diskPercent)}%` }"
                    ></div>
                  </div>
                </div>
              </div>
            ...
            >
              {{
                remoteResourcesLoading
                  ? t('common.loading')
                  : t('dashboard.resources.noRemoteSessions', '当前没有已配置 SSH 主机')
              }}
            </div>
            <div
              v-else-if="dashboardShowRemoteResourcesBoolean"
              data-testid="dashboard-remote-resources"
              class="sr-only"
            >
      ...
      <aside data-testid="dashboard-recent-activity" class="border-t border-border/70 pt-5">
        <header class="flex flex-col gap-3 pb-3 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex min-w-0 items-center gap-2.5">
            ...
            <div class="min-w-0">
              ...
              <p class="mt-0.5 truncate text-xs text-text-secondary">
                {{ t('dashboard.recentActivityHint', '最近的审计事件') }}
              </p>
            </div>
          </div>
        ...
        <div
          v-if="isLoadingLogs && recentAuditLogs.length === 0"
