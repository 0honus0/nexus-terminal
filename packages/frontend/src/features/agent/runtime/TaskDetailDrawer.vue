...
<template>
  ...
  <aside
    v-if="visible"
    class="absolute inset-y-0 right-0 z-20 flex w-[min(540px,94%)] flex-col border-l border-border/60 bg-background shadow-2xl"
  ...
  >
    <header class="flex h-14 shrink-0 items-center justify-between border-b border-border/60 px-4">
      <div class="flex min-w-0 items-center gap-2.5">
        ...
        <div class="min-w-0">
          ...
          <span v-if="snapshot" class="mt-1 block truncate font-mono text-[10px] text-text-secondary">{{
            snapshot.id
          }}</span>
        </div>
      </div>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto p-4 text-[13px]">
      <template v-if="snapshot">
        <section class="rounded-2xl border border-border/60 bg-card/55 p-4 shadow-sm">
          <div class="flex items-start justify-between gap-3">
            ...
            <span class="rounded-full bg-header px-2.5 py-1 text-[10px] text-text-secondary">
          ...
          <div class="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 border-t border-border/60 pt-3 sm:grid-cols-4">
            <div class="min-w-0">
              <div class="text-[10px] text-text-secondary">{{ $t('agent.tasks.goal') }}</div>
              <div class="mt-1 truncate text-[11px] font-medium">{{ snapshot.goalStatus }}</div>
            </div>
            <div class="min-w-0">
              <div class="text-[10px] text-text-secondary">{{ $t('agent.tasks.steps') }}</div>
              <div class="mt-1 text-[11px] font-medium">{{ snapshot.usage.steps }}</div>
            </div>
            <div class="min-w-0">
              <div class="text-[10px] text-text-secondary">{{ $t('agent.tasks.tokens') }}</div>
              <div class="mt-1 text-[11px] font-medium">
                {{ snapshot.usage.inputTokens + snapshot.usage.outputTokens }}
              </div>
            </div>
            <div class="min-w-0">
              <div class="text-[10px] text-text-secondary">{{ $t('agent.tasks.activeTime') }}</div>
              <div class="mt-1 text-[11px] font-medium">{{ snapshot.activeExecutionSeconds }}s</div>
            </div>
          </div>
          <div
          ...
          >
            <p class="max-w-72 text-[10px] leading-4 text-text-secondary">{{ $t('agent.tasks.deleteRunHint') }}</p>
            <div class="flex shrink-0 items-center gap-1.5">
              <button
                v-if="deleteArmed"
                type="button"
                class="rounded-lg border border-border/70 px-2.5 py-1.5 text-[10px] font-medium text-text-secondary hover:bg-header"
                :disabled="busy"
              ...
              <button
                type="button"
                class="rounded-lg border border-error/40 px-2.5 py-1.5 text-[10px] font-semibold text-error hover:bg-error/10 disabled:opacity-50"
                :disabled="busy"
        ...
        <section class="mt-4 rounded-2xl border border-border/60 bg-card/55 p-4">
          <div class="flex items-center justify-between gap-2">
            <div>
              <h3 class="flex items-center gap-1.5 font-medium">
                <i class="fa-solid fa-bookmark text-[10px] text-text-secondary" aria-hidden="true"></i>
                {{ $t('agent.tasks.checkpoints') }}
              </h3>
              <p class="mt-1 text-[10px] leading-4 text-text-secondary">{{ $t('agent.tasks.checkpointHint') }}</p>
            </div>
            <button
              type="button"
          ...
          <div
            ...
            class="mt-3 rounded-xl bg-background/70 px-3 py-3 text-[10px] text-text-secondary"
          >
          ...
          <article v-for="checkpoint in checkpoints" :key="checkpoint.id" class="mt-2 rounded-xl bg-background/70 p-3">
            <div class="flex items-center justify-between gap-2">
              ...
              <button
                v-if="terminal.has(snapshot.status)"
                type="button"
                class="shrink-0 rounded-lg bg-primary px-2.5 py-1.5 text-[10px] font-semibold text-white disabled:opacity-50"
        ...
        <details class="mt-4 rounded-2xl border border-border/60 bg-card/55 p-4">
          ...
          <article v-for="entry in snapshot.recentEntries" :key="entry.id" class="mt-2 rounded-xl bg-background/70 p-3">
            ...
            <pre class="mt-1 max-h-44 overflow-auto whitespace-pre-wrap break-words text-[10px] leading-4">{{
