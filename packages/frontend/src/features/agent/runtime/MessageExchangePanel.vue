...
<template>
  <section class="rounded border border-border p-3">
    ...
    <p v-if="messages.length === 0" class="mt-2 text-[11px] text-text-secondary">
    ...
    <div v-else class="mt-2 space-y-2">
      <article v-for="message in messages" :key="message.id" class="rounded bg-background p-2">
        ...
        <pre class="mt-1 max-h-40 overflow-auto whitespace-pre-wrap break-words text-[11px]">{{
          JSON.stringify(message.body, null, 2)
        }}</pre>
        <div v-if="message.artifactRefs.length" class="mt-1 text-[11px] text-text-secondary">
