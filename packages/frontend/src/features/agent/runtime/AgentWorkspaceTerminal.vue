<script setup lang="ts">
  ...
  import { formatWorkspaceTerminalError } from './workspace-terminal-errors';
  ...
  const handleTerminalError = (code: string): void => {
    error.value = formatWorkspaceTerminalError(t, code);
    notice.value = '';
  };
  const handleTerminalClosed = (): void => {
    channel.value = null;
    opened.value = false;
    if (!error.value) notice.value = t('agent.workspaceRuntime.terminalClosed');
  };
...
<template>
  <div class="mt-2 rounded border border-border bg-background p-2">
    ...
    <p v-else-if="notice" class="mt-1 text-[11px] text-text-secondary">{{ notice }}</p>
