<template>
  <OverlayPanel :visible="visible" panel-class="max-w-2xl max-h-[90vh] flex flex-col p-6" @close="handleCancel">
    <!-- Header -->
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-col sm:flex-row gap-4">
          ...
          <div class="form-group sm:w-48">
            <label for="transferMethod" class="block text-sm font-medium text-text-secondary mb-1">{{
      ...
      <div class="border border-border rounded-md p-4 space-y-4 max-h-72 overflow-y-auto bg-header/30">
        ...
        <div v-else class="space-y-3">
          <div v-for="group in filteredGroupedConnections" :key="getGroupId(group)" class="tag-group">
            <div class="flex items-center py-1.5 cursor-pointer group" @click="toggleTagGroupExpansion(group)">
              <input
                type="checkbox"
              ...
              <i
                :class="[
                  'fas',
                  (expandedTagGroups[getGroupId(group)] ?? true) ? 'fa-chevron-down' : 'fa-chevron-right',
