<script setup lang="ts">
  ...
  const filteredAndGroupedConnections = computed(() => {
    ...
    // 修改：结果包含 tagId
    const result: { groupName: string; connections: ConnectionInfo[]; tagId: number | null }[] = sortedGroupNames.map(
      (name) => ({
        groupName: name,
...
<template>
  <div
    ...
    class="h-full flex flex-col overflow-hidden bg-background text-foreground"
  >
              ...
              <ul v-show="expandedGroups[groupData.groupName]" class="list-none p-0 m-0 pl-3">
                ...
                >
                  <i
                    :class="[
                      'fas',
                      conn.type === 'RDP' ? 'fa-desktop' : conn.type === 'VNC' ? 'fa-plug' : 'fa-server',
          ...
          <ul v-else class="list-none p-0 m-0">
            ...
            >
              <i
                :class="[
                  'fas',
                  conn.type === 'RDP' ? 'fa-desktop' : conn.type === 'VNC' ? 'fa-chalkboard' : 'fa-server',
                  'mr-2.5 w-4 text-center text-text-secondary group-hover:text-primary',
                  { 'text-white': conn.id === highlightedConnectionId },
                ]"
              ></i>
    ...
    <teleport to="body">
      ...
      >
        <ul class="list-none p-0 m-0">
          <li
            v-if="
              contextTargetTagGroup && contextTargetTagGroup.connections.some((c: ConnectionInfo) => c.type === 'SSH')
            "
            class="group px-4 py-1.5 cursor-pointer flex items-center text-foreground hover:bg-primary/10 hover:text-primary text-sm transition-colors duration-150 rounded-md mx-1"
