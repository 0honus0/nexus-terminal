  ...
  if (confirmed) {
    ...
    if (!success) {
      showAlertDialog({
        ...
        message: t('connections.errors.deleteFailed', {
          error: connectionsStore.error || '未知错误',
        }),
      });
    }
  }
...
const handleTestConnection = async (connectionId: number) => {
  ...
  if (result.success) {
    ...
    showAlertDialog({
      title: t('common.error'),
      message: t('connections.test.failed', { error: result.message || '未知错误' }),
    });
  }
};
</script>
...
<template>
  <div class="mt-2 font-sans">
    ...
    <!-- 遍历分组 -->
    <div
      v-for="(groupConnections, groupName) in groupedConnections"
      :key="groupName"
      class="mb-6 border border-border rounded-md overflow-hidden bg-background"
    >
      <h4
        class="m-0 px-4 py-2 text-base font-semibold bg-header text-foreground border-b border-border"
      >
      ...
      <div class="overflow-x-auto">
        <table v-if="groupConnections.length > 0" class="w-full">
          <thead class="bg-header">
            <tr>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
              ...
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
                {{ t('connections.table.host') }}
              </th>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
                {{ t('connections.table.port') }}
              </th>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
                {{ t('connections.table.user') }}
              </th>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
                {{ t('connections.table.authMethod') }}
              </th>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
                {{ t('connections.table.tags') }}
              </th>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
                {{ t('connections.table.lastConnected') }}
              </th>
              <th
                class="px-4 py-2 text-left text-xs font-medium text-text-secondary uppercase tracking-wider border-b-2 border-border"
              >
          ...
          <tbody class="divide-y divide-border">
            ...
            >
              ...
              <td class="px-4 py-3 text-sm text-foreground">
                <div v-if="getConnectionTagNames(conn).length > 0" class="flex flex-wrap gap-1">
                  <span
                    ...
                    :key="tagName"
                    class="px-2 py-0.5 text-xs rounded bg-background-alt border border-border text-text-secondary"
                  >
                    {{ tagName }}
                  </span>
                </div>
              ...
              <td class="px-4 py-3 text-sm text-foreground whitespace-nowrap">
                {{ formatTimestamp(conn.last_connected_at) }}
              </td>
              <td class="px-4 py-3 text-sm text-foreground whitespace-nowrap">
                <button
                ...
                >
                  {{
                    testingState[conn.id]
                      ? t('connections.actions.testing')
