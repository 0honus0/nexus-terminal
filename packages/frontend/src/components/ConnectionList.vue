    ...
    if (confirmed) {
      ...
      if (!success) {
        showAlertDialog({
          ...
          message: t('connections.errors.deleteFailed', { error: connectionsStore.error || '未知错误' }),
        });
      }
    }
  };
          ...
          <tbody class="divide-y divide-border">
            <tr v-for="conn in groupConnections" :key="conn.id" class="hover:bg-hover transition-colors duration-150">
              ...
              <td class="px-4 py-3 text-sm text-foreground whitespace-nowrap">
                ...
                >
                  {{ testingState[conn.id] ? t('connections.actions.testing') : t('connections.actions.test') }}
                </button>
                <button
