<script setup lang="ts">
  ...
  const suggestions = computed(() => {
    ...
    if (!inputValue.value) {
      ...
      result = currentAvailableTags.filter(
        (tag) => tag.name.toLowerCase().includes(lowerCaseInput) && !selectedTagIds.value.includes(tag.id), // 排除已选中的
      );
    }
    return result;
  });
  ...
  const handleFocus = () => {
    ...
    if (!currentInput) {
      ...
      potentialSuggestions = currentAvailableTags.filter((tag) => tag.name.toLowerCase().includes(lowerCaseInput));
    }
  ...
  const handleKeyDown = async (event: KeyboardEvent) => {
    if (event.key === 'Enter' && inputValue.value.trim()) {
      ...
      } else if (!existingTag && allowCreate.value) {
        // Only create if allowed and not existing
...
<template>
  <div class="relative w-full">
    <div
      class="flex flex-wrap items-center gap-1 p-1.5 border border-border rounded cursor-text bg-background"
    ...
    >
      <div class="inline-flex flex-wrap gap-1">
        <span
          ...
          :data-tag-id="tag.id"
          class="inline-flex items-center bg-background-alt text-foreground text-sm px-2 py-0.5 rounded whitespace-nowrap border border-border"
        >
          {{ tag.name }}
          <button
            type="button"
    ...
    <ul
      ...
      class="absolute top-full left-0 right-0 mt-0.5 bg-background border border-border rounded-b shadow-md list-none p-0 m-0 max-h-[150px] overflow-y-auto z-10"
    >
      <li
