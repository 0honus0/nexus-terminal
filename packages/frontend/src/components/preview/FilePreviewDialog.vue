<script setup lang="ts">
  ...
  const isRefreshing = computed(() =>
    Boolean(tabsContext && activeTabId.value && tabsContext.refreshingTabIds.value.has(activeTabId.value)),
  );
  const hide = () => {
    if (tabsContext) {
      tabsContext.hide();
  ...
  const scrollActiveTabIntoView = () => {
    ...
    const activeTab = Array.from(dialogRef.value.querySelectorAll<HTMLElement>('[data-preview-tab-id]')).find(
  ...
  onMounted(() => {
    if (!tabsContext) {
      standalonePreviouslyFocusedElement =
...
<template>
  <Teleport to="body">
    ...
    >
      <section
        class="file-preview-panel relative flex h-full w-full max-w-[1400px] flex-col overflow-hidden rounded-lg border border-border bg-background text-foreground shadow-2xl"
      >
        <div
        ...
        >
          <div class="min-w-0 flex-1">
            ...
            <div v-if="props.subtitle" class="hidden truncate text-xs text-text-secondary sm:block">
              {{ props.subtitle }}
            </div>
          </div>
          <slot name="toolbar" />
