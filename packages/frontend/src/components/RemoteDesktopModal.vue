<script setup lang="ts">
  ...
  const readRemoteTouchMode = (): RemoteTouchMode => {
    try {
      return window.localStorage.getItem(REMOTE_TOUCH_MODE_STORAGE_KEY) === 'touchpad' ? 'touchpad' : 'direct';
    } catch {
      return 'direct';
    }
  };
  ...
  const bindRemoteTouchInput = (displayEl: HTMLElement) => {
    ...
    remoteTouchInput = attachRemoteTouchInput(displayEl, guacClient.value, remoteTouchMode.value, {
  ...
  const { didDrag: restoreButtonDragged, startDragging: startRestoreButtonDrag } = useDraggablePosition({
  ...
  >
    ...
    <div
      ...
      :class="[
        ...
        isBrowserFullscreen
          ? 'rdp-panel-fullscreen rounded-none border-0 shadow-none bg-black'
          : 'rounded-lg border border-border shadow-xl',
      ]"
    >
      <div
        v-show="!isBrowserFullscreen"
        data-testid="rdp-window-header"
        class="flex items-center justify-between p-3 border-b border-border flex-shrink-0"
      >
        <h3 class="text-base font-semibold truncate">
          <i class="fas fa-desktop mr-2 text-text-secondary"></i>
          {{ t('remoteDesktopModal.title') }} -
          {{ props.connection?.name || props.connection?.host || t('remoteDesktopModal.titlePlaceholder') }}
        </h3>
      ...
      <div ref="rdpContainerRef" class="relative bg-black overflow-hidden flex-1">
        <div ref="rdpDisplayRef" data-testid="rdp-display-container" class="rdp-display-container w-full h-full"></div>
        <div
      ...
      <div
        v-show="!isBrowserFullscreen"
        data-testid="rdp-window-footer"
        class="p-2 border-t border-border flex-shrink-0 text-xs text-text-secondary bg-header flex items-center justify-between gap-2 flex-wrap"
      >
