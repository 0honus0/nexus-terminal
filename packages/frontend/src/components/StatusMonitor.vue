        ...
        <span class="auto-summary" aria-hidden="true">
          <span class="summary-row summary-resources">
            <span class="summary-metric summary-cpu"
              ><b>CPU</b><span class="summary-percent">{{ formatPercentageText(displayCpuPercent) }}</span></span
            >
            <i class="summary-separator">·</i>
            <span class="summary-metric summary-memory"
              ><b>内存</b><span class="summary-percent">{{ formatPercentageText(displayMemPercent) }}</span></span
            >
            <i class="summary-separator">·</i>
            <span class="summary-metric summary-disk"
              ><b>磁盘</b><span class="summary-percent">{{ formatPercentageText(displayDiskPercent) }}</span></span
            >
          </span>
      ...
      <div class="monitor-content">
        ...
        >
          ...
          <span class="network-rate rate-down"
            ><DownloadIcon /><b>{{ formatBytesPerSecond(currentServerStatus?.netRxRate) }}</b></span
          >
          <span class="network-rate rate-up"
            ><UploadIcon /><b>{{ formatBytesPerSecond(currentServerStatus?.netTxRate) }}</b></span
          >
        </button>
        ...
        <section v-if="selectedMetric" class="history-card" :style="{ '--history-accent': selectedMetricColor }">
          ...
          <div class="history-chart-wrap" @pointermove="handleHistoryPointerMove" @pointerleave="clearHistoryHover">
            <svg
              class="history-chart"
              viewBox="0 0 248 126"
              preserveAspectRatio="none"
              role="img"
              :aria-label="`${selectedMetricTitle}图表`"
            >
              <defs>
              ...
              <path v-if="selectedMetric !== 'network'" class="history-area" :d="singleAreaPath" />
              <polyline v-if="selectedMetric !== 'network'" class="history-line" :points="singleLinePoints" />
              <polyline
    ...
    <Teleport to="body">
      ...
      >
        ...
        <span v-else class="history-tooltip-row">
          <i></i><span>{{ selectedMetricLabel }}</span
          ><b>{{ formatHistoryPercentage(historyHoverData.primaryValue) }}</b>
        </span>
      </div>
    </Teleport>
  </div>
...
<script setup lang="ts">
  ...
  const { statusMonitorShowIpBoolean, statusMonitorIntervalSecondsNumber, statusMonitorScaleNumber } =
  ...
  const statusScaleSaver = createLatestValueSaver<string>({
    ...
    onPendingChange: (pending) => {
      statusScaleSyncLocked.value = pending;
    },
  ...
  const memDisplay = computed(() => {
    ...
    return data?.memUsed === undefined || data.memTotal === undefined
      ? '—'
      : formatMemoryPair(data.memUsed, data.memTotal);
  });
  const memDisplayCompact = computed(() => {
    const data = currentServerStatus.value;
    return data?.memUsed === undefined || data.memTotal === undefined
      ? '—'
      : formatMemoryPairCompact(data.memUsed, data.memTotal);
  });
  ...
  const diskDisplay = computed(() => {
    const data = currentServerStatus.value;
    return data?.diskUsed === undefined || data.diskTotal === undefined
      ? '—'
      : `${(data.diskUsed / 1024 / 1024).toFixed(1)} / ${(data.diskTotal / 1024 / 1024).toFixed(1)} GB`;
  });
  const diskDisplayCompact = computed(() => {
    const data = currentServerStatus.value;
    return data?.diskUsed === undefined || data.diskTotal === undefined
      ? '—'
      : `${(data.diskUsed / 1024 / 1024).toFixed(1)}/${(data.diskTotal / 1024 / 1024).toFixed(1)}G`;
  });
  ...
  >(() => [
    {
      ...
      detail: '',
      compactDetail: cpuModelDisplay.value,
      color: '#42a5ff',
      icon: CpuIcon,
    },
    {
      key: 'memory',
      name: '内存',
      percent: displayMemPercent.value,
      detail: memDisplay.value,
      compactDetail: memDisplayCompact.value,
      color: '#36d982',
      icon: MemoryIcon,
    },
    {
      ...
      detail: swapDisplay.value,
      compactDetail: swapDisplayCompact.value,
      color: '#a66cff',
      icon: SwapIcon,
    },
    {
      ...
      detail: diskDisplay.value,
      compactDetail: diskDisplayCompact.value,
      color: '#ff814a',
      icon: DiskIcon,
    },
  ]);
  ...
  const rawNetworkHistoryMax = computed(() =>
    Math.max(
  ...
  const niceNetworkAxisMax = (rawMaxBytes: number) => {
    ...
    const unit = rawMaxBytes >= 1024 ** 2 ? 1024 ** 2 : rawMaxBytes >= 1024 ? 1024 : 1;
  ...
  const pointString = (values: number[], maxValue: number) => {
    ...
    return values
      .map((value, index) => {
        const x =
          HISTORY_PLOT_LEFT +
          (values.length === 1 ? HISTORY_PLOT_WIDTH : (index / (values.length - 1)) * HISTORY_PLOT_WIDTH);
  ...
  const singleAreaPath = computed(
    () =>
      `M${HISTORY_PLOT_LEFT},${HISTORY_PLOT_BOTTOM} L${singleLinePoints.value.replaceAll(' ', ' L')} L${HISTORY_PLOT_RIGHT},${HISTORY_PLOT_BOTTOM} Z`,
  );
  ...
  const historyValueY = (value: number, maxValue: number) =>
    HISTORY_PLOT_BOTTOM - Math.min(1, Math.max(0, value / Math.max(1, maxValue))) * HISTORY_PLOT_HEIGHT;
  ...
  let attachedStatusManager: { activate: () => void; deactivate: () => void; refreshInterval?: () => void } | null =
    null;
...
<style scoped>
  ...
  .history-axis-y-middle {
    top: 41.27%;
  }
  .history-axis-y-bottom {
    top: 74.6%;
  }
  .history-axis-x {
    top: 92.06%;
  ...
  .history-axis-x-start {
    left: 4.84%;
    transform: translateY(-50%);
  }
  .history-axis-x-middle {
    left: 50.81%;
  ...
  .history-hover-marker .history-hover-download {
    stroke: #35db81;
  }
  .history-hover-marker .history-hover-upload {
    stroke: #ff814a;
  }
  .history-tooltip {
    position: fixed;
  ...
  .history-tooltip-row:not(.rate-down):not(.rate-up) {
    color: var(--history-accent);
  }
  .history-tooltip-row span {
    color: var(--text-color-secondary);
  }
  ...
  @container status-pane (max-width: 250px) and (min-height: 640px) {
    ...
    .status-monitor:not(.has-history) .metric-identity {
      gap: 0.46rem;
    }
  ...
  @container status-pane (max-width: 145px) {
    ...
    .history-card .chart-grid {
      display: block;
    }
    .history-axis-x-middle {
      display: none;
    }
    .history-card .chart-grid line {
      stroke-opacity: 0.8;
    }
    .history-chart {
      min-height: 2.2rem;
  ...
  @container status-pane (max-width: 145px) and (max-height: 380px) {
    ...
    .status-monitor .metric-detail-compact {
      display: none;
    }
  }
  ...
  @container status-pane (max-height: 235px) {
    ...
    .summary-metric {
      ...
      gap: clamp(0.22rem, 2cqw, 0.42rem) !important;
    }
    .summary-percent {
      display: inline;
      color: var(--status-text);
    ...
    .summary-resources {
      ...
      padding-inline: clamp(0.08rem, 1.2cqw, 0.22rem);
    }
    .summary-separator {
      align-self: center;
    ...
    .summary-network {
      ...
      justify-content: center;
      column-gap: clamp(0.2rem, 1.2cqw, 0.32rem);
      padding-inline: 0;
    }
    ...
    .summary-network .rate-up .rate-unit {
      color: #ff814a;
    }
    .summary-network .rate-down svg,
    .summary-network .rate-down .rate-unit {
      color: #35db81;
    }
    .summary-network .rate-unit {
      font-weight: 700;
  ...
  @container status-pane (max-width: 132px) {
    ...
    .summary-row {
      min-height: 0;
      font-size: clamp(0.59rem, 6.4cqw, 0.68rem);
    }
    .summary-resources {
      ...
      row-gap: clamp(0.24rem, 2.8cqh, 0.4rem);
      padding-inline: 0;
    }
    .summary-separator {
      display: none;
    ...
    .summary-network {
      ...
      row-gap: clamp(0.22rem, 2.5cqh, 0.36rem);
      padding-inline: 0;
    }
    .summary-network > span {
      width: auto;
      ...
      display: grid;
      grid-template-columns: 0.82rem minmax(4.4ch, max-content);
      align-items: center;
      justify-content: center;
      column-gap: 0.22rem;
    }
  ...
  /* 当前窄面板优先保证资源文字完整，再切换横向摘要 */
  @container status-pane (max-width: 180px) {
    .summary-resources {
      width: min(100%, 5.2rem);
      display: grid !important;
      grid-template-columns: 1fr;
  ...
  @container status-pane (max-width: 200px) and (max-height: 130px) {
    .summary-network > span {
      ...
      grid-template-columns: 0.82rem minmax(0, 1fr) max-content !important;
