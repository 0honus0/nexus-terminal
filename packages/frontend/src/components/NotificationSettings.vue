<template>
  <div data-testid="notification-settings" class="p-0">
    ...
    <div v-else-if="!store.isLoading && !store.error && settings.length > 0" class="grid gap-4 mt-4">
      ...
      >
        ...
        <div class="flex-grow">
          ...
          <small class="block text-sm text-text-secondary mt-1">{{ getEventNames(setting.enabled_events) }}</small>
          <!-- Events text -->
        </div>
    ...
    <div v-if="showAddForm || editingSetting" class="mt-6 p-6 border border-border bg-background rounded-lg shadow-sm">
      <!-- Form container -->
