  ...
  >
    <div class="flex items-center justify-between gap-3 border-b border-border px-3 py-2">
      ...
      <div class="flex items-center gap-2">
        <span
          v-if="transferList.some((item) => item.status === 'running')"
    ...
    <ul class="custom-scrollbar m-0 max-h-52 list-none overflow-y-auto p-3">
      <li v-for="transfer in transferList" :key="transfer.id" class="mb-3 last:mb-0">
          ...
          <span class="min-w-0 flex-1 truncate" :title="transfer.currentFile || transfer.label">
            {{
              transfer.operation === 'move'
                ? t('fileManager.transferOperation.move')
                : t('fileManager.transferOperation.copy')
            }}
        ...
        <div class="mt-1 flex items-center justify-between gap-2 text-[11px] text-text-secondary">
          ...
          <span v-else-if="transfer.status === 'error'" class="text-red-600">{{
            transfer.error || t('fileManager.errors.generic')
          }}</span>
          ...
          <span class="shrink-0 tabular-nums">
            <template v-if="transfer.totalKnown && transfer.totalBytes > 0"
              >{{ formatBytes(transfer.bytesWritten) }} / {{ formatBytes(transfer.totalBytes) }}</template
            >
            <template v-else-if="transfer.totalKnown"
              >{{ transfer.completedFiles }} / {{ transfer.totalFiles }}</template
            >
