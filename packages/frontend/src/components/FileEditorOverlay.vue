<script setup lang="ts">
  ...
  const currentSelectedEncoding = computed(() =>
    normalizeEncodingOptionValue(activeTab.value?.selectedEncoding ?? 'utf-8'),
  );
  ...
  const handleReloadRequest = async () => {
    ...
    if (
      tab.isModified &&
      !window.confirm(t('fileManager.confirmDiscardChanges', 'Discard unsaved changes and reload?'))
    )
      return;
...
<template>
  ...
  <div
    v-if="isVisible"
    data-testid="file-editor-overlay"
    ...
    <div class="editor-popup" :style="popupStyle">
      ...
      <div v-if="activeTab" class="editor-header" :class="{ 'is-mobile': props.isMobile }">
        <span>
          ...
          }}<template v-if="shareFileEditorTabsBoolean && currentTabSessionName">({{ currentTabSessionName }})</template
          >: {{ currentTabFilePath }}
        ...
        <div class="editor-actions">
          ...
          <button
            ...
            :disabled="currentTabIsSaving || currentTabIsLoading || !!currentTabLoadingError || !activeTab"
            class="save-btn"
          >
      ...
      <div v-else class="editor-header editor-header-placeholder" :class="{ 'is-mobile': props.isMobile }">
        ...
        <button @click="handleCloseButton" class="close-editor-btn" :title="t('fileManager.actions.closeEditor')">
          ✖
        </button>
      </div>
      ...
      <div
        v-if="!props.isMobile"
        data-testid="file-editor-resize-handle"
        class="resize-handle"
