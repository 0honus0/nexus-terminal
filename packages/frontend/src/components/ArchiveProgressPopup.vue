<script setup lang="ts">
  ...
  const operationLabel = computed(() =>
    props.progress.operation === 'compress'
      ...
      <div class="archive-progress-header" @pointerdown="startDragging">
        <div class="archive-title">
          ...
          <div class="min-w-0">
            <div class="truncate font-semibold" :title="props.sessionLabel || undefined">
              <span v-if="props.sessionLabel">{{ props.sessionLabel }} · </span>{{ operationLabel }}
              {{ progress.archiveName || '...' }}
            </div>
        ...
        <div class="archive-actions">
          ...
          <button
            ...
            type="button"
            data-testid="archive-progress-hide"
            class="icon-button"
            @click="hidePopup"
      ...
      <div class="archive-progress-body">
        <div v-if="progress.percent !== null" class="space-y-1.5">
          <div class="flex items-center justify-between gap-4 text-xs">
            <span>{{
              t('fileManager.archiveProgress.filesProcessedTotal', {
          ...
          >
            <div class="progress-value" :style="{ transform: `scaleX(${normalizedPercent / 100})` }"></div>
          </div>
        </div>
        ...
        <button type="button" class="stop-button" :disabled="progress.cancelling" @click="emit('cancel')">
...
<style scoped>
  ...
  .archive-progress-card.dragging {
    cursor: grabbing;
    transition: none;
  }
