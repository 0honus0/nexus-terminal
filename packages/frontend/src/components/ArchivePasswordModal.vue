<script setup lang="ts">
  ...
  const title = computed(() =>
    props.mode === 'decompress'
      ? t('fileManager.archivePassword.decompressTitle', 'Extract password-protected zip')
      : t('fileManager.archivePassword.compressTitle', 'Create password-protected zip'),
  );
  ...
  const validationMessage = computed(() => {
    if (props.errorMessage && !externalErrorDismissed.value) return props.errorMessage;
    ...
    if (props.mode === 'compress' && confirmPassword.value && password.value !== confirmPassword.value) {
  ...
  const isConfirmDisabled = computed(() => {
    ...
    if (props.mode === 'compress' && password.value !== confirmPassword.value) return true;
    return false;
  });
    ...
    <div class="space-y-4">
      <label class="block">
        ...
        <input
          ...
          :autocomplete="mode === 'compress' ? 'new-password' : 'current-password'"
      ...
      <label class="flex items-center gap-2 text-sm text-text-secondary cursor-pointer select-none">
        <input v-model="showPassword" type="checkbox" class="accent-primary" />
        {{ t('fileManager.archivePassword.showPassword', 'Show password') }}
      </label>

      <p v-if="validationMessage" class="text-sm text-error" data-testid="archive-password-error">
        {{ validationMessage }}
      </p>
