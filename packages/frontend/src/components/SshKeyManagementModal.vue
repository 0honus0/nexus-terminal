<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name || (!keyToEdit.value && !formData.private_key)) {
      // Private key is required only for new keys
  ...
  >
      ...
      <div class="flex-grow overflow-y-auto border border-border rounded-md" style="max-height: 50vh">
        <table class="min-w-full divide-y divide-border">
          ...
          <tbody class="bg-background divide-y divide-border">
            <tr v-if="isLoading">
              <td colspan="2" class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary text-center">
    ...
    <div v-else class="flex flex-col h-full">
      ...
      <form @submit.prevent="handleSubmit" class="flex-grow overflow-y-auto pr-2 space-y-4">
        <div>
          <label for="key-name" class="block text-sm font-medium text-text-secondary mb-1">{{
            t('sshKeys.modal.keyName')
          }}</label>
          <input
            type="text"
        ...
        <div>
          <label for="key-private" class="block text-sm font-medium text-text-secondary mb-1">{{
            t('sshKeys.modal.privateKey')
          }}</label>
          <textarea
            id="key-private"
        ...
        <div>
          <label for="key-passphrase" class="block text-sm font-medium text-text-secondary mb-1"
            >{{ t('sshKeys.modal.passphrase') }} ({{ t('connections.form.optional') }})</label
          >
          <input
            type="password"
            id="key-passphrase"
