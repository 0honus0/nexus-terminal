<script setup lang="ts">
  ...
  const cancellableCount = computed(
    () =>
      Object.values(props.uploads).filter((upload) =>
        ['pending', 'uploading', 'paused', 'conflict'].includes(upload.status),
      ).length,
  );
  const sourceHidden = computed(() =>
    props.progressSourceId ? progressCenter.isSourceHidden(props.progressSourceId) : false,
  );
  ...
  const sampleTotalUploadSpeed = () => {
    ...
    totalUploadSpeed.value =
      totalUploadSpeed.value > 0 ? totalUploadSpeed.value * 0.55 + instantSpeed * 0.45 : instantSpeed;
  ...
  const popupWidth = computed({
    ...
    set: (width: number) => {
      popupSize.value = { ...popupSize.value, width };
    },
  });
  const popupHeight = computed({
    get: () => popupSize.value.height,
    set: (height: number) => {
      popupSize.value = { ...popupSize.value, height };
    },
  });
  ...
  >
    ...
    <ul data-testid="file-upload-list" class="custom-scrollbar m-0 min-h-0 flex-1 list-none overflow-y-auto p-3">
      <li v-for="upload in uploadList" :key="upload.id" class="upload-task-row mb-1.5 text-xs last:mb-0">
        <span class="min-w-0 truncate" :title="upload.filename"
          >{{ upload.filename }} ({{ t(`fileManager.uploadStatus.${upload.status}`) }})</span
        >
        <div class="min-w-0">
          <progress
            v-if="(upload.status === 'uploading' && upload.progress < 100) || upload.status === 'pending'"
            data-testid="file-upload-progress-bar"
            :value="upload.progress"
            max="100"
            class="h-2 w-full [&::-webkit-progress-bar]:rounded-lg [&::-webkit-progress-value]:rounded-lg [&::-webkit-progress-bar]:bg-gray-300 [&::-webkit-progress-value]:bg-blue-600 [&::-moz-progress-bar]:bg-blue-600"
          ></progress>
        </div>
        ...
        <!-- 只有在可取消状态时显示取消按钮 -->
        <button
          v-if="['pending', 'uploading', 'paused', 'conflict'].includes(upload.status)"
          data-testid="file-upload-cancel"
          @click="handleCancel(upload.id)"
          class="justify-self-end px-1.5 py-0.5 text-xs bg-red-100 border border-red-300 text-red-700 cursor-pointer rounded hover:bg-red-200"
        >
          {{ t('fileManager.actions.cancel') }}
        </button>
        <span v-else></span>
        <span v-if="upload.status === 'error'" class="col-span-4 text-red-600 text-xs">
