    ...
    ([newIsVisible, newPosition], [oldIsVisible, oldPosition]) => {
      if (newIsVisible) {
        ...
        const positionChangedWhileVisible =
          oldIsVisible && oldPosition && (newPosition.x !== oldPosition.x || newPosition.y !== oldPosition.y);
      ...
      <template v-for="(menuItem, index) in items" :key="index">
        ...
        <template v-else-if="isMobile && menuItem.submenu && menuItem.submenu.length > 0">
          ...
          >
            ...
            <span v-if="subItem.shortcut" class="text-[11px] text-text-secondary whitespace-nowrap">{{
              subItem.shortcut
            }}</span>
          </li>
        ...
        <li
          ...
          :class="[
            'px-4 py-1.5 text-foreground text-sm flex items-center justify-between gap-4 transition-colors duration-150 rounded mx-1',
            isItemDisabled(menuItem)
              ? 'opacity-40 cursor-not-allowed'
              : 'cursor-pointer hover:bg-primary/10 hover:text-primary',
          ]"
        >
          <span>{{ menuItem.label }}</span>
          <span v-if="menuItem.shortcut" class="text-[11px] text-text-secondary whitespace-nowrap">{{
            menuItem.shortcut
          }}</span>
        </li>
