<script setup lang="ts">
  ...
  const {
    clipboardState,
    sourceSessionId: clipboardSourceSessionId,
    sourcePaths: clipboardSourcePaths,
    sourceBaseDir: clipboardSourceBaseDir,
  } = storeToRefs(fileClipboardStore);
  ...
  const previewTabDescriptors = computed(() =>
    previewTabs.value.map((entry) => ({
      id: entry.id,
  ...
  const getFileIconClassBase = (filename: string): string => {
    ...
    const iconMap: { [key: string]: string } = {
      ...
      /* 'gitattributes': 'fab fa-git-alt', */ /* 'gitmodules': 'fab fa-git-alt', */ gitkeep: 'fab fa-git-alt', // Removed duplicate gitattributes and gitmodules
  ...
  const scheduleRecursiveSearch = () => {
    ...
    recursiveSearchTimer = setTimeout(() => {
      ...
      void manager
        .searchDirectory(rootPath, query)
        .then((result) => {
        ...
        .catch((error) => {
          ...
          uiNotificationsStore.showError(
            error instanceof Error ? error.message : t('fileManager.errors.searchFailed', 'Failed to search files.'),
          );
        })
        .finally(() => {
  ...
  const virtualTopPadding = computed(() =>
    shouldVirtualizeFileList.value ? virtualStartIndex.value * estimatedFileRowHeight.value : 0,
  );
  const virtualBottomPadding = computed(() =>
    shouldVirtualizeFileList.value
  ...
  const openFileTarget = async (item: FileListItem, filePath: string): Promise<void> => {
    ...
    if (previewProvider) {
      ...
      if (maxInlineSize && item.attrs.size > maxInlineSize) {
        uiNotificationsStore.showError(t('fileManager.preview.fileTooLarge', { size: formatSize(maxInlineSize) }));
        return;
      }
  ...
  const itemForOpen = (item: FileListItem): FileListItem =>
    item.basename ? { ...item, filename: item.basename } : item;
    ...
    if (item.attrs.isSymbolicLink) {
      ...
      unregisterSuccess = wsOnMessage('sftp:realpath:success', (payload: any, message: WebSocketMessage) => {
        ...
        if (targetType === 'unknown') {
          console.warn(
            `[FileManager ${props.sessionId}-${props.instanceId}] Symlink target type is unknown; attempting to open it as a file.`,
          );
        }
      ...
      unregisterError = wsOnMessage('sftp:realpath:error', (payload: any, message: WebSocketMessage) => {
        ...
        console.error(
          `[FileManager ${props.sessionId}-${props.instanceId}] Failed to resolve symlink '${itemPath}': ${serverErrorMsg}`,
        );
  ...
  const setFileClipboard = (operation: 'copy' | 'cut') => {
    ...
    const manager = currentSftpManager.value;
    const sourcePaths = Array.from(selectedItems.value).map((filename) =>
    ...
    console.log(
      `[FileManager ${props.sessionId}-${props.instanceId}] ${operation === 'copy' ? 'Copied' : 'Cut'} to shared clipboard:`,
      sourcePaths,
    );
  };
  ...
  const deleteSourcePathsAfterCrossHostCopy = (
  ...
  const handlePaste = async () => {
    if (!currentSftpManager.value || !clipboardState.value.hasContent || clipboardSourcePaths.value.length === 0)
      return;
    const manager = currentSftpManager.value;
    ...
    console.log(
      `[FileManager ${props.sessionId}-${props.instanceId}] Pasting items. Operation: ${operation}, Source session: ${sourceSessionId}, Destination session: ${destinationSessionId}, Sources: ${sources.join(', ')}, Destination: ${destinationDir}`,
    );
    ...
    for (const item of items) {
      ...
      void (async () => {
        try {
          ...
          console.error(
            `[FileManager ${props.sessionId}-${props.instanceId}] Failed to create download ticket for ${item.filename}:`,
            error,
          );
  ...
  const triggerDownloadDirectory = (item: FileListItem) => {
    ...
    const directoryPath =
      item.path || currentSftpManager.value.joinPath(currentSftpManager.value.currentPath.value, item.filename);
  ...
  const handleCompress = (items: FileListItem[], format: CompressFormat) => {
    ...
    void currentSftpManager.value.compressItems(items, format).catch((error) => {
      if (error instanceof Error && (error.message === 'ARCHIVE_CANCELLED' || error.name === 'ARCHIVE_IN_PROGRESS'))
        return;
  ...
  const handleArchivePasswordConfirm = (password: string) => {
    ...
    if (mode === 'compress' && items.length > 0) {
      void manager.compressItems(items, 'zip', password).catch((error) => {
        ...
        console.error(
          `[FileManager ${props.sessionId}-${props.instanceId}] Password-protected ZIP compression failed:`,
          error,
        );
      });
      return;
    ...
    if (mode === 'decompress' && item) {
      void manager.decompressItem(item, password).catch((error) => {
        ...
        if (
          ...
          ['INVALID_PASSWORD', 'PASSWORD_TOO_LONG', 'INVALID_PASSWORD_FORMAT'].includes(error.name)
        ) {
          openArchivePasswordModal('decompress', { item, error: getArchivePasswordErrorMessage(error) });
          return;
        }
        console.error(
          `[FileManager ${props.sessionId}-${props.instanceId}] Password-protected ZIP extraction failed:`,
          error,
        );
      });
    }
  ...
  const handleMobileContextPointerStart = (event: PointerEvent, item?: FileListItem) => {
    ...
    if (target instanceof Element) {
      try {
        ...
        /* browser may reject capture during native gestures */
      }
    }
  ...
  watch(
    () => currentSftpManager.value?.contentRevision.value,
    () => {
      if (searchQuery.value.trim()) scheduleRecursiveSearch();
    },
  );
  ...
  const persistLayoutSettings = async ({ multiplier, widths }: FileManagerLayoutSnapshot) => {
    console.log(
      `[FileManager ${props.sessionId}-${props.instanceId}] Persisting layout: multiplier=${multiplier}, widths=${JSON.stringify(widths)}`,
    );
  ...
  const layoutSettingsSaver = createLatestValueSaver<FileManagerLayoutSnapshot>({
    ...
    onPendingChange: (pending) => {
      rowScaleSyncLocked.value = pending;
    },
    onError: (error) =>
      console.error(`[FileManager ${props.sessionId}-${props.instanceId}] Failed to persist layout settings:`, error),
  });
  ...
  watchEffect((onCleanup) => {
    ...
    if (manager && isManagerReady && !manager.isLoading.value && !manager.initialLoadDone.value) {
      console.log(
        `[FileManager ${props.sessionId}-${props.instanceId}] Connection ready for manager, fetching initial path for the first time (isLoading: ${manager.isLoading.value}, initialLoadDone: ${manager.initialLoadDone.value}).`,
      );
      ...
      if (wasManagerReady === false && !manager.isLoading.value) {
        ...
        console.log(
          `[FileManager ${props.sessionId}-${props.instanceId}] SFTP connection recovered. Reloading previous path: ${pathBeforeReconnect}`,
        );
        manager.loadDirectory(pathBeforeReconnect, false);
      } else {
        console.log(
          `[FileManager ${props.sessionId}-${props.instanceId}] SFTP manager is already ready. Keeping cached directory without reload.`,
        );
      }
      cleanupListeners(); // 清理可能存在的旧监听器
    } else if (!isManagerReady && manager?.initialLoadDone.value) {
  ...
  const handlePathInputFocus = () => {
    ...
    if (!currentSftpManager.value || currentSftpManager.value.isLoading.value || !props.wsDeps.isConnected.value)
      return;
  ...
  const sendCdCommandToTerminal = () => {
    ...
    unregisterResult = props.wsDeps.onMessage('ssh:change_directory:result', (payload, message) => {
      ...
      uiNotificationsStore.showSuccess(
        t('fileManager.notifications.terminalPathChanged', { path: payload?.path || currentPath }),
      );
    });
  ...
  const syncPathFromTerminal = () => {
    ...
    const unregisterError = props.wsDeps.onMessage('ssh:exec_silent:error', (payload, message) => {
      ...
      finish();
      uiNotificationsStore.showError(
        payload?.error || t('fileManager.errors.pathReadFailed', 'Failed to read terminal path.'),
      );
    });
  ...
  const handleWheel = (event: WheelEvent) => {
    ...
    const container = fileListContainerRef.value;
    const anchoredRow =
      shouldVirtualizeFileList.value && container ? container.scrollTop / oldEstimatedRowHeight : null;
  ...
  <div class="file-manager-root flex flex-col h-full overflow-hidden bg-background text-foreground text-sm font-sans">
    <div class="file-manager-toolbar flex items-center justify-between flex-wrap gap-1 p-2 bg-header flex-shrink-0">
      ...
      <div class="file-manager-actions flex items-center gap-1 min-w-0">
        ...
        <button
          ...
          :disabled="
            !currentSftpManager || !props.wsDeps.isConnected.value || isEditingPath || isSyncingPathFromTerminal
          "
      ...
      <div
        ...
        class="file-manager-path-input relative flex items-center bg-background border border-border rounded px-1.5 py-0.5 min-w-0"
      >
        <span
          v-show="!isEditingPath && !showPathHistoryDropdown"
          @click="startPathEdit"
          class="text-text-secondary pr-2 cursor-text truncate min-w-0"
        >
          <strong
    ...
    >
      ...
      >
        ...
        <thead class="sticky top-0 z-10 bg-header">
          <tr>
            <th
              ...
              :style="{
                paddingTop: `calc(0.4rem * var(--row-size-multiplier))`,
                paddingBottom: `calc(0.4rem * var(--row-size-multiplier))`,
                paddingLeft: '1rem',
                paddingRight: '0.5rem',
              }"
            >
          ...
          >
            ...
            >
              <i
                ...
                :style="{ fontSize: `calc(1.1em * max(0.85, var(--row-size-multiplier) * 0.5 + 0.5))` }"
              ></i>
            </td>
            <td
        ...
        <tbody v-else>
          ...
          <tr
            v-for="(item, index) in virtualFileList"
            :key="item.path || item.filename"
            :draggable="!props.isMobile && item.filename !== '..'"
            ...
            :class="[
              { 'cursor-pointer': item.attrs.isDirectory || item.attrs.isFile },
              {
                'bg-primary text-white':
                  ...
                  virtualStartIndex + index + (currentSftpManager?.currentPath.value !== '/' ? 1 : 0) === selectedIndex,
              },
              {
                'hover:bg-header/50': !(
                  selectedItems.has(item.filename) ||
              ...
              <i
                :class="[
                      ...
                      : `${getFileIconClassBase(itemBaseName(item))} text-text-secondary`,
                  {
                    'text-white':
                      selectedItems.has(item.filename) ||
                      virtualStartIndex + index + (currentSftpManager?.currentPath.value !== '/' ? 1 : 0) ===
                        selectedIndex,
                  },
                ]"
                :style="{ fontSize: `calc(1.1em * max(0.85, var(--row-size-multiplier) * 0.5 + 0.5))` }"
    ...
    >
      <div class="file-preview-loading-card">
        ...
        <button type="button" :aria-label="t('fileManager.preview.close')" @click="handlePreviewCloseButton">×</button>
      </div>
    </div>
