<script setup lang="ts">
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      errorMessage.value =
        error.message || t('favoritePaths.addEditForm.errors.genericSaveError', 'Failed to save favorite path.');
...
<template>
  <OverlayPanel :visible="isVisible" :z-index="60" panel-class="max-w-md flex flex-col p-6" @close="closeModal">
    <!-- Header -->
