    ...
    <div class="p-6 space-y-4"> <!-- Reduced space-y for tighter layout -->
       <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-text-secondary"> <!-- Flex container for info items, allow wrap -->
          ...
          <a v-else-if="isUpdateAvailable && latestVersion"
             :href="getReleaseUrl(latestVersion)"
          ...
          <a :href="releaseRepositoryUrl" target="_blank" rel="noopener noreferrer" class="text-primary hover:underline inline-flex items-center">
...
import { getReleaseUrl, releaseRepository, releaseRepositoryUrl } from '../../config/release';
