      ...
      <div v-if="ipBlacklistEnabled" class="space-y-6 pt-4">
        ...
        <form @submit.prevent="handleUpdateBlacklistSettings" class="flex flex-wrap items-end gap-4">
          <div class="flex-grow min-w-[150px]">
            <label for="maxLoginAttempts" class="block text-sm font-medium text-text-secondary mb-1">{{
              $t('settings.ipBlacklist.maxAttemptsLabel')
            }}</label>
            <input
          ...
          <div class="flex-grow min-w-[150px]">
            <label for="loginBanDuration" class="block text-sm font-medium text-text-secondary mb-1">{{
              $t('settings.ipBlacklist.banDurationLabel')
            }}</label>
            <input
          ...
          <p
            ...
            :class="['w-full mt-2 text-sm', blacklistSettingsSuccess ? 'text-success' : 'text-error']"
          >
        ...
        <!-- Error state -->
        <div v-if="ipBlacklist.error" class="p-3 border-l-4 border-error bg-error/10 text-error text-sm rounded">
          {{ ipBlacklist.error }}
        </div>
