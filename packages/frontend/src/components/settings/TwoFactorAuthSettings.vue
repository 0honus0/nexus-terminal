<template>
  <div class="settings-section-content">
    ...
    <div v-else>
      ...
      <div v-if="isSettingUp2FA && setupData" class="mt-4 space-y-4 p-4 border border-border rounded-md bg-header/30">
        ...
        <form @submit.prevent="handleVerifyAndActivate2FA" class="space-y-4">
          <div>
            <label for="verificationCode" class="block text-sm font-medium text-text-secondary mb-1">{{
    ...
    <p v-if="twoFactorMessage" :class="['mt-3 text-sm', twoFactorSuccess ? 'text-success' : 'text-error']">
      {{ twoFactorMessage }}
    </p>
  </div>
</template>
