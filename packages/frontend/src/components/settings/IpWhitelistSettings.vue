<template>
  <div data-testid="ip-whitelist-settings" class="settings-section-content">
    ...
    <form @submit.prevent="handleUpdateIpWhitelist" class="space-y-4">
      <div>
        <label for="ipWhitelist" class="block text-sm font-medium text-text-secondary mb-1">{{
          $t('settings.ipWhitelist.label')
        }}</label>
        <textarea
      ...
      <div class="flex items-center justify-between">
        ...
        <p v-if="ipWhitelistMessage" :class="['text-sm', ipWhitelistSuccess ? 'text-success' : 'text-error']">
          {{ ipWhitelistMessage }}
        </p>
      </div>
    </form>
...
<script setup lang="ts">
  ...
  const { ipWhitelistInput, ipWhitelistLoading, ipWhitelistMessage, ipWhitelistSuccess, handleUpdateIpWhitelist } =
    useIpWhitelist();
</script>
