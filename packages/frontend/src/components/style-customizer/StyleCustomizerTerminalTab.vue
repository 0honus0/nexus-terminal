...
<template>
  <div v-if="!isEditingTheme">
    <section>
      ...
      <!-- 预览区域 -->
      <div class="my-6 border border-border rounded-lg overflow-hidden bg-background">
        <div class="px-4 py-2 bg-header border-b border-border">
          <span class="text-sm font-medium text-foreground">{{
            t('styleCustomizer.preview') || '预览'
          }}</span>
        </div>
        <TerminalPreview height="400px" />
      </div>
