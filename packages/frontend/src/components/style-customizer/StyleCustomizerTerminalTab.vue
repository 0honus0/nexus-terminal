  ...
  const handleSaveEditingTheme = async () => {
    ...
    try {
      ...
      if (props.editingTheme._id) {
        ...
        await appearanceStore.updateTerminalTheme(props.editingTheme._id, updateDto.name, updateDto.themeData);
...
<template>
  <section v-if="!isEditingTheme" data-testid="terminal-style-settings">
    ...
    <div class="space-y-3 mb-3">
      ...
      <div class="space-y-3">
        <div class="grid grid-cols-1 md:grid-cols-[auto_1fr] items-center gap-2">
          <label for="terminalTextShadowOffsetX" class="text-left text-foreground text-sm font-medium"
        ...
        <div class="grid grid-cols-1 md:grid-cols-[auto_1fr] items-center gap-2">
          <label for="terminalTextShadowOffsetY" class="text-left text-foreground text-sm font-medium"
    ...
    <div class="mt-4 mb-6 flex gap-2 flex-wrap items-center pb-4 border-b border-dashed border-border">
      <button
        data-testid="terminal-theme-add"
    ...
    >
      ...
      >
        <span
          ...
          :class="theme._id === activeTerminalThemeId?.toString() ? 'font-bold text-button-text' : 'text-foreground'"
          :title="theme.name"
          >{{ theme.name }}</span
        >
        <div class="flex md:col-start-2 md:col-end-3 flex-shrink-0 gap-2 justify-start md:justify-end flex-wrap">
          ...
          <button
            ...
            :title="theme.isPreset ? t('styleCustomizer.editAsCopy', 'Edit as Copy') : t('common.edit')"
            :class="[
              'px-3 py-1.5 text-xs md:text-sm border rounded transition-colors duration-200 ease-in-out whitespace-nowrap disabled:opacity-60 disabled:cursor-not-allowed',
              theme._id === activeTerminalThemeId?.toString()
  ...
  <section v-if="isEditingTheme && editingTheme" data-testid="terminal-theme-editor">
    ...
    <p v-if="saveThemeError" class="text-error-text bg-error/10 border border-error/30 px-3 py-2 rounded text-sm mb-3">
      {{ saveThemeError }}
    </p>
    ...
    <div class="mt-4 flex justify-end gap-2 pt-4 border-t border-border">
      <button
        data-testid="terminal-theme-cancel"
      ...
      <button
        data-testid="terminal-theme-save"
