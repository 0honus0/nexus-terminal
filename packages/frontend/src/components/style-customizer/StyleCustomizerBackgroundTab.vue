<script setup lang="ts">
  ...
  const handleEditPresetAsNew = async (preset: { name: string; type: 'preset' | 'custom' }) => {
    ...
    try {
      ...
      notificationsStore.addNotification({
        type: 'error',
        message: t('styleCustomizer.errorFetchingPresetContentForCopy', { message: e.message, name: preset.name }),
      });
    }
  };
  ...
  <section>
    ...
    <div v-if="localTerminalBackgroundEnabled">
      ...
      <div class="mt-4 pt-4 border-t border-border/50">
        <label for="terminalBgOverlayOpacity" class="block text-sm font-medium text-foreground mb-1">{{
          t('styleCustomizer.terminalBgOverlayOpacity', '终端背景蒙版透明度:')
        }}</label>
      ...
      <div v-if="currentActiveTab === 'local'">
        ...
        >
          ...
          >
            ...
            >
              <span class="text-foreground font-medium" :title="preset.name.replace(/\.html$/, '')">{{
                preset.name.replace(/\.html$/, '')
              }}</span>
              <span
                ...
                @click="
                  async () => {
                    try {
                      ...
                      notificationsStore.addNotification({
                        type: 'error',
                        message: t('styleCustomizer.errorFetchingPresetContentForEdit', { message: e.message }),
                      });
                    }
                  }
                "
