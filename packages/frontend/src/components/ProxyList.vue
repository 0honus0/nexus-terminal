    ...
    if (confirmed) {
      ...
      if (!success) {
        showAlertDialog({
          ...
          message: t('proxies.errors.deleteFailed', { error: proxiesStore.error || t('common.unknownError') }),
        });
      }
    }
  };
...
<template>
  <div class="mt-4">
    ...
    <div v-else-if="proxies.length === 0" class="p-4 mb-4 border-l-4 border-blue-400 bg-blue-100 text-blue-700 rounded">
    ...
    <div v-else class="grid gap-4 mt-4">
      ...
      >
        ...
        <div class="flex-grow space-y-1">
          ...
          <div class="text-xs text-text-secondary pt-1">
            <!-- Updated At -->
            <i class="fas fa-clock mr-1 opacity-70"></i> {{ formatTimestamp(proxy.updated_at) }}
          </div>
        </div>
        <div class="flex items-center flex-shrink-0 space-x-3 pt-1">
          <!-- Actions section -->
          <button
            data-testid="proxy-edit"
            @click="emit('edit-proxy', proxy)"
