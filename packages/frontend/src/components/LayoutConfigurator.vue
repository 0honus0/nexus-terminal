...
const handleNodeRemove = async (payload: {
  parentNodeId: string | undefined;
  nodeIndex: number;
}) => {
...
const handleAvailablePaneDragEnd = (event: DraggableEndEvent) => {
  ...
  if (event.to !== event.from) {
    const paneName =
      event.oldIndex !== undefined ? localAvailablePanes.value[event.oldIndex] : null;
...
<template>
  <div
    v-if="isVisible"
    class="fixed inset-0 bg-overlay flex justify-center items-center z-[1000]"
        ...
        <section class="flex flex-col overflow-y-auto border-r border-border pr-6 min-w-[200px]">
          ...
          >
            <template #item="{ element }: { element: PaneName }">
              <li
                class="flex items-center p-2 mb-2 bg-background-alt border border-border rounded cursor-grab transition-colors duration-150 hover:bg-hover active:cursor-grabbing active:bg-border text-sm"
              >
                <i
                  class="fas fa-grip-vertical mr-2 text-text-alt cursor-grab active:cursor-grabbing"
                ></i>
        ...
        <div class="flex flex-col">
          ...
          <section class="flex flex-col min-w-[350px] flex-grow">
            <div class="flex justify-between items-center mb-4">
              <!-- +++ Flex container for title and switch +++ -->
              <h3 class="mt-0 mb-0 text-base font-semibold text-text-secondary">
              ...
              <div class="flex items-center gap-2">
                <label
                  ...
                  >{{ t('layoutConfigurator.lockLayout', '锁定布局') }}</label
                >
                <button
                  type="button"
                  @click="handleLayoutLockChange"
            ...
            <div
              class="flex-grow border-2 border-dashed border-border-alt rounded p-4 bg-background-alt flex flex-col overflow-auto min-h-[250px]"
            >
              <LayoutNodeEditor
          ...
          <div class="grid grid-cols-2 gap-6 border-t border-border pt-4 mt-4 min-h-[150px]">
            ...
            <section class="flex flex-col">
              <h3 class="mt-0 mb-4 text-base font-semibold text-text-secondary">
                {{ t('layoutConfigurator.leftSidebar', '左侧栏面板') }}
              </h3>
              <draggable
              ...
              >
                <template #item="{ element, index }: { element: PaneName; index: number }">
                  <li
                    class="flex items-center justify-between p-2 mb-2 bg-hover border border-border rounded cursor-grab transition-colors duration-150 hover:bg-border active:cursor-grabbing active:bg-border-alt text-sm"
                  >
                    <div class="flex items-center overflow-hidden">
                      <i
                        class="fas fa-grip-vertical mr-2 text-text-alt cursor-grab active:cursor-grabbing flex-shrink-0"
                      ></i>
                      <span
                        class="flex-grow mr-2 overflow-hidden text-ellipsis whitespace-nowrap"
                        >{{ paneLabels[element] || element }}</span
                      >
                    </div>
                    <button
                      @click="removeSidebarPane('left', index)"
                      class="bg-transparent border-none text-text-alt text-lg cursor-pointer p-1 leading-none flex-shrink-0 hover:text-error"
                ...
                <template #footer>
                  <li
                    v-if="localSidebarPanes.left.length === 0"
                    class="text-center text-text-alt p-4 italic text-sm w-full min-h-[50px] flex items-center justify-center"
                  >
            ...
            <section class="flex flex-col">
              <h3 class="mt-0 mb-4 text-base font-semibold text-text-secondary">
                {{ t('layoutConfigurator.rightSidebar', '右侧栏面板') }}
              </h3>
              <draggable
              ...
              >
                <template #item="{ element, index }: { element: PaneName; index: number }">
                  ...
                  >
                    ...
                    <button
                      @click="removeSidebarPane('right', index)"
                      class="bg-transparent border-none text-text-alt text-lg cursor-pointer p-1 leading-none flex-shrink-0 hover:text-error"
                      :title="t('common.remove', '移除')"
                    >
                      &times;
                ...
                <template #footer>
                  <li
                    v-if="localSidebarPanes.right.length === 0"
                    class="text-center text-text-alt p-4 italic text-sm w-full min-h-[50px] flex items-center justify-center"
                  >
                    {{ t('layoutConfigurator.dropHere', '从可用面板拖拽到此处') }}
                  </li>
