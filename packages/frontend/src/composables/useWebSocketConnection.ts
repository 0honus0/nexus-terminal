...
import { parseTerminalBinaryFrame, TerminalFrameFlag, TerminalFrameType } from '../utils/terminalBinaryProtocol';
...
export function createWebSocketConnectionManager(
  ...
  const ensureUploadTransport = async (): Promise<WebSocket> => {
    ...
    connectPromise = new Promise<WebSocket>((resolve, reject) => {
      ...
      const timeoutId = window.setTimeout(() => {
        ...
        try {
          socket.close(1000, 'Upload transport handshake timeout');
        } catch {
          /* ignore */
        }
      }, 5000);
  ...
  const waitForUploadBackpressure = (signal?: AbortSignal): Promise<void> =>
   ...
   */
  const sendBinaryMessage = async (frame: ArrayBuffer, maxBufferedBytes = 8 * 1024 * 1024): Promise<void> => {
