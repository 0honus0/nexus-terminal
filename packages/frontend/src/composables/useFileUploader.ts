...
const normalizeRelativeDirectory = (relativePath?: string): string => {
  ...
  const normalized = relativePath.replace(/\\/g, '/').replace(/^\.\//, '').replace(/^\/+/, '').replace(/\/+$/, '');
...
export function useFileUploader(
  ...
  const describeUploadTuning = (profile: UploadNetworkProfile): string => {
    ...
    return (
      `profile=${profile}, rtt=${Math.round(uploadNetwork.smoothedRttMs)}ms, throughput=${(uploadNetwork.smoothedThroughputBps / MiB).toFixed(2)}MiB/s, ` +
  ...
  const applyUploadNetworkCandidate = () => {
    ...
    uploadNetwork.candidateCount = 0;
    console.log(
      `[FileUploader ${sessionIdForLog.value}] Adaptive upload tuning changed: ${describeUploadTuning(candidate)}`,
    );
  ...
  const recordUploadNetworkAck = (sampleRttMs: number, committedBytes: number) => {
    ...
    if (uploadNetwork.rttSampleCount === 0) {
      ...
      uploadNetwork.smoothedRttMs =
        uploadNetwork.smoothedRttMs * (1 - UPLOAD_NETWORK_RTT_EWMA_ALPHA) + sampleRttMs * UPLOAD_NETWORK_RTT_EWMA_ALPHA;
    }
    ...
    const shouldSample =
      elapsedMs >= UPLOAD_THROUGHPUT_WINDOW_MAX_MS || (elapsedMs >= UPLOAD_THROUGHPUT_WINDOW_MIN_MS && enoughData);
    ...
    if (uploadNetwork.throughputSampleCount === 0) {
      ...
      uploadNetwork.smoothedThroughputBps =
        uploadNetwork.smoothedThroughputBps * (1 - UPLOAD_NETWORK_THROUGHPUT_EWMA_ALPHA) +
  ...
  const requestUploadBatchPrepare = (prepareId: string) => {
    ...
    console.log(
      `[FileUploader ${sessionIdForLog.value}] Preparing ${batch.directories.length} remote directories for ${prepareId}.`,
    );
  ...
  const requestUploadStartNow = (uploadId: string, restart = false): boolean => {
    ...
    console.log(
      `[FileUploader ${sessionIdForLog.value}] Starting upload ${uploadId} to ${transfer.remotePath}${restart ? ' after reconnect' : ''}`,
    );
    wsDeps.value.sendMessage({
  ...
  const pumpUpload = async (uploadId: string): Promise<void> => {
    ...
    try {
      ...
      if (!failedUpload || failedUpload.status === 'cancelled' || uploadTransferStates.get(uploadId) !== transfer)
        return;

      if (!wsDeps.value.isConnected.value) {
        console.warn(
          `[FileUploader ${sessionIdForLog.value}] Upload ${uploadId} paused because the WebSocket disconnected.`,
        );
      ...
      failedUpload.error =
        error instanceof Error && error.message ? error.message : t('fileManager.errors.readFileError');
  ...
  const startFileUploadBatch = (files: UploadBatchFile[], directories: string[] = []) => {
    ...
    const pipelineMode: UploadPipelineMode =
      files.length === 1 && directories.length === 0 && preparedFiles[0]?.relativeDirectory === '' ? 'single' : 'batch';
    const initialTuning = getUploadTuning();
    const initialChunkSize = pipelineMode === 'single' ? initialTuning.singleChunkSize : initialTuning.batchChunkSize;
    const initialByteWindow =
      pipelineMode === 'single' ? initialTuning.singleByteWindow : initialTuning.batchByteWindow;
    console.log(
  ...
  const onUploadPrepareError = (payload: MessagePayload) => {
    ...
    console.error(
      `[FileUploader ${sessionIdForLog.value}] Remote directory preparation failed for ${prepareId}: ${errorMessage}`,
    );
    for (const uploadId of [...batch.uploadIds]) {
      const upload = uploads[uploadId];
      if (upload) {
  ...
  const onUploadProgress = (payload: MessagePayload, message: WebSocketMessage) => {
    ...
    if (upload && upload.status === 'uploading') {
      ...
      if (typeof payload?.bytesWritten === 'number' && typeof payload?.totalSize === 'number') {
        ...
        upload.progress =
          payload.totalSize === 0 ? 100 : Math.min(100, (upload.bytesWritten / payload.totalSize) * 100);
      } else {
        console.warn(
  ...
  const onUploadChunkAck = (payload: MessagePayload, message: WebSocketMessage) => {
    ...
    if (upload && typeof payload?.bytesWritten === 'number') {
      ...
      upload.progress =
        transfer.file.size === 0 ? 100 : Math.min(100, (upload.bytesWritten / transfer.file.size) * 100);
    } else if (upload && typeof payload?.progress === 'number') {
      upload.progress = Math.min(100, Math.max(upload.progress, payload.progress));
    }

    const acknowledgedChunkIndex = typeof payload?.chunkIndex === 'number' ? payload.chunkIndex : undefined;
