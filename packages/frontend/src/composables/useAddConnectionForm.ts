...
export function useAddConnectionForm(props: AddConnectionFormProps, emit: AddConnectionFormEmits) {
  ...
  const parseIpRange = (ipRangeStr: string): string[] | { error: string } => {
    ...
    if (startIpParts.slice(0, 3).join('.') !== endIpParts.slice(0, 3).join('.')) {
      return {
        error: t('connections.form.errorIpRangeNotSameSubnet', 'IP 范围必须在同一个C段子网中 (例如 1.2.3.x ~ 1.2.3.y)'),
      };
    }
  ...
  const parseScriptLine = (
    ...
    if (!match) {
      return {
        ...
        tags: [],
        note: null,
        error: t('connections.form.scriptErrorInvalidUserHostPortFormat', { part: userHostPortPart }),
      };
    }
      ...
      if (isNaN(port) || port <= 0 || port > 65535) {
        uiNotificationsStore.showError(
          t('connections.form.scriptErrorInvalidPort', {
            line,
            port: portStr || (parsed.type === 'RDP' ? '3389' : parsed.type === 'VNC' ? '5900' : '22'),
          }),
        );
        allConnectionsValid = false;
        break;
  ...
  const handleSubmit = async () => {
    ...
    if (formData.type === 'SSH') {
      ...
      if (formData.rdpRemoteAppEnabled && !formData.rdpRemoteApp.trim()) {
        uiNotificationsStore.showError(
          t('connections.form.errorRemoteAppAliasRequired', '启用 RemoteApp 时必须填写 RemoteApp 别名。'),
        );
        return;
      }
    ...
    if (!isEditMode.value && formData.host.includes('~')) {
      ...
      if (Array.isArray(parsedIpsResult)) {
        ...
        if (formData.type === 'SSH' && formData.auth_method === 'password' && !formData.password) {
          uiNotificationsStore.showError(
            t('connections.form.errorPasswordRequiredForBatchSSH', '批量添加 SSH (密码认证) 连接时，必须提供密码。'),
          );
          return;
        }
        if (formData.type === 'RDP' && !formData.password) {
          uiNotificationsStore.showError(
            t('connections.form.errorPasswordRequiredForBatchRDP', '批量添加 RDP 连接时，必须提供密码。'),
          );
          return;
        }
        if (formData.type === 'VNC' && !formData.vncPassword) {
          uiNotificationsStore.showError(
            t('connections.form.errorPasswordRequiredForBatchVNC', '批量添加 VNC 连接时，必须提供 VNC 密码。'),
          );
          return;
        }
    ...
    if (isEditMode.value && formData.host.includes('~')) {
      uiNotificationsStore.showError(
        t('connections.form.errorIpRangeNotAllowedInEditMode', '编辑模式下不支持 IP 范围。请使用单个 IP 地址。'),
      );
      return;
    }
  ...
  const handleDeleteConnection = async () => {
    ...
    if (success) {
      ...
      uiNotificationsStore.showError(
        t('connections.form.errorDelete', { error: connectionsStore.error || t('errors.unknown', '未知错误') }),
      );
    }
  };
