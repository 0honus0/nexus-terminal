...
export function createSftpActionsManager(
  ...
  const loadDirectory = (path: string, forceRefresh: boolean = false) => {
    ...
    console.info(`[SFTP ${instanceSessionId}] ${forceRefresh ? '强制' : ''}加载目录: ${path}`); // 日志改为中文，并标明是否强制
      ...
      unregisterError = onMessage(
        ...
        (payload: MessagePayload, message: WebSocketMessage) => {
          ...
          if (message.requestId === requestId) {
            ...
            unregisterError?.();
            const errorMsg =
              errorPayload.details || errorPayload.error || t('fileManager.errors.compressFailed'); // 基础错误信息
        ...
        (payload: MessagePayload, message: WebSocketMessage) => {
          ...
          if (message.requestId === requestId) {
            ...
            const errorMsg =
              ...
              errorPayload.error ||
              t('fileManager.errors.decompressFailed'); // 基础错误信息
            uiNotificationsStore.showError(
  ...
  const removeNodeFromTree = (parentPath: string, filename: string): boolean => {
    ...
    console.warn(
      `[SFTP ${instanceSessionId}] 尝试从文件树 ${parentPath} 移除节点 ${filename} 失败`
    );
    return false;
  };
  ...
  const addOrUpdateNodeInTree = (parentPath: string, item: FileListItem): boolean => {
    ...
    if (parentNode) {
      ...
      const existingIndex = parentNode.children.findIndex(
        (node) => node.filename === item.filename
      );
      if (existingIndex !== -1) {
        // 更新现有节点
  ...
  return {
    // State
    fileList, // 暴露计算属性 (类型已在接口中定义为 Readonly<ComputedRef>)
    isLoading, // (类型已在接口中定义为 Readonly<Ref>)
    // error: readonly(error), // 移除 error
    fileTree, // (类型已在接口中定义为 Readonly<FileTreeNode>)
    initialLoadDone, // (类型已在接口中定义为 Readonly<Ref>)
