...
const parentDirectoryPath = (path: string): string => {
  if (path === '/') return '/';
...
export function createSftpActionsManager(
  ...
  let directoryLoadTimeout: ReturnType<typeof setTimeout> | null = null;
  const clearDirectoryLoadTimeout = () => {
    if (directoryLoadTimeout !== null) {
      clearTimeout(directoryLoadTimeout);
  ...
  const hasActiveOperations = computed(() => archiveProgress.active || Object.keys(transferTasks).length > 0);
  ...
  const removeTransferLater = (requestId: string, delay = 800) => {
    window.setTimeout(() => {
      delete transferTasks[requestId];
    }, delay);
  };
  ...
  const findNodeByPath = (root: FileTreeNode, path: string, createIfMissing: boolean = false): FileTreeNode | null => {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (currentNode.children) {
        ...
        // currentNode.children might be [] for a file
      ...
      if (!nextNode) {
        ...
        if (createIfMissing) {
          ...
          nextNode = reactive({
            // Use reactive for placeholders too
            filename: part,
          ...
          if (!currentNode.children) {
            // Should have been initialized above if null
            currentNode.children = [];
          }
  ...
  const loadDirectory = (path: string, forceRefresh: boolean = false) => {
    ...
    clearDirectoryLoadTimeout();
    directoryLoadTimeout = setTimeout(() => {
      if (loadingRequestId.value !== requestId) return;
      console.error(`[SFTP ${instanceSessionId}] 加载目录 ${path} 超时 (ID: ${requestId})`);
  ...
  const copyItemsFromSession = (
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `[SFTP ${instanceSessionId}] 发送 sftp:cross_copy 请求 (ID: ${requestId}) Source session: ${sourceSessionId}, Sources: ${sourcePaths.join(', ')}, Dest: ${destinationDir}`,
      );
    });
  };
  ...
  const compressItems = (
    ...
    return new Promise((resolve, reject) => {
      ...
      unregisterSuccess = onMessage('sftp:compress:success', (payload: MessagePayload, message: WebSocketMessage) => {
        ...
        if (successPayload.warning) {
          uiNotificationsStore.showWarning(
            t('fileManager.notifications.compressSuccessWithWarning', {
              name: archiveName,
      ...
      unregisterCancelled = onMessage(
        'sftp:archive:cancelled',
        (payload: MessagePayload, message: WebSocketMessage) => {
          ...
          if (payload?.cleaned === false) {
            uiNotificationsStore.showWarning(
              t(
                'fileManager.archiveProgress.cancelledCleanupFailed',
                '压缩已停止，但连接中断导致临时文件可能需要手动清理',
              ),
            );
          } else {
      ...
      unregisterCancelled = onMessage(
        ...
        (payload: MessagePayload, message: WebSocketMessage) => {
          ...
          uiNotificationsStore.showInfo(
            t('fileManager.archiveProgress.decompressCancelled', '解压已停止；已解出的部分文件会保留。'),
          );
          reject(new Error('ARCHIVE_CANCELLED'));
        },
      );
  ...
  const onSftpReaddirError = (payload: MessagePayload, message: WebSocketMessage) => {
    ...
    // Reset first so a recovery request is not blocked by the in-flight guard.
    isLoading.value = false;
    loadingRequestId.value = null;
