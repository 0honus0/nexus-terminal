  ...
  const handleRegisterNewPasskey = async () => {
    ...
    try {
      ...
      if (
        ...
        passkeyMessage.value =
          error.response?.data?.message || error.message || t('settings.passkey.error.registrationFailed');
      }
      passkeySuccess.value = false;
    } finally {
      passkeyRegistrationLoading.value = false;
  ...
  const handleDeletePasskey = async (credentialID: string) => {
    ...
    try {
      ...
      passkeyDeleteError.value =
        error.response?.data?.message || error.message || t('settings.passkey.error.deleteFailedGeneral');
      passkeySuccess.value = false;
    } finally {
