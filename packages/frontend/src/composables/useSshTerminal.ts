...
export function createSshTerminalManager(
  ...
  const bufferTerminalOutput = (data: string | Uint8Array, acknowledge?: () => void) => {
    ...
    while (
      bufferedOutputBytes > MAX_BUFFERED_OUTPUT_BYTES &&
      terminalOutputBuffer.length - terminalOutputBufferHead > 1
    ) {
  ...
  const sendInputData = (data: string): void => {
    ...
    if (data.length <= DIRECT_INPUT_MAX_CODE_UNITS && inputQueue.length === 0 && inputBytesInFlight === 0) {
      sendMessage({
        type: 'ssh:input',
