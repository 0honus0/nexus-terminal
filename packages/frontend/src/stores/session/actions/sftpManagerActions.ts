...
export const getOrCreateSftpManager = (
  ...
  if (!session) {
    console.error(
      `[SftpManagerActions] 尝试为不存在的会话 ${sessionId}（解析为 ${resolvedSessionId}）获取 SFTP 管理器`,
    );
    return null;
  }
  ...
  if (session) {
    ...
    if (manager) {
      ...
      if (manager.hasActiveOperations.value) {
        console.log(
          `[SftpManagerActions] 会话 ${resolvedSessionId} 的 SFTP 管理器 ${instanceId} 仍有长任务，延迟清理直到任务结束。`,
        );
