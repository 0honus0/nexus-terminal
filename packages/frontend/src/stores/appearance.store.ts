...
const normalizeWindowThemeColor = (color: string | undefined | null): string => {
  ...
  return value && /^#[0-9a-f]{6}$/i.test(value) ? value.toUpperCase() : DEFAULT_WINDOW_THEME_COLOR;
};
...
export const useAppearanceStore = defineStore('appearance', () => {
  ...
  const currentUiTheme = computed<Record<string, string>>(() => {
    ...
    return normalizeUiTheme(
      typeof parsed === 'object' && parsed !== null && !Array.isArray(parsed) ? (parsed as Record<string, string>) : {},
    );
  });
  ...
  const currentTerminalFontFamily = computed<string>(() => {
    return (
      appearanceSettings.value.terminalFontFamily || 'Consolas, "Courier New", monospace, "Microsoft YaHei", "微软雅黑"'
    ); // 提供默认值
  });
  ...
  async function fetchRemoteHtmlPresets(repoUrlParam?: string) {
    ...
    try {
      ...
      if (repoUrlParam) {
        // Only send if explicitly passed to this action
  ...
  ); // 添加 immediate: true 确保初始加载时应用默认主题
