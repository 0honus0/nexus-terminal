...
export const useAppearanceStore = defineStore('appearance', () => {
  ...
  const currentTerminalTheme = computed<ITheme>(() => {
    ...
    const activeTheme = allTerminalThemes.value.find(
      (t) => parseInt(t._id ?? '-1', 10) === activeId
    );
  ...
  const currentTerminalFontFamily = computed<string>(() => {
    return (
      appearanceSettings.value.terminalFontFamily ||
  ...
  const currentEditorFontFamily = computed<string>(() => {
    return (
      appearanceSettings.value.editorFontFamily || 'Consolas, "Noto Sans SC", "Microsoft YaHei"'
    ); // 提供默认值
  });
