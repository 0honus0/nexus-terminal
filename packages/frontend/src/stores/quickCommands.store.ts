...
export interface QuickCommandFE {
  // Renamed from QuickCommand if necessary
  id: number;
  name: string | null;
  command: string;
  usage_count: number;
...
export const useQuickCommandsStore = defineStore('quickCommands', () => {
    ...
    try {
      ...
      if (cachedData) {
        ...
        if (
          ...
          parsedData.every(
            (item) =>
              Array.isArray(item.tagIds) && (item.variables === undefined || typeof item.variables === 'object'),
          )
        ) {
  ...
  const addQuickCommand = async (
    ...
    try {
      ...
      const response = await apiClient.post<{ message: string; command: QuickCommandFE }>('/quick-commands', {
        name,
        command,
        tagIds,
        variables,
  ...
  const updateQuickCommand = async (
    ...
    try {
      ...
      const response = await apiClient.put<{ message: string; command: QuickCommandFE }>(`/quick-commands/${id}`, {
        name,
        command,
        tagIds,
        variables,
