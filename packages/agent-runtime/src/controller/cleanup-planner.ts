  ...
  async runtimeCleanup(userId: number): Promise<{ deleted: string[]; quarantined: string[] }> {
    ...
    const activeEnvironmentIds = new Set(
      this.journal
        ...
        .map((job) => job.environmentId),
    );
    for (const env of this.journal.environments()) {
      if (
        env.userId !== userId ||
        env.retained ||
        activeStatuses.has(env.status) ||
        activeEnvironmentIds.has(env.environmentId)
      ) {
        continue;
      }
      try {
        ...
        const target = path.join(this.root, 'quarantine', `environment-${env.environmentId}`);
        ...
        quarantined.push(env.environmentId);
      }
    }
