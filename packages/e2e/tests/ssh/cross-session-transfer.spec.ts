...
import { E2E_SSH, resetTestSshFilesystem, setTestSshLstatDenyPrefix } from '../../support/ssh';
import {
  closeWebSocket,
  type E2eWebSocket,
...
async function runTransfer(
  ...
  const failedPromise = waitForJson(
    ...
    throw new Error(`Transfer failed: ${String(message.payload?.message ?? 'unknown error')}`);
  });
...
test('cross-session copy and move transfer data and progress over real SFTP', async ({ request }) => {
  ...
  try {
    ...
    await slowStep('cross-session directory copy works when source and target absolute paths match', async () => {
      const progress = await runTransfer(destination.socket, `cross-same-path-${crypto.randomUUID()}`, {
        mode: 'copy',
        sourceWorkspaceId: source.workspaceId,
        sources: ['/folder-seed'],
        destination: '/',
      ...
      expect(progress).toMatchObject({ totalKnown: true });
      expect(Number(progress?.totalFiles)).toBeGreaterThan(0);
      expect(Number(progress?.totalBytes)).toBeGreaterThan(0);
      await expect(readRemoteFile(destination.socket, '/folder-seed/nested.txt')).resolves.toContain('nested');
    });

    await slowStep('cross-session directory copy reuses READDIR metadata for child entries', async () => {
      await setTestSshLstatDenyPrefix('/folder-seed/');
      try {
        const progress = await runTransfer(destination.socket, `cross-folder-${crypto.randomUUID()}`, {
          mode: 'copy',
          sourceWorkspaceId: source.workspaceId,
          sources: ['/folder-seed'],
          destination: '/cross-target',
        ...
        expect(Number(progress?.totalFiles)).toBeGreaterThan(0);
        expect(Number(progress?.totalBytes)).toBeGreaterThan(0);
        await expect(readRemoteFile(destination.socket, '/cross-target/folder-seed/nested.txt')).resolves.toContain(
          'nested',
        );
      } finally {
        await setTestSshLstatDenyPrefix('');
      }
    });
