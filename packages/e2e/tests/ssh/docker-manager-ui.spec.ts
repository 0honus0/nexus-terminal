...
test('Workspace layout lock and top-navigation toggle affect the live shell and persist', async ({ page, context }) => {
  ...
  try {
    ...
    await step('a narrow File Manager collapses metadata columns and centers the loading state', async () => {
      ...
      expect(pathMetrics.toolbarPaddingLeft).toBeGreaterThan(0);
      expect(pathMetrics.toolbarPaddingLeft).toBeLessThanOrEqual(8);
      expect(Math.abs(pathMetrics.toolbarPaddingLeft - pathMetrics.toolbarPaddingRight)).toBeLessThan(0.5);
