...
try {
  ...
  guacServer.on('open', (client) => {
    ...
    console.log(
      `[Remote Gateway] Guacd 连接已建立。客户端 ID: ${clientId}, Guacamole ID: ${client.guacamoleConnectionId || '未知'}`,
    );
  });
  ...
  guacServer.on('error', (client, error) => {
    console.error(
      `[Remote Gateway] GuacamoleLite 客户端错误。客户端 ID: ${client.connectionId || '未知客户端ID'}:`,
      error,
    );
  });
} catch (error) {
...
app.post('/api/remote-desktop/token', (req: Request, res: Response): void => {
  ...
  if (
    ...
    !Number.isInteger(normalizedDpi) ||
    normalizedDpi < 48 ||
    normalizedDpi > 480
  ) {
  ...
  if (
    (username !== undefined && (typeof username !== 'string' || username.length > 512)) ||
