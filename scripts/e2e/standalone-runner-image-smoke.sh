...
for _ in {1..30}; do
  if availability="$(curl -fsS \
    ...
    if RUNNER_AVAILABILITY="$availability" node -e '
      ...
      process.exit(value.available === true ? 0 : 1);
    '; then
      break
    fi
  fi
...
RUNNER_AVAILABILITY="$availability" node -e '
  ...
  if (value.mode !== "native" || value.isolation !== "logical") {
    console.error("Runner runtime mode mismatch:", value);
    process.exit(1);
  }
'
...
RUNNER_BASE_URL="$base_url" RUNNER_TOKEN="$token" node <<'NODE'
